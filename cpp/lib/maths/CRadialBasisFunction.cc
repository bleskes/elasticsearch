...
//! </pre>
double gaussianSquareDerivative(double x,
                                double centre,
                                double scale)
{
...
//! </pre>
double gaussianProduct(double x,
                       double centre1,
                       double centre2,
                       double scale1,
                       double scale2)
{
    ...
    double m = (scale1 * scale1 * centre1
                + scale2 * scale2 * centre2) / (scale * scale);
...
//! </pre>
double inverseQuadraticSquareDerivative(double x,
                                        double centre,
                                        double scale)
{
    double r = scale * (x - centre);
...
//! </pre>
double inverseQuadraticProduct(double x,
                               double centre1,
                               double centre2,
                               double scale1,
                               double scale2)
{
    ...
    if ((d * d) > 1.0)
    {
        return (  scale1 * scale2 / d * ::log((1.0 + r1 * r1) / (1.0 + r2 * r2))
                + scale1 * (1.0 - (ss * sd) / (d * d)) * ::atan(r1)
                + scale2 * (1.0 + (ss * sd) / (d * d)) * ::atan(r2))
               / ((1.0 + (ss * ss) / (d * d)) * (d * d + sd * sd));
    }
    return (  scale1 * scale2 * d * ::log((1.0 + r1 * r1) / (1.0 + r2 * r2))
            + (d * d - ss * sd) * scale1 * ::atan(r1)
...
double CGaussianBasisFunction::value(double x,
                                     double centre,
                                     double scale) const
{
    double r = x - centre;
...
double CGaussianBasisFunction::derivative(double x,
                                          double centre,
                                          double scale) const
{
    double r = x - centre;
...
bool CGaussianBasisFunction::scale(double distance,
                                   double value,
                                   double &result) const
{
...
double CGaussianBasisFunction::mean(double a,
                                    double b,
                                    double centre,
                                    double scale) const
{
    ...
    return std::max(boost::math::double_constants::root_pi / 2.0 / scale
                    * (boost::math::erf(scale * (b - centre))
                       - boost::math::erf(scale * (a - centre))) / (b - a), 0.0);
}
                                                    ...
                                                    double scale) const
{
    ...
    return std::max((gaussianSquareDerivative(b, centre, scale)
                     - gaussianSquareDerivative(a, centre, scale)) / (b - a), 0.0);
}
                                       ...
                                       double scale2) const
{
    ...
    return std::max((gaussianProduct(b, centre1, centre2, scale1, scale2)
                     - gaussianProduct(a, centre1, centre2, scale1, scale2)) / (b - a), 0.0);
}
...
double CInverseQuadraticBasisFunction::value(double x,
                                             double centre,
                                             double scale) const
{
    double r = x - centre;
...
double CInverseQuadraticBasisFunction::derivative(double x,
                                                  double centre,
                                                  double scale) const
{
    double r = x - centre;
...
double CInverseQuadraticBasisFunction::mean(double a,
                                            double b,
                                            double centre,
                                            double scale) const
{
    ...
    return std::max((::atan(scale * (b - centre))
                    - ::atan(scale * (a - centre))) / scale / (b - a), 0.0);
}
                                                            ...
                                                            double scale) const
{
    ...
    double fmin = contains(a, b, centre) ? 0.0 : std::min(fa, fb);
    double fmax = (contains(a, b, maxima[0]) || contains(a, b, maxima[1])) ?
                  this->derivative(maxima[0], centre, scale) : std::max(fa, fb);
    ...
    return std::max((inverseQuadraticSquareDerivative(b, centre, scale)
                     - inverseQuadraticSquareDerivative(a, centre, scale)) / (b - a), 0.0);
}

bool CInverseQuadraticBasisFunction::scale(double distance,
                                           double value,
                                           double &result) const
{
    if (value <= 0.0 || value >= 1.0)
                                               ...
                                               double scale2) const
{
    ...
    return std::max((inverseQuadraticProduct(b, centre1, centre2, scale1, scale2)
                     - inverseQuadraticProduct(a, centre1, centre2, scale1, scale2)) / (b - a), 0.0);
}
