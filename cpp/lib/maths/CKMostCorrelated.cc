...
#endif
typedef void (CKMostCorrelated::TSizeVectorPackedBitVectorPrUMap::*TSizeVectorPackedBitVectorPrUMapReserve)(std::size_t);
typedef void (CKMostCorrelated::TSizeVectorUMap::*TSizeVectorUMapReserve)(std::size_t);
const TSizeVectorUMapReserve PROJECTED_RESERVE = &CKMostCorrelated::TSizeVectorUMap::reserve;
const TSizeVectorPackedBitVectorPrUMapReserve CURRENT_PROJECTED_RESERVE = &CKMostCorrelated::TSizeVectorPackedBitVectorPrUMap::reserve;
...
void CKMostCorrelated::mostCorrelated(TSizeSizePrVec &result) const
{
    ...
    if (N > 0)
    {
        ...
        {
            result.push_back(std::make_pair(m_MostCorrelated[i].s_X, m_MostCorrelated[i].s_Y));
        }
    }
}
                                      ...
                                      TDoubleVec *pearson) const
{
    ...
    if (n > 0)
    {
        ...
        for (std::size_t i = 0u; i < n; ++i)
        {
            correlates.push_back(std::make_pair(m_MostCorrelated[i].s_X, m_MostCorrelated[i].s_Y));
            if (pearson)
            {
                pearson->push_back(maths::CBasicStatistics::mean(m_MostCorrelated[i].s_Correlation));
            }
        }
    }
}
...
void CKMostCorrelated::correlations(TDoubleVec &result) const
{
    ...
    if (N > 0)
    {
        ...
        for (std::size_t i = 0u; i < N; ++i)
        {
            result.push_back(maths::CBasicStatistics::mean(m_MostCorrelated[i].s_Correlation));
        }
    }
}
...
{
    core::CAllocationStrategy::resize(m_Moments, n);
}
    ...
    for (std::size_t i = 0u; i < remove.size(); ++i)
    {
        if (remove[i] < m_Moments.size())
        {
            ...
            m_MostCorrelated.erase(std::remove_if(m_MostCorrelated.begin(),
                                                  ...
                                                  CMatches(remove[i])), m_MostCorrelated.end());
        }
    }
}
         ...
         ++i)
    {
        ...
        if (j == m_Projected.end())
        {
            ...
            j = m_Projected.emplace(boost::unordered::piecewise_construct,
                                    boost::make_tuple(X),
                                    boost::make_tuple(zero, indicator)).first;
        }
...
void CKMostCorrelated::mostCorrelated(TCorrelationVec &result) const
{
    ...
    std::size_t replace = std::max(static_cast<std::size_t>(
                                         REPLACE_FRACTION * static_cast<double>(desired) + 0.5),
    ...
    if (10 * replace > V * (V - 1))
    {
             ...
             ++x)
        {
            ...
            while (++y != m_Projected.end())
            {
                ...
                {
                    SCorrelation cxy(X, x->second.first, x->second.second,
    ...
    else
    {
        ...
        for (TSizeVectorPackedBitVectorPrUMapCItr i = m_Projected.begin();
             ...
             ++i)
        {
            points.push_back(TPointSizePr(i->second.first.to<double>().toBoostArray(), i->first));
        }
        ...
        try
        {
            ...
            for (std::size_t i = 0u; i < seeds.size(); ++i)
            {
                ...
                for (std::size_t j = 0u; j < nearest.size(); ++j)
                {
                    ...
                    std::size_t S = mostCorrelated.biggest().s_X;
                    std::size_t T = mostCorrelated.biggest().s_Y;
...
{
    return COrderings::lexicographical_compare(-this->absCorrelation(), s_X, s_Y,
...
double CKMostCorrelated::SCorrelation::distance(double amax) const
{
    return static_cast<double>(NUMBER_PROJECTIONS)
           * amax
           * 2.0 * (1.0 - ::fabs(maths::CBasicStatistics::mean(s_Correlation)));
}
...
{
    return         maths::CBasicStatistics::print(s_Correlation)
