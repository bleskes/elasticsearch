...
const std::string CTimeSeriesTestData::DEFAULT_BIVALUED_REGEX("\\s*(\\d+|\\d+\\.\\d+)\\s+([-]*\\d+|\\d+\\.\\d+|\\d+\\.\\d+e-?\\d+)\\s+([-]*\\d+|\\d+\\.\\d+|\\d+\\.\\d+e-?\\d+)\\s*");
...
const std::string CTimeSeriesTestData::CSV_SPLUNK_REGEX("^\"(\\d+-\\d+-\\d+T\\d+:\\d+:\\d+)\\..*\",[\"]*([-]*[\\d\\.]+)[\"]*");
const std::string CTimeSeriesTestData::CSV_SPLUNK_BIVALUED_REGEX("^\"(\\d+-\\d+-\\d+T\\d+:\\d+:\\d+)\\..*\",[\"]*([-]*[\\d\\.]+)[\"]*, [\"]*([-]*[\\d\\.]+)[\"]*");
const std::string CTimeSeriesTestData::CSV_SPLUNK_DATE_FORMAT("%Y-%m-%dT%H:%M:%S");
                                ...
                                const std::string &dateFormat)
{
    ...
    return CTimeSeriesTestData::parse(fileName, results,
                                      unused, unused,
                                      regex, dateFormat);
}
...
bool CTimeSeriesTestData::parseCounter(const std::string &fileName, TTimeDoublePrVec &results)
{
    ...
    double last(0);
    bool   started(false);
    for (TTimeDoublePrVecItr itr = results.begin(); itr != results.end(); ++itr)
    {
        ...
        {
            itr->second = value - last;
            if (itr->second < 0)
            {
                LOG_WARN("Negative value " << value << "<" << last << "@" << itr->first << " setting counter to 0 ");
                itr->second = 0;
            }
        }
        last = value;
...
void CTimeSeriesTestData::transform(const TTimeDoublePrVec &data, TDoubleVec &results)
{
    ...
    for (TTimeDoublePrVecCItr itr = data.begin(); itr != data.end(); ++itr)
    {
...
void CTimeSeriesTestData::derive(const TTimeDoublePrVec &data, TTimeDoublePrVec &results)
{
    ...
    for (TTimeDoublePrVecCItr itr = data.begin(); itr != data.end(); ++itr)
    {
        if (hasStarted)
        {
            double v = itr->second - lastValue;

            results.push_back(TTimeDoublePr(itr->first, v));
        }
        else
        {
            hasStarted = true;
                              ...
                              TTimeDoublePrVec &results)
{
    ...
    typedef TTimeDoubleMap::iterator        TTimeDoubleMapItr;

    TTimeDoubleMap dataMap;

    for (TTimeDoublePrVecCItr itr = data.begin(); itr != data.end(); ++itr)
    {
        if (dataMap.insert(TTimeDoubleMap::value_type(itr->first, itr->second)).second == false)
        {
            LOG_ERROR("Duplicate values " << itr->first);
            return false;
        }
    }
    ...
    {
        TTimeDoubleMapItr itr = dataMap.find(t);
        if (itr == dataMap.end())
        {
            results.push_back(TTimeDoublePr(t, 0));
        }
        else
        {
            results.push_back(TTimeDoublePr(t, itr->second));
        }
    }
                                ...
                                core_t::TTime &maxTime)
{
    ...
    if (file.readAllLines(boost::bind(&CTimeSeriesTestData::parseLine<T>,
                                      boost::cref(tokenRegex),
                                      boost::cref(dateFormat),
                                      _1,
                                      boost::ref(results)),
                          remainder) == false)
    {
                                    ...
                                    std::vector<std::pair<core_t::TTime, T> > &results)
{
    ...
    results.push_back(std::make_pair(time, T()));
