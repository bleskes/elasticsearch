                                     ...
                                     double &result)
{
    ...
    static const double CONSTANT = ::log(boost::math::double_constants::root_two_pi) - 2.0;

    double p_ = m_ / n_;
    result = - 0.5 * ::log(n_ * (1.0 - p_) * p_)
             + m_ * ::log(p / p_)
             + (n_ - m_) * ::log((1.0 - p) / (1.0 - p_))
             + CONSTANT;
    return maths_t::E_FpNoErrors;
}
                                                                     ...
                                                                     double &result)
{
    ...
    try
    {
        ...
        double q = p + eps;
        double chernoff = m_ * (  q * ::log(p / q)
                                + (1.0 - q) * ::log((1.0 - p) / (1.0 - q)));
                                                                     ...
                                                                     double &result)
{
    ...
    try
    {
        ...
        double bound = logf + ::log(1.0 +   n_ / (m_ + 1.0)
                                          * (::exp(p / (1.0 - p) * (n_ - m_) / n_) - 1.0));
                                                           ...
                                                           double &result)
{
    ...
    try
    {
        ...
        double f = status == maths_t::E_FpOverflowed ? 0.0 : ::exp(logf);
        result = std::min(::log(oneMinusF + f) + LOG_TWO, 0.0);
        return maths_t::E_FpNoErrors;
    }
    catch (const std::exception &e)
    {
                                             ...
                                             double &result)
{
    ...
    for (std::size_t i = 0u; i < ni.size(); ++i)
    {
        ...
        if (ni_ > 0.0)
        {
            ...
            logP += ni_ * ::log(pi_) - boost::math::lgamma(ni_ + 1.0);
        }
    }
