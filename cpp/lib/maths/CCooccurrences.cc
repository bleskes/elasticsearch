...
struct SCooccurrence
{
    ...
    {
        return s_Nxy * rhs.s_X * rhs.s_Y < rhs.s_Nxy *s_Nx * s_Ny;
    }
                                        ...
                                        TDoubleVec &significances) const
{
    ...
    for (std::size_t i = 0u; i < mostSignificant.count(); ++i)
    {
        ...
        top.push_back(std::make_pair(co.s_X, co.s_Y));
...
{
    seed = maths::CChecksum::calculate(seed, m_MaximumLength);
    seed = maths::CChecksum::calculate(seed, m_Length);
    seed = maths::CChecksum::calculate(seed, m_IndicatorWidth);
    seed = maths::CChecksum::calculate(seed, m_Offset);
    seed = maths::CChecksum::calculate(seed, m_CurrentIndicators);
    return maths::CChecksum::calculate(seed, m_Indicators);
}
