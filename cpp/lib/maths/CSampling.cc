...
//! Implementation of integer uniform sampling.
template<typename RNG>
void doUniformSample(RNG &rng, std::size_t a, std::size_t b, std::size_t n, TSizeVec &result)
{
    result.clear();
    result.reserve(n);
...
//! Implementation of real uniform sampling.
template<typename RNG>
double doUniformSample(RNG &rng, double a, double b)
{
    boost::random::uniform_real_distribution<double> uniform(a, b);
    return uniform(rng);
}

//! Implementation of real uniform sampling.
template<typename RNG>
void doUniformSample(RNG &rng, double a, double b, std::size_t n, TDoubleVec &result)
{
    result.clear();
    result.reserve(n);
    boost::random::uniform_real_distribution<double> uniform(a, b);
...
std::size_t doCategoricalSample(RNG &rng, TDoubleVec &probabilities)
{
    ...
    {
        TSizeVec result;
        doUniformSample(rng, 0, p, 1, result);
        return result[0];
    }
    else
    {
                                        ...
                                        TSizeVec &result)
{
    ...
    if (probabilities[p - 1] == 0.0)
    {
        doUniformSample(rng, 0, p, n, result);
    }
    else
    {
        result.reserve(n);
                                           ...
                                           TSizeVec &result)
{
    ...
    for (std::size_t i = 0u; i < n; ++i, --p)
    {
        if (probabilities[p - 1] <= 0.0)
        {
            doUniformSample(rng, 0, indices.size(), 1, s);
...
{
    typedef Eigen::JacobiSVD<Eigen::MatrixXd> TJacobiSvd;
...
{
    typedef typename SEigenVectorNx1<N>::Type TEigenVector;
    typedef typename SEigenMatrixNxN<N>::Type TEigenMatrix;
    typedef Eigen::JacobiSVD<TEigenMatrix> TJacobiSvd;
    ...
    TJacobiSvd svd(covariance.toEigen(), Eigen::ComputeFullU | Eigen::ComputeFullV);

    // Get the singular values, these are the variances of the normals
    // to sample.
    const TEigenVector &S = svd.singularValues();
    const TEigenMatrix &U = svd.matrixU();
    T stddevs[N] = {};
    for (std::size_t i = 0u; i < N; ++i)
    {
                               ...
                               TSizeVec &sampling)
{
    ...
    if (::fabs(totalRemainder) > 0.5)
    {
        ...
        for (std::size_t i = 0u; i < choices.size(); ++i)
        {
            ...
            {
                candidates.push_back(TDoubleSizePr(-::fabs(remainders[choices[i]][i]), i));
            }
        }
...
#define MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(N)                                                     \
void CSampling::multivariateNormalSampleDelegate(const CVectorNx1<double, N> &mean,                \
...
void CSampling::multivariateNormalSampleDelegate(CPRNG::CXorOShiro128Plus &rng,                    \
...
void CSampling::multivariateNormalSampleDelegate(CPRNG::CXorShift1024Mult &rng,                    \
                                                 const CVectorNx1<double, N> &mean,                \
                                                 const CSymmetricMatrixNxN<double, N> &covariance, \
                                                 std::size_t n,                                    \
                                                 std::vector<CVectorNx1<double, N> > &samples)     \
...
MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(2)
MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(3)
MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(4)
MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(5)
MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(6)
MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(7)
MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(8)
MULTIVARIATE_NORMAL_SAMPLE_DELEGATE(9)
#undef MULTIVARIATE_NORMAL_SAMPLE_DELEGATE
...
void CSampling::uniformSample(std::size_t a, std::size_t b, std::size_t n, TSizeVec &result)
{
    core::CScopedFastLock scopedLock(ms_Lock);
...
double CSampling::uniformSample(double a, double b)
{
    core::CScopedFastLock scopedLock(ms_Lock);
    return doUniformSample(ms_Rng, a, b);
}

double CSampling::uniformSample(CPRNG::CXorOShiro128Plus &rng, double a, double b)
{
    return doUniformSample(rng, a, b);
}

double CSampling::uniformSample(CPRNG::CXorShift1024Mult &rng, double a, double b)
{
    return doUniformSample(rng, a, b);
}

void CSampling::uniformSample(double a, double b, std::size_t n, TDoubleVec &result)
{
    core::CScopedFastLock scopedLock(ms_Lock);
    doUniformSample(ms_Rng, a, b, n, result);
}
