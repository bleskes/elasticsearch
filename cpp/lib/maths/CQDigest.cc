...
CQDigest::CQDigest(uint64_t k,
                   double decayRate) :
        m_K(k),
        m_N(0u),
        m_Root(0),
        m_NodeAllocator(static_cast<std::size_t>(3 * m_K + 2)),
...
void CQDigest::merge(const CQDigest &digest)
{
    ...
    for (TNodePtrVecCItr i = nodes.begin(); i != nodes.end(); ++i)
    {
        m_N += (*i)->count();
        m_Root->insert(m_NodeAllocator, **i);
    }
...
bool CQDigest::scale(double factor)
{
    ...
    for (TNodePtrVecCItr i = nodes.begin(); i != nodes.end(); ++i)
    {
        sketch.push_back(TUInt32UInt32UInt64Tr((*i)->min(),
                                               (*i)->max(),
                                               (*i)->count()));
    }
        ...
        if (count > 0)
        {
            for (uint32_t j = 0u; j < span; ++j)
            {
                this->add(static_cast<uint32_t>(
                              factor * static_cast<double>(min + j) + 0.5), count);
            }
        }
        if (remainder > 0)
        {
...
bool CQDigest::quantileSublevelSetSupremum(double f,
...
double CQDigest::cdfQuantile(double n, double p, double q)
{
    ...
    {
        LOG_ERROR("Failed to calculate c.d.f. quantile: " << e.what()
                  << ", n = " << n
                  << ", p = " << p
                  << ", q = " << q);
    }
                   ...
                   double &upperBound) const
{
    ...
    if (confidence > 0.0)
    {
        lowerBound = cdfQuantile(static_cast<double>(m_N),
                                 lowerBound,
    ...
    if (confidence > 0.0)
    {
        upperBound = cdfQuantile(static_cast<double>(m_N),
                                 upperBound,
                                 (100.0 + confidence) / 200.0);
    }
                   ...
                   double &upperBound) const
{
    ...
    lowerBound =   std::max(supremumLowerBound - infimumUpperBound, 0.0)
                 / std::max(static_cast<double>(supremum - infimum), 1.0);
    upperBound =   std::max(supremumUpperBound - infimumLowerBound, 0.0)
                 / std::max(static_cast<double>(supremum - infimum), 1.0);

    LOG_TRACE("x = " << x
              << ", supremum = " << supremum
              << ", infimum = " << infimum
              << ", cdf(supremum) = [" << supremumLowerBound
              << "," << supremumUpperBound << "]"
              << ", cdf(infimum) = [" << infimumLowerBound
              << "," << infimumUpperBound << "]"
...
void CQDigest::summary(TUInt32UInt64PrVec &result) const
{
    ...
    for (std::size_t i = 1u; i < nodes.size(); ++i)
    {
        if (nodes[i]->max() != last)
        {
            result.push_back(TUInt32UInt64Pr(last, count));
            last = nodes[i]->max();
        }
    ...
    {
        result.push_back(TUInt32UInt64Pr(m_Root->max(), count));
    }
...
{
    seed = maths::CChecksum::calculate(seed, m_K);
    seed = maths::CChecksum::calculate(seed, m_N);
    seed = maths::CChecksum::calculate(seed, m_DecayRate);
    TUInt32UInt64PrVec summary;
    this->summary(summary);
    return maths::CChecksum::calculate(seed, summary);
}
...
std::size_t CQDigest::CNode::size(void) const
{
    ...
    for (TNodePtrVecCItr itr = m_Descendants.begin();
         itr != m_Descendants.end();
         ++itr)
    {
        size += (*itr)->size();
    }
...
uint32_t CQDigest::CNode::quantile(uint64_t leftCount,
                                   uint64_t n) const
{
    ...
    for (TNodePtrVecCItr itr = m_Descendants.begin();
         ...
         ++itr)
    {
        uint64_t count = (*itr)->subtreeCount();
        if (leftCount + count >= n)
        {
            return (*itr)->quantile(leftCount, n);
        }
        leftCount += count;
    }
                                                  ...
                                                  uint32_t &result) const
{
    ...
    for (TNodePtrVecCRItr itr = m_Descendants.rbegin();
         itr != m_Descendants.rend();
         ++itr)
    {
        leftCount -= (*itr)->subtreeCount();
        if (leftCount + (*itr)->count() < n
            && (*itr)->quantileSublevelSetSupremum(n, leftCount, result))
        {
            break;
        }
    }
...
void CQDigest::CNode::cdfLowerBound(uint32_t x,
                                    uint64_t &result) const
{
    ...
    else
    {
        for (TNodePtrVecCItr itr = m_Descendants.begin();
             ...
             ++itr)
        {
            (*itr)->cdfLowerBound(x, result);
        }
    }
}

void CQDigest::CNode::cdfUpperBound(uint32_t x,
                                    uint64_t &result) const
{
    ...
    else if (m_Min <= x)
    {
        ...
        for (TNodePtrVecCItr itr = m_Descendants.begin();
             ...
             ++itr)
        {
            (*itr)->cdfUpperBound(x, result);
        }
    }
}

void CQDigest::CNode::sublevelSetSupremum(const int64_t x,
                                          uint32_t &result) const
{
    for (TNodePtrVecCRItr itr = m_Descendants.rbegin();
         itr != m_Descendants.rend();
         ++itr)
    {
        if (static_cast<int64_t>((*itr)->max()) > x)
        {
            result = std::min(result, (*itr)->max());
        }
        else
        {
            (*itr)->sublevelSetSupremum(x, result);
            break;
        }
    }
...
void CQDigest::CNode::superlevelSetInfimum(uint32_t x,
                                           uint32_t &result) const
{
    for (TNodePtrVecCItr itr = m_Descendants.begin();
         itr != m_Descendants.end();
         ++itr)
    {
        ...
        {
            (*itr)->superlevelSetInfimum(x, result);
            break;
        }
    }
                                         ...
                                         const uint32_t &value)
{
    ...
    CNode *result = m_Count == 0 ?
                    this : &allocator.create(CNode(m_Min, m_Max, 0, 0));
...
CQDigest::CNode &CQDigest::CNode::insert(CNodeAllocator &allocator,
                                         const CNode &node)
{
    ...
    TNodePtrVecItr next = std::lower_bound(m_Descendants.begin(),
                                           m_Descendants.end(),
                                           &node,
                                           SPostLess());
    ...
    // in post order.
    for (TNodePtrVecItr itr = next; itr != m_Descendants.end(); ++itr)
    {
        if ((*itr)->isAncestor(node) || **itr == node)
        {
            return (*itr)->insert(allocator, node);
        }
    }
...
{
    inserter.insertLevel(NODE_TAG,
                         boost::bind(&CNode::acceptPersistInserter,
                                     this,
                                     _1));
    ...
    for (TNodePtrVecCItr itr = m_Descendants.begin();
         ...
         ++itr)
    {
        (*itr)->persistRecursive(nodeTag, inserter);
    }
}
...
bool CQDigest::CNode::checkInvariants(uint64_t compressionFactor) const
{
    ...
    {
        LOG_ERROR("Bad subtree count: expected " << subtreeCount
                  << " got " << m_SubtreeCount);
        return false;
    }
...
CQDigest::CNode *CQDigest::CNode::sibling(const CNode &node) const
{
    ...
    TNodePtrVecCItr next = std::lower_bound(m_Descendants.begin(),
                                            m_Descendants.end(),
                                            &sibling,
                                            SPostLess());
...
void CQDigest::CNode::removeDescendant(CNode &node)
{
    ...
    m_Descendants.erase(std::remove(m_Descendants.begin(),
                                    m_Descendants.end(), &node),
                        m_Descendants.end());
}
...
bool CQDigest::CNode::takeDescendants(CNode &node)
{
    ...
    if (!this->isAncestor(node))
    {
        ...
        for (TNodePtrVecCItr itr = node.beginDescendants();
             itr != node.endDescendants();
             ++itr)
        {
            if (this->isAncestor(**itr))
            {
                nodesToTake.push_back(*itr);
                (*itr)->m_Ancestor = this;
                m_SubtreeCount += (*itr)->subtreeCount();
            }
            else
            {
                nodesToLeave.push_back(*itr);
            }
        }
        ...
        std::merge(m_Descendants.begin(), m_Descendants.end(),
                   nodesToTake.begin(), nodesToTake.end(),
                   std::back_inserter(descendants), SPostLess());
    ...
    std::merge(m_Descendants.begin(), m_Descendants.end(),
               node.beginDescendants(), node.endDescendants(),
               std::back_inserter(descendants), SPostLess());
...
void CQDigest::CNodeAllocator::release(CNode &node)
{
    ...
    if (m_Nodes.size() > 1u)
    {
        TNodeVecListItr nodeItr = m_Nodes.begin();
...
std::size_t CQDigest::CNodeAllocator::findBlock(const CNode &node) const
{
    ...
    typedef std::less_equal<const CNode*> TNodePtrLessEqual;
    const TNodePtrLessEqual le = TNodePtrLessEqual();

    for (TNodeVecListCItr i = m_Nodes.begin(); i != m_Nodes.end(); ++i, ++result)
    {
        TNodeVecCItr first = i->begin();
        TNodeVecCItr last = i->end();
        if (first == last)
        {
            continue;
        }
