...
void setConstant(std::size_t dimension,
...
CMultivariateConstantPrior::CMultivariateConstantPrior(std::size_t dimension,
        ...
        CMultivariatePrior(maths_t::E_DiscreteData, 0.0),
...
void CMultivariateConstantPrior::setToNonInformative(double /*offset*/,
                                                     double /*decayRate*/)
{
    m_Constant.reset();
}
                                                       ...
                                                       double &result) const
{
    ...
    for (std::size_t i = 0u; i < samples.size(); ++i)
    {
        ...
        numberSamples += this->smallest(maths_t::countForUpdate(m_Dimension,
                                                                weightStyles,
                                                                weights[i]));
    }
...
void CMultivariateConstantPrior::print(const std::string &separator,
