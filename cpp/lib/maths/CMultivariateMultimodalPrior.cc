    ...
    try
    {
        ...
        for (std::size_t j = 0u; j < modes.size(); ++j)
        {
            ...
            maths_t::EFloatingPointErrorStatus status =
                    modes[j].s_Prior->jointLogMarginalLikelihood(weightStyles, sample, weights, modeLogLikelihood);
           ...
           std::string &result)
{
    ...
    for (std::size_t i = 0u; i < modes.size(); ++i)
    {
        ...
        modes[i].s_Prior->print(separator_, result);
    }
}
                       ...
                       std::size_t targetIndex)
{
    ...
    TModeVecCItr leftMode = std::find_if(modes.begin(),
                                         modes.end(),
