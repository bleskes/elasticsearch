...
//! \brief Converts a knot point to a string.
class CToString
{
    public:
        std::string operator()(const TFloatFloatPr &knot) const
        {
            return  knot.first.toString()
                  + core::CPersistUtils::PAIR_DELIMITER
                  + knot.second.toString();
        }
};

//! \brief Extracts a knot point from a string.
class CFromString
{
    public:
        bool operator()(const std::string &token, TFloatFloatPr &knot) const
        {
            std::size_t delimPos(token.find(core::CPersistUtils::PAIR_DELIMITER));
            if (delimPos == std::string::npos)
            {
                return false;
            }
            m_Token.assign(token, 0, delimPos);
            if (!this->valueFromToken(knot.first))
            {
                return false;
            }
            m_Token.assign(token, delimPos + 1, token.length() - delimPos);
            return this->valueFromToken(knot.second);
        }

    private:
        bool valueFromToken(CFloatStorage &value) const
        {
            return value.fromString(m_Token);
        }
...
bool CQuantileSketch::acceptRestoreTraverser(core::CStateRestoreTraverser &traverser)
{
    do
    {
        ...
        RESTORE(KNOTS_TAG, core::CPersistUtils::fromString(traverser.value(), CFromString(), m_Knots))
...
void CQuantileSketch::acceptPersistInserter(core::CStatePersistInserter &inserter) const
{
    ...
    inserter.insertValue(KNOTS_TAG, core::CPersistUtils::toString(m_Knots, CToString()));
...
{
    ++m_Unsorted;
    m_Knots.push_back(std::make_pair(x, n));
    m_Count += n;
...
bool CQuantileSketch::cdf(double x_, double &result) const
{
    ...
    ptrdiff_t k = std::lower_bound(m_Knots.begin(), m_Knots.end(),
                                   x, COrderings::SFirstLess()) - m_Knots.begin();
    LOG_TRACE("k = " << k);
...
{
    seed = maths::CChecksum::calculate(seed, m_MaxSize);
    for (std::size_t i = 0u; i < m_Knots.size(); ++i)
    {
        seed = maths::CChecksum::calculate(seed, m_Knots[i]);
    }
...
bool CQuantileSketch::checkInvariants(void) const
{
    ...
    {
        LOG_ERROR("Unordered knots: " << core::CContainerPrinter::print(m_Knots.begin(),
                                                                        m_Knots.end() - m_Unsorted));
        return false;
    }
    double count = 0.0;
    for (std::size_t i = 0u; i < m_Knots.size(); ++i)
...
void CQuantileSketch::reduce(void)
{
    ...
    if (m_Knots.size() > target)
    {
        ...
        {
            costs.push_back(std::make_pair(this->cost(m_Knots[i], m_Knots[i+1]), u01(rng)));
