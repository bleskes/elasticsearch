...
void CSetToolsTest::testPairGet(void)
{
    LOG_DEBUG("+------------------------------+");
    LOG_DEBUG("|  CSetToolsTest::testPairGet  |");
    LOG_DEBUG("+------------------------------+");

    typedef std::pair<int, double> TIntDoublePr;
    typedef std::vector<TIntDoublePr> TIntDoublePrVec;
    typedef boost::transform_iterator<maths::CSetTools::CPairGet<0>,
                                      TIntDoublePrVec::const_iterator,
                                      int> TFirstCItr;
    typedef boost::transform_iterator<maths::CSetTools::CPairGet<1>,
                                      TIntDoublePrVec::const_iterator,
                                      double> TSecondCItr;

    TIntDoublePr values_[] =
        {
            TIntDoublePr( 3,  4.2),
            TIntDoublePr( 1,  5.1),
            TIntDoublePr( 6,  1.2),
    ...
    TIntDoublePrVec values(boost::begin(values_), boost::end(values_));

    std::string ints = core::CContainerPrinter::print(TFirstCItr(values.begin(), maths::CSetTools::CPairGet<0>()),
                                                      TFirstCItr(values.end(), maths::CSetTools::CPairGet<0>()));
    LOG_DEBUG("ints = " << ints);
    CPPUNIT_ASSERT_EQUAL(std::string("[3, 1, 6, -1, 10]"), ints);

    std::string doubles = core::CContainerPrinter::print(TSecondCItr(values.begin(), maths::CSetTools::CPairGet<1>()),
                                                         TSecondCItr(values.end(), maths::CSetTools::CPairGet<1>()));
    LOG_DEBUG("doubles = " << doubles);
    CPPUNIT_ASSERT_EQUAL(std::string("[4.2, 5.1, 1.2, 3.6, -7.5]"), doubles);
}
