...
COneOfNPrior::TPriorPtrVec clone(const TPriorPtrVec &models,
                                 const maths::CPrior::TOptionalDouble &decayRate = maths::CPrior::TOptionalDouble())
{
...
void COneOfNPriorTest::testModelSelection(void)
{
...

    {
        ...
        boost::math::poisson_distribution<double> poisson(rate);
        boost::math::normal_distribution<double> normal(mean, ::sqrt(variance));
...

    {
        ...
        boost::math::normal_distribution<double> poissonApprox(mean, ::sqrt(mean));
        boost::math::normal_distribution<double> normal(mean, ::sqrt(variance));
...
void COneOfNPriorTest::testMarginalLikelihood(void)
{
    ...
    for (std::size_t i = 2u; i < samples.size(); ++i)
    {
        ...
        for (std::size_t j = 0u; j < 20; ++j, x += dx)
        {
            ...
            CPPUNIT_ASSERT_DOUBLES_EQUAL(fx, dFdx,
                                         std::max(1e-5, 1e-3 * FxPlusEps));
...

    {
        ...
        for (std::size_t i = 0u; i < boost::size(means); ++i)
        {
            for (std::size_t j = 0u; j < boost::size(variances); ++j)
            {
                ...
                double mode;
                double fmode;
                maths::CCompositeFunctions::CExp<maths::CPriorUtils::CLogMarginalLikelihood<maths::CPrior> > likelihood(filter);
...
void COneOfNPriorTest::testSampleMarginalLikelihood(void)
{
    ...
    // We expect 6 samples from the normal and 4 from the log normal.
    COneOfNPrior::TPriorCPtrVec posteriorModels = filter.models();
    TDouble1Vec normalSamples;
    posteriorModels[0]->sampleMarginalLikelihood(6, normalSamples);
    TDouble1Vec logNormalSamples;
    posteriorModels[1]->sampleMarginalLikelihood(4, logNormalSamples);
