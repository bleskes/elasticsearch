...
void CChecksumTest::testContainers(void)
{
    ...
    {
        TSizeAnEnumMap::value_type values[] =
            {
                TSizeAnEnumMap::value_type(-1, E_2),
...
void CChecksumTest::testAccumulators(void)
{
    ...
    // Test accumulators.
    {
        ...
        value.add(378.0);
        value.add(653.0);
        LOG_DEBUG("checksum expected = " << core::CHashing::hashCombine(seed, maths::CBasicStatistics::checksum(value)));
        LOG_DEBUG("checksum actual   = " << maths::CChecksum::calculate(seed, value));
        CPPUNIT_ASSERT_EQUAL(core::CHashing::hashCombine(seed, maths::CBasicStatistics::checksum(value)),
