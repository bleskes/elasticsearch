...
// Note: calling this class CSeasonalComponent causes Solaris Studio
// 12.3 to get confused between unnamed-namespace::CSeasonalComponent
// and maths::CSeasonalComponent in subsequent code, hence it is named
...
class CTestSeasonalComponent : public maths::CSeasonalComponent
{
    ...
    public:
        ...
        {
            TTimeTimePrMeanVarPrVec empty;
            return this->maths::CSeasonalComponent::initialize(0, 0, empty);
        }
...
void CSeasonalComponentTest::testConstantPeriodic(void)
{
    ...
    // Test smooth.
    {
        ...
        LOG_DEBUG("totalError1 = " << totalError1);
        LOG_DEBUG("totalError2 = " << totalError2);
        CPPUNIT_ASSERT(totalError1 < 0.7);
        CPPUNIT_ASSERT(totalError2 < 0.26);
    }
    ...
    // Test high slope.
    {
        ...
        for (std::size_t i = 0u, d = 0u; i < n; ++i)
        {
            ...
            if (samples[i].first >= time + core::constants::DAY)
            {
                ...
                LOG_DEBUG("error1 = " << error1);
                LOG_DEBUG("error2 = " << error2);
                CPPUNIT_ASSERT(error1 < 14.0);
                CPPUNIT_ASSERT(error2 < 6.0);
                totalError1 += error1;
                totalError2 += error2;
        ...
        LOG_DEBUG("totalError1 = " << totalError1);
        LOG_DEBUG("totalError2 = " << totalError2);
        CPPUNIT_ASSERT(totalError1 < 6.2);
        CPPUNIT_ASSERT(totalError2 < 3.3);
    }
}
...
void CSeasonalComponentTest::testTimeVaryingPeriodic(void)
{
    ...
    for (std::size_t d = 0u; d < 365; ++d)
    {
        ...
        if (seasonal.initialized())
        {
            ...
            LOG_DEBUG("error1 = " << error1);
            LOG_DEBUG("error2 = " << error2);
            CPPUNIT_ASSERT(error1 < 29.0);
            CPPUNIT_ASSERT(error2 < 18.0);
            totalError1 += error1;
            totalError2 += error2;
            numberErrors += 1.0;
    ...
    CPPUNIT_ASSERT(totalError1 / numberErrors < 20.0);
...
void CSeasonalComponentTest::testVeryLowVariation(void)
{
    ...
    for (std::size_t i = 0u, d = 0u; i < n; ++i)
    {
        ...
        if (samples[i].first >= time + core::constants::DAY)
        {
            ...
            CPPUNIT_ASSERT_DOUBLES_EQUAL(0.0, error1, 0.5 * deviation);
    ...
    CPPUNIT_ASSERT_DOUBLES_EQUAL(totalError1, 0.0, 0.15 * deviation);
...
void CSeasonalComponentTest::testVariance(void)
{
    ...
    {
        seasonal.addPoint(function[i].first,
                          function[i].second);
    }
    ...
    for (core_t::TTime i = 0u; i < 48; ++i)
    {
        ...
        CPPUNIT_ASSERT_DOUBLES_EQUAL(v_, v, 0.5 * v_);
