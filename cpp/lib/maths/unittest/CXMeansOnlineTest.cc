...
void CXMeansOnlineTest::testCluster(void)
{
    ...
    TXMeans2 clusterer(maths_t::E_ContinuousData,
...
void CXMeansOnlineTest::testClusteringVanilla(void)
{
    ...
    for (std::size_t t = 0u; t < 10; ++t)
    {
        ...
        for (std::size_t i = 0u; i < clusters.size(); ++i)
        {
            ...
            CPPUNIT_ASSERT(meanError[0] < 0.021);
            CPPUNIT_ASSERT(covError[0] < 0.18);
...
void CXMeansOnlineTest::testClusteringWithOutliers(void)
{
    ...
    {
        outliers.push_back(TDoubleVec(boost::begin(outliers_[i]),
                                      boost::end(outliers_[i])));
    }
...
void CXMeansOnlineTest::testManyClusters(void)
{
    ...
    // the estimated distribution verses the generating distribution are
...
void CXMeansOnlineTest::testLargeHistory(void)
{
    ...
    TXMeans2ForTest reference(maths_t::E_ContinuousData,
                              maths_t::E_ClustersFractionWeight,
                              0.001, // decay rate
...
void CXMeansOnlineTest::testLatLongData(void)
{
    ...
    CPPUNIT_ASSERT(maths::CTimeSeriesTestData::parse("testfiles/lat_lng.csv",
                                                     timeseries,
                                                     maths::CTimeSeriesTestData::CSV_UNIX_BIVALUED_REGEX));
