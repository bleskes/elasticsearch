...
typedef boost::unordered_map<TVector2, std::size_t, TVector2::CHash> TVector2SizeUMap;
typedef TVector2SizeUMap::const_iterator TVector2SizeUMapCItr;
...
void CBootstrapClustererTest::testThickets(void)
{
    ...
    for (std::size_t t = 0u; t < trials; ++t)
    {
        ...
        for (std::size_t i = 0u; i < v; ++i)
        {
            if (i < k[0])      { expectedClusters[0].push_back(i); }
            else if (i < k[1]) { expectedClusters[1].push_back(i); }
            else               { expectedClusters[2].push_back(i); }
        }
...
void CBootstrapClustererTest::testNonConvexClustering(void)
{
    ...
    for (std::size_t t = 0u; t < 10; ++t)
    {
        ...
        for (std::size_t i = 0u; i < bootstrapClusters.size(); ++i)
        {
            ...
            {
                TVector2SizeUMapCItr k = lookup.find(bootstrapClusters[i][j]);
        ...
        for (std::size_t i = 0u; i < xmeans.clusters().size(); ++i)
        {
            ...
            {
                TVector2SizeUMapCItr k = lookup.find(xmeans.clusters()[i].points()[j]);
                CPPUNIT_ASSERT(k != lookup.end());
