...
//! Converts a (uin32_t, CFloatStorage) pair to a string.
class CToString
{
    public:
        std::string operator()(const TUInt32FloatPr &value) const
        {
...
//! Initializes a (uin32_t, CFloatStorage) pair from a string.
class CFromString
{
    public:
        bool operator()(const std::string &token,
                        TUInt32FloatPr &result) const
        {
            std::size_t delimPos = token.find(PAIR_DELIMITER);
            if (delimPos == std::string::npos)
            {
                return false;
            }
            m_Token.assign(token, 0, delimPos);
            if (core::CStringUtils::stringToType(m_Token, result.first) == false)
            {
                return false;
            }
            m_Token.assign(token,
                           delimPos + 1,
                           token.length() - delimPos - 1);
            if (result.second.fromString(m_Token) == false)
            {
                return false;
            }
            return true;
...
void CCountMinSketch::acceptPersistInserter(core::CStatePersistInserter &inserter) const
{
    ...
    inserter.insertValue(TOTAL_COUNT_TAG,
                         m_TotalCount,
    ...
    if (counts)
    {
        inserter.insertValue(CATEGORY_COUNTS_TAG,
                             core::CPersistUtils::toString(*counts,
                                                           CToString(),
                                                           DELIMITER));
    }
    else
    {
        try
        {
            ...
            inserter.insertLevel(SKETCH_TAG,
                                 boost::bind(&SSketch::acceptPersistInserter,
                                             &sketch,
                                             _1));
        }
        catch (const std::exception &e)
...
CCountMinSketch::SSketch::SSketch(void) :
        s_Hashes(),
        s_Counts()
{
}
...
void CCountMinSketch::SSketch::acceptPersistInserter(core::CStatePersistInserter &inserter) const
{
    ...
    inserter.insertValue(HASHES_TAG,
                         core::CPersistUtils::toString(s_Hashes,
                                                       hashToString,
                                                       DELIMITER));
    for (std::size_t i = 0u; i < s_Counts.size(); ++i)
    {
        inserter.insertValue(COUNTS_TAG,
                             core::CPersistUtils::toString(s_Counts[i],
                                                           CFloatStorage::CToString(),
                                                           DELIMITER));
    }
}
