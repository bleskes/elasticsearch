    ...
    public:
        ...
        bool operator()(double x, TValue &result) const
        {
            try
            {
                ...
                result(0) = ::exp(m_M + 0.5 / x * (1.0 / m_P + 1.0)) * fx;
                result(1) = fx;
            }
            catch (const std::exception &e)
            {
...
class CVarianceKernel
{
    ...
    public:
        ...
        bool operator()(const TValue &x, TValue &result) const
        {
            ...
            {
                boost::math::gamma_distribution<> gamma(m_A, 1.0 / m_B);
                boost::math::normal_distribution<> normal(m_M, ::sqrt(1.0 / x(0) / m_P));
                double fx = boost::math::pdf(normal, x(1)) * boost::math::pdf(gamma, x(0));
                double m  = ::exp(x(1) + 0.5 / x(0));
                result(0) = (m * m * (::exp(1.0 / x(0)) - 1.0) + pow2(m - m_Mean)) * fx;
                result(1) = fx;
            }
            catch (const std::exception &e)
            {
...
class CProbabilityOfLessLikelySamples : core::CNonCopyable
{
    public:
        ...
        bool operator()(double x, double &result) const
        {
            ...
            if (   !evaluateFunctionOnJointDistribution(m_WeightStyles,
                                                        m_Samples,
                                                        m_Weights,
            ...
            try
            {
                    ...
                    || maths_t::hasCountVarianceScale(m_WeightStyles, m_Weights))
                {
                    ...
                    for (std::size_t i = 0u; i < m_Weights.size(); ++i)
                    {
                        ...
                        {
                            m_Scales.push_back(std::make_pair(1.0, 0.0));
                        }
                        else
                        {
                            double t = r + ::log(s + varianceScale * (1.0 - s));
                            m_Scales.push_back(std::make_pair(t / r, 0.5 * (r - t)));
                            logVarianceScaleSum += ::log(t / r);
                        }
                    }
                }
                ...
                m_Constant = 0.5 * (::log(m_Precision) - ::log(impliedPrecision))
                             - 0.5 * m_NumberSamples * LOG_2_PI
                             - 0.5 * logVarianceScaleSum
...
const double CLogMarginalLikelihood::LOG_2_PI = ::log(boost::math::double_constants::two_pi);
    ...
    public:
        ...
        bool operator()(double x, double &result) const
        {
            ...
            for (std::size_t i = 0u; i < m_Samples.size(); ++i)
            {
                ...
                double n = maths_t::countForUpdate(m_WeightStyles, m_Weights[i]);
                residual = ::log(residual + x) - m_Mean;
...
const std::string MINIMUM_TAG("g");
const std::string MAXIMUM_TAG("h");
...
void CLogNormalMeanPrecConjugate::adjustOffset(const TWeightStyleVec &weightStyles,
                                               const TDouble1Vec &samples,
                                               const TDouble4Vec1Vec &weights)
{
    ...
    try
    {
        if (this->isInteger())
        {
            ...
            for (std::size_t i = 0u; i < samples.size(); ++i)
            {
                ...
                numberSamples += n;
                double t = varianceScale == 1.0 ? r : r + ::log(s + varianceScale * (1.0 - s));
            ...
            for (std::size_t i = 0u; i < scaledSamples.size(); ++i)
            {
                ...
                scaledSamples[i] = scale == 1.0 ?
                                   samples[i] + m_Offset :
                                   ::exp(mean + (::log(samples[i] + m_Offset) - mean)
                                               / ::sqrt(scale));
            }
            ...
            detail::CLogSampleSquareDeviation deviationFunction(weightStyles,
                                                                ...
                                                                weights,
                                                                logSamplesMean);
            CIntegration::gaussLegendre<CIntegration::OrderFive>(deviationFunction,
                                                                 0.0, 1.0,
        ...
        else
        {
            ...
            for (std::size_t i = 0u; i < samples.size(); ++i)
            {
                ...
                double scale = r == t ? 1.0 : t / r;
                double shift = (r - t) / 2.0;
                logSamplesMoments.add(::log(x) - shift, n / scale);
            }
...
void CLogNormalMeanPrecConjugate::propagateForwardsByTime(double time)
{
    ...
    m_GaussianPrecision = (alpha * m_GaussianPrecision
                           + (1.0 - alpha) * NON_INFORMATIVE_PRECISION);
    ...
    double factor = std::min((alpha * m_GammaShape
                              + (1.0 - alpha) * NON_INFORMATIVE_SHAPE)
                             / m_GammaShape, 1.0);
...
CLogNormalMeanPrecConjugate::marginalLikelihoodSupport(void) const
{
                                                           ...
                                                           const TDouble4Vec &weights) const
{
    if (this->isNonInformative())
    {
        return ::exp(m_GaussianMean) - m_Offset;
    }
    ...
    return (normalPrecision == 0.0 ?
            0.0 : ::exp(normalMean - 1.0 / normalPrecision))
           - m_Offset;
}
                                                               ...
                                                               const TDouble4Vec &weights) const
{
    ...
    double vh =   ::exp(2.0 * m_GaussianMean + m_GammaRate / m_GammaShape
                                               * (2.0 / m_GaussianPrecision + 1.0))
               * (::exp(m_GammaRate / m_GammaShape) - 1.0);
                                                        ...
                                                        double &result) const
{
    ...
    if (this->isInteger())
    {
        CIntegration::logGaussLegendre<CIntegration::OrderThree>(logMarginalLikelihood,
                                                                 0.0, 1.0,
                                                                 result);
    }
    else
    ...
    maths_t::EFloatingPointErrorStatus status =
            static_cast<maths_t::EFloatingPointErrorStatus>(
                                                           ...
                                                           TDouble1Vec &samples) const
{
    ...
    if (this->isNonInformative())
    {
        ...
        samples.push_back(::exp(m_GaussianMean) - m_Offset);
        return;
    }
    ...
    double scale = ::sqrt((m_GaussianPrecision + 1.0)
                          / m_GaussianPrecision
                          * m_GammaRate / m_GammaShape);
    try
    {
        ...
        for (std::size_t i = 1u; i < numberSamples; ++i)
        {
            ...
            double xq = ::log(boost::math::quantile(lognormal, q));
                                                   ...
                                                   double &upperBound) const
{
    ...
    if (this->isInteger())
    {
        ...
        // on the interval [0,1].
        double value;
        if (!CIntegration::logGaussLegendre<CIntegration::OrderThree>(minusLogCdf,
                                                                      0.0, 1.0,
                                                                      value))
        {
                                                             ...
                                                             double &upperBound) const
{
    ...
    if (this->isInteger())
    {
        ...
        // on the interval [0,1].
        double value;
        if (!CIntegration::logGaussLegendre<CIntegration::OrderThree>(minusLogCdfComplement,
                                                                      0.0, 1.0,
                                                                      value))
        {
...
std::string CLogNormalMeanPrecConjugate::printJointDensityFunction(void) const
{
    ...
    boost::math::normal_distribution<> gaussian(m_GaussianMean, 1.0 / ::sqrt(precision));
...
CLogNormalMeanPrecConjugate::confidenceIntervalNormalMean(double percentage) const
{
    ...
    boost::math::gamma_distribution<> gamma(m_GammaShape, 1.0 / m_GammaRate);
    double precision = m_GaussianPrecision * this->normalPrecision();
    xLower = m_GaussianMean + xLower / ::sqrt(precision);
    xUpper = m_GaussianMean + xUpper / ::sqrt(precision);
...
double CLogNormalMeanPrecConjugate::mean(void) const
{
    ...
    return ::exp(m_GaussianMean + 0.5 * m_GammaRate / m_GammaShape
...
const double CLogNormalMeanPrecConjugate::NON_INFORMATIVE_SHAPE = 0.5;
