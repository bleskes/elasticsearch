    ...
    public:
                                                          ...
                                                          bool ignoreSingularSubspace)
        {
            ...
            default:
            {
                ...
                Eigen::JacobiSVD<EIGENMATRIX> covariance(covariance_.toEigen(),
                                                         Eigen::ComputeFullU | Eigen::ComputeFullV);
                EIGENVECTOR y(residual.toEigen());
    ...
    public:
                             ...
                             std::vector<VECTOR_PRECISE> &result)
        {
            ...
            else
            {
                ...
                for (std::size_t i = 0u; i < rank; ++i)
                {
                    VECTOR_PRECISE u(covariance.matrixU().col(i));
                    try
                    {
    ...
    public:
                                                          ...
                                                          bool ignoreSingularSubspace)
        {
            ...
            default:
            {
                // Note we use Jacobi SVD here so that we handle the case
                // that m is singular to working precision.
                Eigen::JacobiSVD<EIGENMATRIX> svd(m_.toEigen());
...
maths_t::EFloatingPointErrorStatus inverseQuadraticProduct(std::size_t d,                               \
                                                           ...
                                                           bool ignoreSingularSubspace)                 \
{                                                                                                       \
    return CInverseQuadraticProduct<SEigenMatrixNxN<N>::Type,                                           \
                                    SEigenVectorNx1<N>::Type>::compute(                                 \
...
maths_t::EFloatingPointErrorStatus inverseQuadraticProduct(std::size_t d,
                                                           ...
                                                           bool ignoreSingularSubspace)
{
    return CInverseQuadraticProduct<SEigenMatrix::Type,
                                    SEigenVector::Type>::compute(
                                            d, covariance, residual, result, ignoreSingularSubspace);
}
maths_t::EFloatingPointErrorStatus inverseQuadraticProduct(std::size_t d,
...
maths_t::EFloatingPointErrorStatus gaussianLogLikelihood(std::size_t d,                               \
                                                         ...
                                                         bool ignoreSingularSubspace)                 \
{                                                                                                     \
    return CGaussianLogLikelihood<SEigenMatrixNxN<N>::Type,                                           \
                                  SEigenVectorNx1<N>::Type>::compute(                                 \
                                          d, covariance, residual, result, ignoreSingularSubspace);   \
}
...
maths_t::EFloatingPointErrorStatus gaussianLogLikelihood(std::size_t d,
                                                         ...
                                                         bool ignoreSingularSubspace)
{
    return CGaussianLogLikelihood<SEigenMatrix::Type,
                                  SEigenVector::Type>::compute(
                                          d, covariance, residual, result, ignoreSingularSubspace);
}
maths_t::EFloatingPointErrorStatus gaussianLogLikelihood(std::size_t d,
...
{                                                                                     \
    CSampleGaussian<SEigenMatrixNxN<N>::Type>::generate(d, mean, covariance, result); \
}
...
{
    return CSampleGaussian<SEigenMatrix::Type>::generate(d, mean, covariance, result);
}
void sampleGaussian(std::size_t d,
...
maths_t::EFloatingPointErrorStatus logDeterminant(std::size_t d,                                          \
                                                  ...
                                                  bool ignoreSingularSubspace)                            \
{                                                                                                         \
    return CLogDeterminant<SEigenMatrixNxN<N>::Type>::compute(d, matrix, result, ignoreSingularSubspace); \
}
...
maths_t::EFloatingPointErrorStatus logDeterminant(std::size_t d,
                                                  ...
                                                  bool ignoreSingularSubspace)
{
    return CLogDeterminant<SEigenMatrix::Type>::compute(d, matrix, result, ignoreSingularSubspace);
}
maths_t::EFloatingPointErrorStatus logDeterminant(std::size_t d,
