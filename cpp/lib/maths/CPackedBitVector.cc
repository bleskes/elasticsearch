...
double CPackedBitVector::inner(const CPackedBitVector &covector, EOperation op) const
{
         ...
         /**/)
    {
        ...
        switch (op)
        {
        case E_AND: result += static_cast<double>((value & covalue) * run); break;
        case E_OR:  result += static_cast<double>((value | covalue) * run); break;
        case E_XOR: result += static_cast<double>((value ^ covalue) * run); break;
        }
