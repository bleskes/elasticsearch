...
void CPrior::swap(CPrior &other)
{
    ...
    std::swap(m_Minimum, other.m_Minimum);
    std::swap(m_Maximum, other.m_Maximum);
}
                        ...
                        const TDouble4Vec1Vec &weights)
{
    ...
    try
    {
        ...
        {
            n += maths_t::countForUpdate(weightStyles, weights[i]);
            m_Minimum.add(samples[i]);
            m_Maximum.add(samples[i]);
        }
    }
...
double CPrior::smallest(void) const
{
    return m_Minimum.count() > 0 ? m_Minimum[0] : boost::numeric::bounds<double>::lowest();
}

double CPrior::largest(void) const
{
    return m_Maximum.count() > 0 ? m_Maximum[0] : boost::numeric::bounds<double>::highest();
}
...
void CPrior::adjustOffsetWithCost(const TWeightStyleVec &weightStyles,
                                    const TDouble1Vec &samples,
                                    const TDouble4Vec1Vec &weights,
                                    COffsetCost &cost,
                                    CApplyOffset &apply)
{
    ...
    if (increment > 0.0)
    {
        ...
        for (std::size_t i = 0u; i < ADJUST_OFFSET_TRIALS; ++i, offset += increment)
        {
...
const CPrior::TMinAccumulator &CPrior::minimum(void) const
{
    return m_Minimum;
}

CPrior::TMinAccumulator &CPrior::minimum(void)
{
    return m_Minimum;
}

const CPrior::TMaxAccumulator &CPrior::maximum(void) const
{
    return m_Maximum;
}

CPrior::TMaxAccumulator &CPrior::maximum(void)
{
    return m_Maximum;
}
...
CPrior::COffsetParameters::~COffsetParameters(void) {}
