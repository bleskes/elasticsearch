...
inline TDoubleBoolPr stationaryPoint(const boost::math::beta_distribution<> &beta)
{
    if (beta.alpha() < 1.0 && beta.beta() < 1.0)
    {
        ...
        return std::make_pair( (beta.alpha() - 1.0)
                             / (beta.alpha() + beta.beta() - 2.0), false);
    }
    return std::make_pair(boost::math::mode(beta), true);
}
...
{
    return -::log(IMPROPER_CDF);
}

double CTools::SMinusLogCdf::operator()(
        const boost::math::normal_distribution<> &normal, double x) const
{
    return safeMinusLogCdf(safeCdf(normal, x));
}

double CTools::SMinusLogCdf::operator()(
        const boost::math::students_t_distribution<> &students, double x) const
{
    return safeMinusLogCdf(safeCdf(students, x));
}

double CTools::SMinusLogCdf::operator()(
        const boost::math::negative_binomial_distribution<> &negativeBinomial, double x) const
{
    return safeMinusLogCdf(safeCdf(negativeBinomial, x));
}

double CTools::SMinusLogCdf::operator()(
        const boost::math::lognormal_distribution<> &logNormal, double x) const
{
...
double CTools::SMinusLogCdf::operator()(
        const boost::math::gamma_distribution<> &gamma, double x) const
{
    return safeMinusLogCdf(safeCdf(gamma, x));
}

double CTools::SMinusLogCdf::operator()(
        const boost::math::beta_distribution<> &beta, double x) const
{
    return safeMinusLogCdf(safeCdf(beta, x));
}
...
{
    return -::log(1.0 - IMPROPER_CDF);
}

double CTools::SMinusLogCdfComplement::operator()(
        const boost::math::normal_distribution<> &normal, double x) const
{
    return safeMinusLogCdf(safeCdfComplement(normal, x));
}
...
double CTools::SMinusLogCdfComplement::operator()(
        const boost::math::gamma_distribution<> &gamma, double x) const
{
    return safeMinusLogCdf(safeCdfComplement(gamma, x));
}

double CTools::SMinusLogCdfComplement::operator()(
        const boost::math::beta_distribution<> &beta, double x) const
{
    return safeMinusLogCdf(safeCdfComplement(beta, x));
}
...
double CTools::CProbabilityOfLessLikelySample::operator()(
        const SImproperDistribution &, double, maths_t::ETail &tail) const
{
...
double CTools::CProbabilityOfLessLikelySample::operator()(
        const boost::math::normal_distribution<> &normal, double x, maths_t::ETail &tail) const
{
    double px = 0.0;

    TDoubleDoublePr support = boost::math::support(normal);
    ...
    case maths_t::E_OneSidedBelow:
        px = truncate(2.0 * safeCdf(normal, x), 0.0, 1.0);
    ...
    case maths_t::E_TwoSided:
    {
        ...
        double m = boost::math::mode(normal);
        if (x < m)
        {
            px = truncate(2.0 * safeCdf(normal, x), 0.0, 1.0);
        }
        ...
        {
            px = truncate(2.0 * safeCdfComplement(normal, x), 0.0, 1.0);
        }
        this->tail(x, m, tail);
        break;
    }
...
double CTools::CProbabilityOfLessLikelySample::operator()(
        const boost::math::students_t_distribution<> &students,
        double x,
        ...
        maths_t::ETail &tail) const
{
    ...
    case maths_t::E_OneSidedAbove:
        ...
        return truncate(2.0 * (  safeCdfComplement(negativeBinomial, x)
                               + safePdf(negativeBinomial, x)), 0.0, 1.0);
    }
    ...
    static const unsigned int MAX_ITERATIONS = 20u;
    ...
    if (x > m)
    {
        ...
        for (;;)
        {
            ...
            step = (maxIterations == MAX_ITERATIONS / 2 ?
                    b1 : (1.0 / shrinkFactor - 1.0) * b1);
        }
    }
    else
    {
        ...
        double logOneMinusP = ::log(1 - p);

        b1 = ::floor(m + ::log(std::max(fx, MIN_DOUBLE)
        ...
        for (;;)
        {
            ...
            // bracket point.
            double lowerBound = b2 + ::log(  std::max(fx, MIN_DOUBLE)
                                           / std::max(f2, MIN_DOUBLE))
                                     / logOneMinusP;
            LOG_TRACE("b2 = " << b2
                      << ", f2 = "
                      << f2 << ", bound = " << lowerBound);
    ...
    double y = boost::numeric::bounds<double>::highest();
    ...
    catch (const std::exception &e)
    {
        ...
        {
            LOG_ERROR("Failed in root finding: " << e.what()
                      << ", x = " << x
                      << ", bracket = (" << b1 << "," << b2 << ")"
                      << ", f(bracket) = (" << f1 - fx << "," << f2 - fx << ")");
    ...
    double py = x < m ?
                safeCdfComplement(negativeBinomial, y) :
...
double CTools::CProbabilityOfLessLikelySample::operator()(
        const boost::math::lognormal_distribution<> &logNormal, double x, maths_t::ETail &tail) const
{
    double px = 0.0;
...
double CTools::CProbabilityOfLessLikelySample::operator()(
        const CLogTDistribution &logt, double x, maths_t::ETail &tail) const
{
    double px = 0.0;
    ...
    double scale = ::sqrt(v) * s;
    double bound = 0.0;
    double fBound = boost::numeric::bounds<double>::highest();
    if (fl < fx)
    {
        bound = ::exp(l);
        fBound = fl;
    ...
    {
        double t1 = l + ::log(fl / fx);
        double t2 = (l - scale) / 8.0 + ::log(scale / 3.0);
...
double CTools::CProbabilityOfLessLikelySample::operator()(
        const boost::math::gamma_distribution<> &gamma, double x, maths_t::ETail &tail) const
{
    double px = 0.0;

    TDoubleDoublePr support = boost::math::support(gamma);
    if (!this->check(support, x, px, tail))
    {
        return px;
    }
    ...
    case maths_t::E_OneSidedBelow:
        tail = static_cast<maths_t::ETail>(tail | maths_t::E_LeftTail);
        return truncate(2.0 * safeCdf(gamma, x), 0.0, 1.0);
    ...
    case maths_t::E_OneSidedAbove:
        tail = static_cast<maths_t::ETail>(tail | maths_t::E_RightTail);
        return truncate(2.0 * safeCdfComplement(gamma, x), 0.0, 1.0);
    }
    ...
    {
        tail = static_cast<maths_t::ETail>(tail | maths_t::E_RightTail);
        return truncate(safeCdfComplement(gamma, x), 0.0, 1.0);
    }

    static const double MIN_DOUBLE = std::numeric_limits<double>::min();
    static const double EPSILON = std::numeric_limits<double>::epsilon();
    static const double CONVERGENCE_TOLERANCE = 1e-4;
    static const double PDF_TOLERANCE = 5e-2;
    static const unsigned int MAX_ITERATIONS = 20u;

    double m = boost::math::mode(gamma);
    ...
    double fx = safePdf(gamma, x);
    double fy = safePdf(gamma, y[i % 2]);
    ...
    if (::fabs(fx - fy) <= PDF_TOLERANCE * std::max(fx, fy))
    {
        ...
        return truncate(safeCdf(gamma, x) + safeCdfComplement(gamma, y[i % 2]), 0.0, 1.0);
    }
    ...
    {
        b = (1.0 + gamma.scale() / (a - m) * ::log(fa / fx)) * a;
        fb = safePdf(gamma, b);
        std::swap(a, b);
        std::swap(fa, fb);
    }
    ...
    px = x > m ? safeCdfComplement(gamma, x) : safeCdf(gamma, x);
    ...
    try
    {
        ...
        double candidate;
        CSolvers::solve(a, b, fa - fx, fb - fx, makePdf(gamma, fx), maxIterations, equal, candidate);
        LOG_TRACE("bracket = (" << a << "," << b << ")"
                  << ", iterations = " << maxIterations
                  << ", f(candidate) = " << safePdf(gamma, candidate) - fx);

        if (::fabs(safePdf(gamma, candidate) - fx) < ::fabs(fy - fx))
        {
            y[i % 2] = candidate;
        }
    }
    ...
    LOG_TRACE("f(x) = " << fx << ", f(y) = " << safePdf(gamma, y[i % 2]));

    double py = x > y[i % 2] ? safeCdf(gamma, y[i % 2]) : safeCdfComplement(gamma, y[i % 2]);

    return truncate(px + py, 0.0, 1.0);
}

double CTools::CProbabilityOfLessLikelySample::operator()(
        const boost::math::beta_distribution<> &beta, double x, maths_t::ETail &tail) const
{
    double px = 0.0;
    ...
    case maths_t::E_OneSidedBelow:
        tail = static_cast<maths_t::ETail>(tail | maths_t::E_LeftTail);
        return truncate(2.0 * safeCdf(beta, x), 0.0, 1.0);
    ...
    case maths_t::E_OneSidedAbove:
        tail = static_cast<maths_t::ETail>(tail | maths_t::E_RightTail);
        return truncate(2.0 * safeCdfComplement(beta, x), 0.0, 1.0);
    }
    ...
    else if (beta.alpha() == 1.0 && beta.beta() == 1.0)
    {
    ...
    else if (beta.alpha() <= 1.0 && beta.beta() >= 1.0)
    {
        ...
        tail = static_cast<maths_t::ETail>(tail | maths_t::E_RightTail);
        return truncate(safeCdfComplement(beta, x), 0.0, 1.0);
    }
    else if (beta.alpha() >= 1.0 && beta.beta() <= 1.0)
    {
        // The distribution is monotone increasing.
        tail = static_cast<maths_t::ETail>(tail | maths_t::E_LeftTail);
        return truncate(safeCdf(beta, x), 0.0, 1.0);
    }
    else
    {
    ...
    static const double CONVERGENCE_TOLERANCE = 1e-4;
    static const double PDF_TOLERANCE = 5e-2;
    static const unsigned int MAX_ITERATIONS = 6u;

    TDoubleBoolPr sp = stationaryPoint(beta);
    ...
    if (x < sp.first)
    {
        ...
        double k = (beta.alpha() - 1.0) / (beta.beta() - 1.0);
        for (;;)
        {
            y[(i + 1) % 2] = 1.0 - ::exp(k * ::log(x / y[i % 2])) * (1.0 - x);
            if (++i == MAX_ITERATIONS
        ...
        if (y[i % 2] > support.second)
        {
            return truncate(sp.second ?
                            safeCdf(beta, x) :
                            safeCdfComplement(beta, x), 0.0, 1.0);
        }
    ...
    else
    {
        ...
        double k = (beta.beta() - 1.0) / (beta.alpha() - 1.0);
        for (;;)
        {
            y[(i + 1) % 2] = ::exp(k * ::log((1.0 - x) / (1.0 -  y[i % 2]))) * x;
            if (++i == MAX_ITERATIONS
                || ::fabs(y[1] - y[0]) < CONVERGENCE_TOLERANCE)
            {
                break;
        ...
        {
            return truncate(sp.second ?
                            safeCdfComplement(beta, x) :
                            safeCdf(beta, x), 0.0, 1.0);
        }

        y[i % 2] = std::min(y[i % 2], sp.first);
    }

    double fx = safePdf(beta, x);
    double fy = safePdf(beta, y[i % 2]);
    LOG_TRACE("f(x) = " << fx << ", f(y) = " << fy);
    ...
    try
    {
        ...
        if (::fabs(error) <= PDF_TOLERANCE * std::max(fx, fy))
        {
            ...
            return truncate(sp.second ?
                            safeCdf(beta, x) + safeCdfComplement(beta, y[i % 2]) :
                            safeCdf(beta, y[i % 2]) - safeCdf(beta, x), 0.0, 1.0);
        }
        else if (error > 0.0)
        {
            if (x < sp.first)
            {
                ...
                double fa = fy - fx;
                double fb = safePdf(beta, bracket.second) - fx;
                fBracket = std::make_pair(fa, fb);
            }
            else
            {
                bracket = std::make_pair(bracket.first, y[i % 2]);
                double fa = safePdf(beta, bracket.first) - fx;
                double fb = fy - fx;
                fBracket = std::make_pair(fa, fb);
            }
        }
        else
        {
            ...
            double fa = safePdf(beta, sp.first) - fx;
            double fb = fy - fx;
            fBracket = std::make_pair(fa, fb);
        }
    }
    ...
    try
    {
        ...
        LOG_TRACE("bracket = " << core::CContainerPrinter::print(bracket)
                  << ", iterations = " << maxIterations
                  << ", f(candidate) = " << safePdf(beta, candidate) - fx
                  << ", eps = " << eps);

        if (::fabs(safePdf(beta, candidate) - fx) < ::fabs(fy - fx))
        {
            y[i % 2] = candidate;
        }
    }
...
//! Transform the \p x to account for variance scaling.
double transform(double x, double mean, double scale)
{
    return scale != 1.0 ? mean + scale * (x - mean) : x;
}
                                                            ...
                                                            double scale)
{
    ...
    {
        deviation = 0.0;
        double a  = transform(modeMean - 2.0 * modeSd, mean, scale);
        double b  = transform(modeMean + 2.0 * modeSd, mean, scale);
        m_Endpoints.push_back(truncate(a, m_A, m_B));
        m_Endpoints.push_back(truncate(b, m_A, m_B));
    }
    else if (deviation >= -0.5)
    {
        deviation = ::sqrt(-2.0 * deviation);
        double a  = transform(modeMean - (deviation + 2.0) * modeSd, mean, scale);
        double m  = transform(modeMean, mean, scale);
        double b  = transform(modeMean + (deviation + 2.0) * modeSd, mean, scale);
        m_Endpoints.push_back(truncate(a, m_A, m_B));
        m_Endpoints.push_back(truncate(m, m_A, m_B));
        m_Endpoints.push_back(truncate(b, m_A, m_B));
    }
    else
    {
        deviation = ::sqrt(-2.0 * deviation);
        double aa = transform(modeMean - (deviation + 2.0) * modeSd, mean, scale);
        double ab = transform(modeMean - (deviation - 1.0) * modeSd, mean, scale);
        double ba = transform(modeMean + (deviation - 1.0) * modeSd, mean, scale);
        double bb = transform(modeMean + (deviation + 2.0) * modeSd, mean, scale);
        m_Endpoints.push_back(truncate(aa, m_A, m_B));
        m_Endpoints.push_back(truncate(ab, m_A, m_B));
        m_Endpoints.push_back(truncate(ba, m_A, m_B));
        m_Endpoints.push_back(truncate(bb, m_A, m_B));
    }
...
void CTools::CMixtureProbabilityOfLessLikelySample::intervals(TDoubleDoublePrVec &intervals)
{
    ...
    {
        intervals.push_back(std::make_pair(m_Endpoints[i - 1], m_Endpoints[i]));
    }
...
const double CTools::CMixtureProbabilityOfLessLikelySample::LOG_ROOT_TWO_PI =
                                 0.5 * ::log(boost::math::double_constants::two_pi);
...
double CTools::SIntervalExpectation::operator()(const boost::math::normal_distribution<> &normal,
                                                double a,
                                                double b) const
{
    if (a > b)
    ...
    double sd = normal.standard_deviation();
    double s  = ::sqrt(2.0) * sd;
    ...
    if (erfb - erfa < ::sqrt(std::numeric_limits<double>::epsilon()))
    {
        return expa == expb ?
               (a + b) / 2.0 :
               (a * expa + b * expb) / (expa + expb);
    }
...
double CTools::SIntervalExpectation::operator()(const boost::math::lognormal_distribution<> &logNormal,
                                                double a,
                                                double b) const
{
    if (a > b)
    ...
    double loga = a <= 0.0     ? NEG_INF : ::log(a);
    double logb = b == POS_INF ? POS_INF : ::log(b);
    double c  = location + scale * scale;
    double s  = ::sqrt(2.0) * scale;
...
double CTools::SIntervalExpectation::operator()(const boost::math::gamma_distribution<> &gamma,
                                                double a,
                                                double b) const
{
    if (a > b)
    ...
    double shape = gamma.shape();
    double rate = 1.0 / gamma.scale();
    ...
    if (gamb - gama < ::sqrt(std::numeric_limits<double>::epsilon()))
    {
        double expa = a <= 0.0     ? 0.0 : ::exp((shape - 1.0) * ::log(a) - rate * a);
        double expb = b == POS_INF ? 0.0 : ::exp((shape - 1.0) * ::log(b) - rate * b);
...
typedef policy<overflow_error<user_error> > AllowOverflow;
}
...
double CTools::safePdf(const boost::math::normal_distribution<> &normal, double x)
{
    return continuousSafePdf(allowOverflow(normal), x);
}

double CTools::safePdf(const boost::math::students_t_distribution<> &students, double x)
{
    return continuousSafePdf(allowOverflow(students), x);
}

double CTools::safePdf(const boost::math::poisson_distribution<> &poisson, double x)
{
    return discreteSafePdf(allowOverflow(poisson), x);
}

double CTools::safePdf(const boost::math::negative_binomial_distribution<> &negativeBinomial, double x)
{
    return discreteSafePdf(allowOverflow(negativeBinomial), x);
}

double CTools::safePdf(const boost::math::lognormal_distribution<> &logNormal, double x)
{
    return continuousSafePdf(allowOverflow(logNormal), x);
}

double CTools::safePdf(const boost::math::gamma_distribution<> &gamma, double x)
{
    TDoubleDoublePr support = boost::math::support(gamma);
    ...
    if (x == support.first)
    {
        ...
        else if (gamma.shape() == 1.0)
        {
            return 1.0 / gamma.scale();
        }
        return 0.0;
    }

    return continuousSafePdf(allowOverflow(gamma), x);
}

double CTools::safePdf(const boost::math::beta_distribution<> &beta, double x)
{
    TDoubleDoublePr support = boost::math::support(beta);
    ...
    if (x == support.first)
    {
        ...
        {
            return 1.0 / boost::math::beta(beta.alpha(), beta.beta());
        }
        else
        {
            return 0.0;
    ...
    return continuousSafePdf(allowOverflow(beta), x);
}

double CTools::safePdf(const boost::math::binomial_distribution<> &binomial, double x)
{
    return discreteSafePdf(allowOverflow(binomial), x);
}

double CTools::safePdf(const boost::math::chi_squared_distribution<> &chi2, double x)
{
...
double CTools::safeCdf(const boost::math::normal_distribution<> &normal, double x)
{
    return continuousSafeCdf(allowOverflow(normal), x);
}

double CTools::safeCdf(const boost::math::students_t_distribution<> &students, double x)
{
    return continuousSafeCdf(allowOverflow(students), x);
}

double CTools::safeCdf(const boost::math::poisson_distribution<> &poisson, double x)
{
    return discreteSafeCdf(allowOverflow(poisson), x);
}

double CTools::safeCdf(const boost::math::negative_binomial_distribution<> &negativeBinomial, double x)
{
    return discreteSafeCdf(allowOverflow(negativeBinomial), x);
}

double CTools::safeCdf(const boost::math::lognormal_distribution<> &logNormal, double x)
{
    return continuousSafeCdf(allowOverflow(logNormal), x);
}

double CTools::safeCdf(const boost::math::gamma_distribution<> &gamma, double x)
{
    return continuousSafeCdf(allowOverflow(gamma), x);
}

double CTools::safeCdf(const boost::math::beta_distribution<> &beta, double x)
{
    return continuousSafeCdf(allowOverflow(beta), x);
}

double CTools::safeCdf(const boost::math::binomial_distribution<> &binomial, double x)
{
    return discreteSafeCdf(allowOverflow(binomial), x);
}

double CTools::safeCdf(const boost::math::chi_squared_distribution<> &chi2, double x)
{
...
double CTools::safeCdfComplement(const boost::math::normal_distribution<> &normal, double x)
{
    return continuousSafeCdfComplement(allowOverflow(normal), x);
}

double CTools::safeCdfComplement(const boost::math::students_t_distribution<> &students, double x)
{
    return continuousSafeCdfComplement(allowOverflow(students), x);
}

double CTools::safeCdfComplement(const boost::math::poisson_distribution<> &poisson, double x)
{
    return discreteSafeCdfComplement(allowOverflow(poisson), x);
}

double CTools::safeCdfComplement(const boost::math::negative_binomial_distribution<> &negativeBinomial, double x)
{
    return discreteSafeCdfComplement(allowOverflow(negativeBinomial), x);
}

double CTools::safeCdfComplement(const boost::math::lognormal_distribution<> &logNormal, double x)
{
    return continuousSafeCdfComplement(allowOverflow(logNormal), x);
}

double CTools::safeCdfComplement(const boost::math::gamma_distribution<> &gamma, double x)
{
    return continuousSafeCdfComplement(allowOverflow(gamma), x);
}

double CTools::safeCdfComplement(const boost::math::beta_distribution<> &beta, double x)
{
    return continuousSafeCdfComplement(allowOverflow(beta), x);
}

double CTools::safeCdfComplement(const boost::math::binomial_distribution<> &binomial, double x)
{
    return discreteSafeCdfComplement(allowOverflow(binomial), x);
}

double CTools::safeCdfComplement(const boost::math::chi_squared_distribution<> &chi2, double x)
{
...
{
    static const double INV_LARGEST_SIGNIFICANT_PROBABILITY = 1.0 / LARGEST_SIGNIFICANT_PROBABILITY;
    static const double INV_SMALL_PROBABILITY = 1.0 / SMALL_PROBABILITY;
    static const double MINUS_LOG_SMALL_PROBABILITY = -::log(SMALL_PROBABILITY);
    static const double MINUS_LOG_MINUSCULE_PROBABILITY = -::log(MINUSCULE_PROBABILITY);
    static const double MINUS_LOG_SMALLEST_PROBABILITY = -::log(smallestProbability());
        ...
        else if (adjP >= MINUSCULE_PROBABILITY)
        {
            ...
            result = SMALL_PROBABILITY_DEVIATION
                     +   (MINUSCULE_PROBABILITY_DEVIATION - SMALL_PROBABILITY_DEVIATION)
                       * (-::log(adjP) - MINUS_LOG_SMALL_PROBABILITY)
        ...
        else
        {
            ...
            result = MINUSCULE_PROBABILITY_DEVIATION
                     +   (MAX_DEVIATION - MINUSCULE_PROBABILITY_DEVIATION)
                       * (-::log(adjP) - MINUS_LOG_MINUSCULE_PROBABILITY)
...
double CTools::inverseDeviation(double deviation)
{
    ...
    {
        LOG_ERROR("Probability " << result
                  << " out of range, deviation =" << deviation);
    }
...
double CTools::differentialEntropy(const boost::math::poisson_distribution<> &poisson)
{
    // Approximate as sum over [mean - 5 * std, mean + 5 * std].

    double mean = boost::math::mean(poisson);
    double deviation = boost::math::standard_deviation(poisson);
...
double CTools::differentialEntropy(const boost::math::normal_distribution<> &normal)
{
    ...
    double variance = boost::math::variance(normal);
    return 0.5 * ::log(boost::math::double_constants::two_pi
                       * boost::math::double_constants::e
                       * variance);
}

double CTools::differentialEntropy(const boost::math::lognormal_distribution<> &logNormal)
{
...
double CTools::differentialEntropy(const boost::math::gamma_distribution<> &gamma)
{
    ...
    double shape = gamma.shape();
    double scale = gamma.scale();

    return shape + ::log(scale)
...
void CTools::CGroup::merge(const CGroup &other,
                           double separation,
                           double min,
                           double max)
{
    ...
    LOG_TRACE("merged = [" << this->leftEndpoint(separation)
              << ", " << this->rightEndpoint(separation) << "]");
}

bool CTools::CGroup::overlap(const CGroup &other,
                             double separation) const
{
    static const double TOL = 1.0 + std::numeric_limits<double>::epsilon();
    ...
    bool result = !(   TOL * (lr + separation) <= rl
                    || ll >= TOL * (rr + separation)
                    || TOL * (rr + separation) <= ll
                    || rl >= TOL * (lr + separation));
    LOG_TRACE("left = [" << ll << ", " << lr << "]"
              << ", right = [" << rl << ", " << rr << "] "
              << (result ? "overlap" : "don't overlap"));
    return result;
}
...
{
    if (x == boost::numeric::bounds<double>::lowest())
    {
        return x;
    }
...
{
    if (x == boost::numeric::bounds<double>::highest())
    {
        return x;
    }
    return (x < 0.0 ? 1.0 - eps : 1.0 + eps) * x;
