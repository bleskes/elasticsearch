...
struct SInfluencerSumSerializer
{
    ...
    typedef boost::unordered_map<TStrPtr, double> TStrPtrDoubleUMap;
    typedef TStrPtrDoubleUMap::const_iterator TStrPtrDoubleUMapCItr;
    ...
    void operator()(const TStrPtrDoubleUMap &map, core::CStatePersistInserter &inserter) const
    {
        TStrCRefDoublePrVec ordered;
        ordered.reserve(map.size());
        for (TStrPtrDoubleUMapCItr i = map.begin(); i != map.end(); ++i)
        {
            ordered.push_back(TStrCRefDoublePr(TStrCRef(*i->first), i->second));
        }
    ...
    bool operator()(TStrPtrDoubleUMap &map, core::CStateRestoreTraverser &traverser) const
    {
        std::string key;
        do
        {
...
CGathererTools::CSumGatherer::CSumGatherer(const SModelParams &params,
        ...
        m_InfluencerBucketSums(std::distance(beginInfluencers, endInfluencers),
                               TStrPtrDoubleUMapQueue(params.s_LatencyBuckets + 3,
                                                   bucketLength,
                                                   startTime,
                                                   TStrPtrDoubleUMap(1)))
{
                                                     ...
                                                     const TSampleVec &emptySample) const
{
    ...
    {
        const TStrPtrDoubleUMap &influencerStats = m_InfluencerBucketSums[i].get(time);
        influenceValues[i].reserve(influencerStats.size());
        for (TStrPtrDoubleUMapCItr itr = influencerStats.begin();
             itr != influencerStats.end();
             ++itr)
        {
            influenceValues[i].push_back(
                    TStrCRefDouble1VecDoublePrPr(TStrCRef(*itr->first),
                                                 TDouble1VecDoublePr(TDouble1Vec(1, itr->second), 1.0)));
    ...
    if (!sum->empty())
    {
        return SMetricFeatureData((*sum)[0].time(),
                                  (*sum)[0].value(),
                                  (*sum)[0].varianceScale(),
                                  (*sum)[0].count(),
                                  influenceValues,
    ...
    return SMetricFeatureData(m_Classifier.isInteger(), *sum);
}
...
void CGathererTools::CSumGatherer::startNewBucket(core_t::TTime time)
{
    ...
    for (std::size_t i = 0u; i < m_InfluencerBucketSums.size(); ++i)
    {
        m_InfluencerBucketSums[i].push(TStrPtrDoubleUMap(), time);
    }
}
...
void CGathererTools::CSumGatherer::acceptPersistInserter(core::CStatePersistInserter &inserter) const
{
    ...
    for (std::size_t i = 0u; i < m_InfluencerBucketSums.size(); ++i)
    {
        inserter.insertLevel(INFLUENCER_BUCKET_SUM_QUEUE_TAG,
                             boost::bind<void>(TStrPtrDoubleUMapQueue::CSerializer<SInfluencerSumSerializer>(),
        ...
        else if (name == INFLUENCER_BUCKET_SUM_QUEUE_TAG)
        {
            if (   i < m_InfluencerBucketSums.size()
                && traverser.traverseSubLevel(
                       boost::bind<bool>(TStrPtrDoubleUMapQueue::CSerializer<SInfluencerSumSerializer>(TStrPtrDoubleUMap(1)),
