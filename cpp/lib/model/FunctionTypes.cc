...
//! Expand a function to all functions that its features map to.
TFunctionVec expandFunction(EFunction function)
{
    const TFeatureVec &features = function_t::features(function);
    TFunctionVec expanded(features.size());
    for (std::size_t i = 0u; i < features.size(); ++i)
    {
        TFeatureVec featureAsVector(1, features[i]);
        expanded[i] = function_t::function(featureAsVector);
    }
    return expanded;
}
...
TFunctionVec statisticFunctions(EFunction function)
{
    // Note that these have to return valid Splunk stats command aggregation
    // statistics. Things like our high and low and non-zero simply map to
    // the corresponding aggregate statistic. For example, low_non_null_sum
    // maps to sum, since the value of this function can be computed from
    // the sum statistic.
    ...
    switch (function)
    {
        ...
        case E_IndividualMeanVelocity:
        case E_IndividualSumVelocity:
            return expandFunction(function);
        case E_IndividualMetricLowMean:
        case E_IndividualMetricHighMean:
        case E_IndividualLatLong:
            return expandFunction(E_IndividualMetricMean);
        case E_IndividualMetricLowVariance:
        case E_IndividualMetricHighVariance:
            return expandFunction(E_IndividualMetricVariance);
        case E_IndividualMetricLowSum:
        case E_IndividualMetricHighSum:
            return expandFunction(E_IndividualMetricSum);
        case E_IndividualMetricNonNullSum:
        case E_IndividualMetricLowNonNullSum:
        case E_IndividualMetricHighNonNullSum:
            return expandFunction(E_IndividualMetricSum);
        ...
        case E_PopulationMetricHighMean:
        case E_PopulationLatLong:
            return expandFunction(E_PopulationMetricMean);
        case E_PopulationMetricLowVariance:
        case E_PopulationMetricHighVariance:
            return expandFunction(E_PopulationMetricVariance);
        case E_PopulationMetricLowSum:
        case E_PopulationMetricHighSum:
            return expandFunction(E_PopulationMetricSum);
