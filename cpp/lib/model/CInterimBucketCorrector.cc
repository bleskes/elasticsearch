...
{
    return  CModelConfig::DEFAULT_DECAY_RATE
          * CModelConfig::bucketNormalizationFactor(bucketLength);
}

double trendDecayRate(core_t::TTime bucketLength)
{
    return CModelConfig::trendDecayRate(meanDecayRate(bucketLength), bucketLength);
}
}
...
void CInterimBucketCorrector::update(core_t::TTime time, std::size_t bucketCount)
{
    ...
    m_CountTrend.propagateForwardsTo(bucketMidPoint);
                                                           ...
                                                           std::size_t currentCount) const
{
    ...
    {
        baselineCount = maths::CBasicStatistics::mean(m_CountTrend.baseline(bucketMidPoint, 0.0));
    }
    else
    {
