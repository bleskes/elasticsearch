...
CDataGatherer::CDataGatherer(model_t::EAnalysisCategory gathererType,
        ...
        m_UseNull(useNull)
{
    // Constructor needs to create 1 bucketgatherer at the startTime, and possibly
    // 1 bucketgatherer at (startTime + bucketLength / 2)
...
const CDataGatherer::TStrPtr &CDataGatherer::personNamePtr(std::size_t pid) const
{
    return m_PeopleRegistry.namePtr(pid);
}

const std::string &CDataGatherer::personName(std::size_t pid,
                                             const std::string &fallback) const
{
    return m_PeopleRegistry.name(pid, fallback);
}

void CDataGatherer::personNonZeroCounts(core_t::TTime time,
...
std::size_t CDataGatherer::addPerson(const std::string &person,
                                     ...
                                     bool &addedPerson)
{
    return m_PeopleRegistry.addName(person,
                                    this->chooseBucketGatherer(0).currentBucketStartTime(),
                                    resourceMonitor,
                                    addedPerson);
}
...
const CDataGatherer::TStrPtr &CDataGatherer::attributeNamePtr(std::size_t cid) const
{
...
bool CDataGatherer::validateSampleTimes(core_t::TTime &startTime,
...
const CDataGatherer::TSizeSizePrStrPtrPrUInt64UMapVec &
...
void CDataGatherer::acceptPersistInserter(core::CStatePersistInserter &inserter) const
{
    ...
    inserter.insertLevel(BUCKET_GATHERER_TAG,
                         boost::bind(&CDataGatherer::persistBucketGatherers,
                                     this,
                                     _1));
}
...
bool CDataGatherer::determineMetricCategory(TMetricCategoryVec &fieldMetricCategories) const
{
    ...
    {
        LOG_ERROR("Unable to map feature "
                  << model_t::print(m_Features.front())
    ...
    do
    {
        ...
        bool convertedOk = (first == 0 && last == std::string::npos) ?
                core::CStringUtils::stringToType(fieldValue,
                                                 value) :
                core::CStringUtils::stringToType(fieldValue.substr(first, last - first),
                                                 value);
        if (!convertedOk)
        {
