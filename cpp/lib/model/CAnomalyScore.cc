...
std::size_t addProbabilities(const TDoubleVec &probabilities,
...
// This is the version to assume for XML that doesn't contain a version
// attribute - NEVER CHANGE THIS
const std::string MISSING_VERSION_XML_VERSION("1");
...
// Increment this every time a change to the XML format is made that requires
// existing state to be discarded
const std::string CAnomalyScore::CURRENT_XML_VERSION("3");
                            ...
                            double &overallProbability)
{
    ...
    {
        double interpolate =
                std::min(  (logPJoint - LOG_SMALLEST_PROBABILITY)
                         / (SMALLEST_LOG_JOINT_PROBABILTY - LOG_SMALLEST_PROBABILITY), 1.0);
        overallAnomalyScore =  (   NORMAL_RANGE_SCORE_FRACTION
                                + (1.0 - NORMAL_RANGE_SCORE_FRACTION) * interpolate)
                              * jointProbabilityWeight
    ...
    else
    {
        overallAnomalyScore =   NORMAL_RANGE_SCORE_FRACTION
                              * jointProbabilityWeight
                              * probabilityToScore(::exp(logPJoint));
    }

    if (logPExtreme < LOG_SMALLEST_PROBABILITY)
    {
        double interpolate =
                std::min(  (logPExtreme - LOG_SMALLEST_PROBABILITY)
                         / (SMALLEST_LOG_EXTREME_PROBABILTY - LOG_SMALLEST_PROBABILITY), 1.0);
        overallAnomalyScore +=  (         NORMAL_RANGE_SCORE_FRACTION
                                 + (1.0 - NORMAL_RANGE_SCORE_FRACTION) * interpolate)
    ...
    else
    {
        overallAnomalyScore +=   NORMAL_RANGE_SCORE_FRACTION
                               * extremeProbabilityWeight
                               * probabilityToScore(::exp(logPExtreme));
    }

    // Invert the deviation in the region it is 1-to-1 otherwise
    // use the weighted harmonic mean.
    overallProbability = overallAnomalyScore > 0.0 ?
                         scoreToProbability(std::min(  overallAnomalyScore
                                                     / NORMAL_RANGE_SCORE_FRACTION,
                                                     SMALLEST_PROBABILITY_DEVIATION)) :
                           ::exp(jointProbabilityWeight * logPJoint)
                         * ::exp(extremeProbabilityWeight * logPExtreme);

    LOG_TRACE("logJointProbability = " << logPJoint
              << ", jointProbabilityWeight = " << jointProbabilityWeight
              << ", logExtremeProbability = " << logPExtreme
...
CAnomalyScore::CNormalizer::CNormalizer(const CModelConfig &config) :
...
bool CAnomalyScore::CNormalizer::normalize(TDoubleVec &scores) const
{
    ...
    if (this->normalize(normalizedScore) == false)
    {
...
bool CAnomalyScore::CNormalizer::normalize(double &score) const
{
    ...
    // normalized score range. Finally to ensure 3) we use a we
    ...
    TDoubleDoublePrVecCItr knotPoint =
            std::lower_bound(m_NormalizedScoreKnotPoints.begin(),
                             m_NormalizedScoreKnotPoints.end(),
                             // make_pair avoids Solaris Studio 12.4 compiler
                             // bug whereas direct pair construction triggers it
                             std::make_pair(m_NoisePercentile, 0.0));
    double signalStrength = m_NoiseMultiplier
                            * 10.0 / DISCRETIZATION_FACTOR
    ...
    normalizedScores[0] = knotPoint->second * std::max(1.0 + signalStrength, 0.0)
                          + m_MaximumNormalizedScore
                            * std::max(2.0 * std::min(50.0 * (l0 + u0)
                                                      / m_NoisePercentile, 1.0) - 1.0, 0.0);
    LOG_TRACE("normalizedScores[0] = " << normalizedScores[0]
              << ", knotPoint = " << knotPoint->second
    ...
    std::size_t lowerKnotPoint =
            std::max(std::lower_bound(m_NormalizedScoreKnotPoints.begin(),
                     ...
                     - m_NormalizedScoreKnotPoints.begin(), ptrdiff_t(1));
    ...
    if (lowerKnotPoint < m_NormalizedScoreKnotPoints.size())
    {
        ...
        normalizedScores[1] = left.second
    ...
    if (upperKnotPoint < m_NormalizedScoreKnotPoints.size())
    {
        ...
        // Linearly interpolate between the two knot points.
        normalizedScores[1] = (  normalizedScores[1]
                               + left.second
                               +   (right.second - left.second)
                                 * (upperPercentile - left.first)
                                 / (right.first - left.first)) / 2.0;
    }
    else
    {
        normalizedScores[1] = (normalizedScores[1] + m_MaximumNormalizedScore ) / 2.0;
    }
    LOG_TRACE("normalizedScores[1] = " << normalizedScores[1]
    ...
    static const double M =  (  probabilityToScore(maths::SMALL_PROBABILITY)
                           ...
                           / (  ::log(maths::SMALL_PROBABILITY)
                              - ::log(maths::LARGEST_SIGNIFICANT_PROBABILITY));
    static const double C = ::log(maths::LARGEST_SIGNIFICANT_PROBABILITY);
    normalizedScores[3] =  m_MaximumNormalizedScore
                         * (0.95 * M * (::log(scoreToProbability(score)) - C) + 0.05);
    LOG_TRACE("normalizedScores[3] = " << normalizedScores[3]
              << ", score = " << score
                                          ...
                                          double &upperBound) const
{
    ...
    lowerBound = fl + (1.0 - fl) * std::max(lowerBound
                                            - cutoffCdfUpperBound
                                            - pdfUpperBound, 0.0)
                                 / std::max(1.0 - cutoffCdfUpperBound,
                                            std::numeric_limits<double>::epsilon());
    upperBound = fu + (1.0 - fu) * std::max(upperBound
                                            - cutoffCdfLowerBound
                                            - pdfLowerBound, 0.0)
...
bool CAnomalyScore::CNormalizer::updateQuantiles(double score)
{
    ...
    if ((n + 1) == k)
    {
        ...
        {
            uint64_t highPercentileCount =
                static_cast<uint64_t>((HIGH_PERCENTILE / 100.0)
            ...
            LOG_TRACE("s(H) = " << m_HighPercentileScore
                      << ", c(H) = " << m_HighPercentileCount
                      << ", percentile = " << 100.0 * static_cast<double>(m_HighPercentileCount)
            ...
            for (/**/; i < L.size(); ++i)
            {
                uint64_t x = L[i].first;
                uint64_t m = L[i].second - L[i - 1].second;

                LOG_TRACE("Adding (" << x << ", " << m << ") to H");
                m_RawScoreHighQuantileSummary.add(L[i].first, m);
            }
        }
    }
    ...
    LOG_TRACE("percentile = " << static_cast<double>(m_HighPercentileCount)
    ...
    if ((n + 1) > k && (n + 1) % k == 0)
    {
        ...
        {
            m_RawScoreQuantileSummary.quantile(HIGH_PERCENTILE / 100.0,
                                               m_HighPercentileScore);
            double lowerBound, upperBound;
            m_RawScoreQuantileSummary.cdf(m_HighPercentileScore,
                                          0.0,
                                          lowerBound,
                                          upperBound);
            m_HighPercentileCount = static_cast<uint64_t>(static_cast<double>(n + 1)
                                                          * lowerBound + 0.5);
...
void CAnomalyScore::CNormalizer::propagateForwardByTime(double time)
{
    ...
    {
        time = ::floor( (QUANTILE_DECAY_TIME - m_TimeToQuantileDecay)
                       / QUANTILE_DECAY_TIME);
        ...
        if (n > 0)
        {
            m_HighPercentileCount = static_cast<uint64_t>(
                                      ...
                                      / static_cast<double>(n)
                                      * static_cast<double>(m_HighPercentileCount) + 0.5);
        }
        m_TimeToQuantileDecay +=   QUANTILE_DECAY_TIME
                                + ::floor(-m_TimeToQuantileDecay / QUANTILE_DECAY_TIME);
    }
}
                                         ...
                                         const std::string &currentVersion)
{
    ...
    LOG_INFO("Upgrading quantiles from version " << loadedVersion <<
             ...
             " - will scale highest score by " << highScoreUpgradeFactor <<
...
void CAnomalyScore::CNormalizer::acceptPersistInserter(core::CStatePersistInserter &inserter) const
{
    ...
    m_MaxScore.persist(MAX_SCORE_TAG, inserter);
...
bool CAnomalyScore::CNormalizer::acceptRestoreTraverser(core::CStateRestoreTraverser &traverser)
{
    do
    {
        ...
        if (name == HIGH_PERCENTILE_SCORE_TAG)
        {
            ...
            m_HighPercentileScore = static_cast<uint32_t>(std::min(highPercentileScore64,
        ...
        else if (name == MAX_SCORE_TAG)
        {
            if (traverser.hasSubLevel())
            {
                if (traverser.traverseSubLevel(boost::bind(&CNormalizer::restoreMaxScore,
                                                           this,
                                                           _1)) == false)
                {
                    LOG_ERROR("Invalid max score in " << traverser.value());
                    return false;
                }
            }
            else if (m_MaxScore.restore(traverser) == false)
            {
                LOG_ERROR("Invalid max score in " << traverser.value());
                return false;
            }
        ...
        {
            if (traverser.traverseSubLevel(boost::bind(&maths::CQDigest::acceptRestoreTraverser,
                                                       &m_RawScoreQuantileSummary,
                                                       _1)) == false)
            {
...
bool CAnomalyScore::CNormalizer::restoreMaxScore(core::CStateRestoreTraverser &traverser)
{
    do
    {
        if (m_MaxScore.restore(traverser) == false)
        {
            return false;
        }
    }
...
// We obfuscate most XML element names to avoid giving away too much information
// about our model.  However, top level attribute names are not obfuscated, as
// we want Splunk to be able to index them.
namespace
{
...
bool CAnomalyScore::normalizerFromJson(const std::string &json,
                                       ...
                                       CNormalizer &normalizer)
{
    ...
    std::string restoredVersion(MISSING_VERSION_XML_VERSION);
    ...
    while (traverser.next())
    {
        ...
        if (name == PRELERTVERSION_ATTRIBUTE)
        {
            ...
            {
                if (normalizer.isUpgradable(restoredVersion, CURRENT_XML_VERSION))
                {
                    LOG_DEBUG("Restored quantiles JSON version is " << restoredVersion <<
                              "; current JSON version is " << CURRENT_XML_VERSION <<
        ...
        else if (name == NORMALIZER_TAG)
        {
            restoredNormalizer = traverser.traverseSubLevel(boost::bind(
                                    &CAnomalyScore::CNormalizer::acceptRestoreTraverser,
                                                        &normalizer,
                                                        _1));
    ...
    if (restoredNormalizer &&
        restoredVersion != CURRENT_XML_VERSION)
    {
        LOG_INFO("Restored quantiles JSON version is " << restoredVersion <<
                 "; current JSON version is " << CURRENT_XML_VERSION <<
                 " - will attempt upgrade");

        if (normalizer.upgrade(restoredVersion, CURRENT_XML_VERSION) == false)
        {
            LOG_ERROR("Failed to upgrade quantiles from version " << restoredVersion <<
                      " to version " << CURRENT_XML_VERSION);
            return false;
        }
    }
                                     ...
                                     std::string &json)
{
    ...
    // The JSON inserter will only close the object when it is destroyed
    {
        ...
        inserter.insertLevel(NORMALIZER_TAG,
                             boost::bind(&CNormalizer::acceptPersistInserter,
                                         &normalizer,
                                         _1));
        std::string key(cue + searchKey);
        inserter.insertValue(KEY, key);
    }
