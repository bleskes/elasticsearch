...
//! \brief Creates new nomralizer instances.
class CNormalizerFactory
{
    public:
        typedef CHierarchicalResultsNormalizer::TNormalizer TNormalizer;

        CNormalizerFactory(const CModelConfig &modelConfig) : m_ModelConfig(modelConfig) {}
...
CHierarchicalResultsNormalizer::CHierarchicalResultsNormalizer(const CModelConfig &modelConfig) :
                                           ...
                                           bool pivot)
{
    ...
    case E_Normalize:
        ...
        {
            LOG_ERROR("Failed to normalize " << node.s_RawAnomalyScore
                      << " for " << node.s_Spec.print());
        }
    ...
    CHierarchicalResultsNormalizer::fromJsonStream(std::istream &inputStream)
{
    ...
    do
    {
        ...
        else
        {
            ...
            if (normalizerVec != 0)
            {
                ...
                normalizerVec->push_back(TWordNormalizerPr(TWord(hashArray),
    ...
    LOG_INFO(this->influencerBucketSet().size() << " influencer bucket normalizers, " <<
