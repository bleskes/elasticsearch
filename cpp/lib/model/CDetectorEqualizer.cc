...
void CDetectorEqualizer::acceptPersistInserter(core::CStatePersistInserter &inserter) const
{
    ...
    for (std::size_t i = 0u; i < m_Sketches.size(); ++i)
    {
        inserter.insertValue(DETECTOR_TAG, m_Sketches[i].first);
        inserter.insertLevel(SKETCH_TAG, boost::bind(&maths::CQuantileSketch::acceptPersistInserter,
                                                     boost::cref(m_Sketches[i].second), _1));
    }
}
...
bool CDetectorEqualizer::acceptRestoreTraverser(core::CStateRestoreTraverser &traverser)
{
    ...
    do
    {
        ...
        if (name == SKETCH_TAG)
        {
            ...
            m_Sketches.push_back(TIntQuantileSketchPr(
                    *detector, maths::CQuantileSketch(SKETCH_INTERPOLATION, SKETCH_SIZE)));
            if (traverser.traverseSubLevel(boost::bind(&maths::CQuantileSketch::acceptRestoreTraverser,
...
double CDetectorEqualizer::correct(int detector, double probability)
{
    ...
    for (std::size_t i = 0u; i < m_Sketches.size(); ++i)
    {
        if (m_Sketches[i].second.count() < MINIMUM_COUNT_FOR_CORRECTION)
        {
            return probability;
        }
    }
    ...
    if (sketch.cdf(logp, percentage))
    {
        ...
        {
            double logpi;
            if (m_Sketches[i].second.quantile(percentage, logpi))
            {
...
void CDetectorEqualizer::age(double factor)
{
    for (std::size_t i = 0u; i < m_Sketches.size(); ++i)
    {
        m_Sketches[i].second.age(factor);
    }
}
...
{
    TIntQuantileSketchPrVecItr i = std::lower_bound(m_Sketches.begin(), m_Sketches.end(),
                                                    detector, maths::COrderings::SFirstLess());
    if (i == m_Sketches.end() || i->first != detector)
    {
        i = m_Sketches.insert(i, TIntQuantileSketchPr(
                detector, maths::CQuantileSketch(SKETCH_INTERPOLATION, SKETCH_SIZE)));
    }
    return i->second;
}
