...
unsigned int CSampleCounts::count(std::size_t id) const
{
    return m_SampleCountOverride > 0 ?
           m_SampleCountOverride : m_SampleCounts[id];
}
...
void CSampleCounts::recycle(const TSizeVec &idsToRemove)
{
    ...
    {
        std::size_t cid = idsToRemove[i];
        if (cid >= m_SampleCounts.size())
        {
            continue;
        }
        m_SampleCounts[cid] = 0;
        m_MeanNonZeroBucketCounts[cid] = TMeanAccumulator();
        m_EffectiveSampleVariances[cid] = TMeanAccumulator();
    }
...
const std::string &
CSampleCounts::name(const CDataGatherer &gatherer,
                                         std::size_t id) const
{
