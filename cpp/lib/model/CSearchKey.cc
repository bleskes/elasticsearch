...
CSearchKey::CSearchKey(core::CStateRestoreTraverser &traverser,
                       bool &successful)
    : m_Identifier(0),
      ...
      m_Hash(0)
{
    successful = traverser.traverseSubLevel(boost::bind(&CSearchKey::acceptRestoreTraverser,
                                                        this,
                                                        _1));
}
...
void CSearchKey::acceptPersistInserter(core::CStatePersistInserter &inserter) const
{
    ...
    {
        inserter.insertValue(INFLUENCE_FIELD_NAME_TAG,
                             *m_InfluenceFieldNames[i]);
    }
}
...
bool CSearchKey::isSimpleCount(function_t::EFunction function,
                                     const std::string &byFieldName)
{
    return function == function_t::E_IndividualCount &&
...
std::string CSearchKey::toCue(void) const
{
    ...
    cue.reserve(64 + // hopefully covers function description and slashes
                m_FieldName->length() +
                m_ByFieldName->length() +
                m_OverFieldName->length() +
...
const CSearchKey::TStrPtrVec &CSearchKey::influenceFieldNames(void) const
{
