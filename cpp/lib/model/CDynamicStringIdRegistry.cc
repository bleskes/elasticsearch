...
const CDynamicStringIdRegistry::TStrPtr &CDynamicStringIdRegistry::namePtr(std::size_t id) const
{
    return m_Names[id];
}
                                              ...
                                              bool &addedPerson)
{
    ...
    std::size_t newId = m_FreeUids.empty() ?
                         m_Names.size() :
                         m_FreeUids.back();
...
bool CDynamicStringIdRegistry::checkInvariants(void) const
{
    ...
    {
        LOG_ERROR("Unexpected extra "
                  << (m_Uids.size() - m_Names.size()) << " " << m_NameType << " uids");
        result = false;
    }
...
uint64_t CDynamicStringIdRegistry::checksum(void) const
{
    ...
    for (std::size_t pid = 0u; pid < m_Names.size(); ++pid)
    {
        ...
        {
            people.push_back(TStrCRef(*m_Names[pid]));
        }
    }
...
bool CDynamicStringIdRegistry::acceptRestoreTraverser(core::CStateRestoreTraverser &traverser)
{
    ...
    // reuse. We mustn't add these to the ID maps. See bug 1127 in Bugzilla
    // for more details.
