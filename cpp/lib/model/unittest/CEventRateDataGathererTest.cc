...
typedef CBucketGatherer::TSizeSizePrStrPtrPr TSizeSizePrStrPtrPr;
...
void addArrival(CDataGatherer &gatherer, core_t::TTime time, const std::string &person, const std::string &attribute)
{
    CDataGatherer::TStrCPtrVec fieldValues;
    fieldValues.push_back(&person);
...
void testPersistence(const SModelParams &params,
                              ...
                              const std::string &valueField)
{
    ...
    CDataGatherer gatherer(model_t::E_EventRate, model_t::E_None, params,
                           "", "", "", "", valueField, influencerFieldNames,
                           false, key, features,
                           startTime, 0);
    CPPUNIT_ASSERT(!gatherer.isPopulation());
    CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(gatherer, "p", valueField, 1));
    ...
    CPPUNIT_ASSERT_EQUAL(static_cast<core_t::TTime>(200),
    ...
    for (std::size_t i = 0, j = 0u; i < data.size(); ++i)
    {
                ...
                time += bucketLength, ++j, gatherer.timeNow(time))
        {
            ...
            CPPUNIT_ASSERT_EQUAL(expected[j],
        ...
        if (j < 5)
        {
            addArrival(gatherer, data[i], "p", influencers[i], valueField.empty() ? EMPTY_STRING : "value");
        }
    }
}


void importCsvData(CDataGatherer &gatherer, const std::string &filename, const TSizeVec &fields)
{
    ...
    while (std::getline(*ifs, line))
    {
        ...
        gatherer.addArrival(fieldValues, data, monitor);
    }
    ifs.reset();
}
...
void CEventRateDataGathererTest::testLatencyPersist(void)
{
...

    {
        ...
        importCsvData(gatherer, "testfiles/files_users_programs.csv", fields);
...
void CEventRateDataGathererTest::singleSeriesTests(void)
{
    ...
    // (which should be ignored).
    {
        ...
        CDataGatherer gatherer(model_t::E_EventRate, model_t::E_None, params,
                               ...
                               startTime, 0);
        CPPUNIT_ASSERT(!gatherer.isPopulation());
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(gatherer, "p"));
        ...
        for (std::size_t i = 0, j = 0u; i < boost::size(data); ++i)
        {
                 ...
                 time += bucketLength, ++j, gatherer.timeNow(time))
            {
                ...
                CPPUNIT_ASSERT_EQUAL(expectedPersonCounts[j],
                                     core::CContainerPrinter::print(featureData[0].second));
    ...
    // Test non-zero count and person bucket count.
    {
        ...
        for (std::size_t i = 0, j = 0u; i < boost::size(data); ++i)
        {
                 ...
                 time += bucketLength, ++j, gatherer.timeNow(time))
            {
                ...
                CPPUNIT_ASSERT_EQUAL(expectedPersonNonZeroCounts[j],
                                     core::CContainerPrinter::print(featureData[0].second));
    ...
    // Test test person indicator by bucket.
    {
        ...
        for (std::size_t i = 0, j = 0u; i < boost::size(data); ++i)
        {
                 ...
                 time += bucketLength, ++j, gatherer.timeNow(time))
            {
                ...
                CPPUNIT_ASSERT_EQUAL(expectedPersonIndicator[j],
                                     core::CContainerPrinter::print(featureData[0].second));
...
void CEventRateDataGathererTest::multipleSeriesTests(void)
{
...

    {
        ...
        CDataGatherer gatherer(model_t::E_EventRate, model_t::E_None, params,
                               ...
                               false, key, features,
                               startTime, 0);
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(gatherer, "p1"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), addPerson(gatherer, "p2"));
        ...
        {
            for (/**/;
                 j < 5 && std::min(data1[i1], data2[i2]) >= time + bucketLength;
                 time += bucketLength, ++j)
            {
                LOG_DEBUG("Processing bucket [" << time << ", " << time + bucketLength << ")");
            ...
            if (data1[i1] < data2[i2])
            {
                ...
                addArrival(gatherer, data1[i1], "p1");
                ++i1;
            }
            else
            {
                LOG_DEBUG("Adding arrival for p2 at " << data2[i2]);
                addArrival(gatherer, data2[i2], "p2");
                ++i2;
            }
        }
...

    {
        ...
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(gatherer, "p1"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), addPerson(gatherer, "p2"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(2), addPerson(gatherer, "p3"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(3), addPerson(gatherer, "p4"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(4), addPerson(gatherer, "p5"));

        for (std::size_t i = 0u; i < 5; ++i)
        {
            addArrival(gatherer, startTime, gatherer.personName(i));
        }
        addArrival(gatherer, startTime + 1, gatherer.personName(2));
        addArrival(gatherer, startTime + 2, gatherer.personName(4));
        addArrival(gatherer, startTime + 3, gatherer.personName(4));
        ...
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), featureData.size());
        CPPUNIT_ASSERT_EQUAL(model_t::E_IndividualCountByBucketAndPerson, featureData[0].first);
        CPPUNIT_ASSERT_EQUAL(std::string("[(2, 2), (4, 3)]"),
                             core::CContainerPrinter::print(featureData[0].second));
    }
}
...
void CEventRateDataGathererTest::testRemovePeople(void)
{
    ...
    CPPUNIT_ASSERT_EQUAL(std::size_t(3), addPerson(gatherer, "p4"));
    CPPUNIT_ASSERT_EQUAL(std::size_t(4), addPerson(gatherer, "p5"));
    CPPUNIT_ASSERT_EQUAL(std::size_t(5), addPerson(gatherer, "p6"));
    CPPUNIT_ASSERT_EQUAL(std::size_t(6), addPerson(gatherer, "p7"));
    CPPUNIT_ASSERT_EQUAL(std::size_t(7), addPerson(gatherer, "p8"));
    ...
    for (std::size_t i = 0u; i < boost::size(counts); ++i)
    {
        ...
        {
            addArrival(gatherer, startTime + time, gatherer.personName(i));
        }
    }

    {
        ...
        CDataGatherer expectedGatherer(model_t::E_EventRate, model_t::E_None, params,
                                       "", "", "", "", "", TStrVec(),
                                       false, key, features,
                                       startTime, 0);
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(expectedGatherer, "p3"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), addPerson(expectedGatherer, "p4"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(2), addPerson(expectedGatherer, "p5"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(3), addPerson(expectedGatherer, "p6"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(4), addPerson(expectedGatherer, "p7"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(5), addPerson(expectedGatherer, "p8"));
        ...
        for (std::size_t i = 0u; i < boost::size(expectedCounts); ++i)
        {
            ...
            {
                addArrival(expectedGatherer, startTime + time, expectedGatherer.personName(i));
            }
        }
    ...
    }
    {
        ...
        CDataGatherer expectedGatherer(model_t::E_EventRate, model_t::E_None, params,
                                       ...
                                       startTime, 0);
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(expectedGatherer, "p3"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), addPerson(expectedGatherer, "p6"));
        CPPUNIT_ASSERT_EQUAL(std::size_t(2), addPerson(expectedGatherer, "p7"));
    ...
    expectedRecycled.push_back(addPerson(gatherer, "p1"));
    expectedRecycled.push_back(addPerson(gatherer, "p7"));
...
void CEventRateDataGathererTest::singleSeriesOutOfOrderFinalResultTests(void)
{
    ...
    core_t::TTime data[] =
        {
            1, 180, 1200, 190, 400, 600, // bucket 1, 2 & 3
            550, 799, 1199, 15,          // bucket 1 & 2
            2490,                        // bucket 5
                                         // bucket 4 is empty
            2420, 2480, 1250,            // bucket 3 & 5
            10000                        // sentinel
        };
    ...
    // (which should be ignored).
    {
        ...
        CDataGatherer gatherer(model_t::E_EventRate, model_t::E_None, params,
                               ...
                               false, key, features,
                               startTime, 0);
        addPerson(gatherer, "p");

        core_t::TTime time = startTime;
        for (std::size_t i = 0, j = 0u; i < boost::size(data); ++i)
        {
            for (/**/;
                 j < 5 && data[i] >= time + latencyTime;
                 time += bucketLength, ++j, gatherer.timeNow(time))
            {
                LOG_DEBUG("Processing bucket [" << time << ", " << time + bucketLength << ")");
...
void CEventRateDataGathererTest::singleSeriesOutOfOrderInterimResultTests(void)
{
    ...
    core_t::TTime data[] =
        {
            1, 1200, 600, // bucket 1, 3 & 2
            1199, 15,     // bucket 2 & 1
            2490,         // bucket 5
                          // bucket 4 is empty
            2420, 1250   // bucket 5 & 3
        };
    ...
    // Bucket 1 only
    addArrival(gatherer, data[0], "p");

    gatherer.featureData(0, bucketLength, featureData);
    CPPUNIT_ASSERT_EQUAL(std::string("[(0, 1)]"),
                         core::CContainerPrinter::print(featureData[0].second));

    // Bucket 1, 2 & 3
    addArrival(gatherer, data[1], "p");
    ...
    // Bucket 1, 2 & 3
    addArrival(gatherer, data[3], "p");

    gatherer.featureData(0, bucketLength, featureData);
    CPPUNIT_ASSERT_EQUAL(std::string("[(0, 1)]"),
                         core::CContainerPrinter::print(featureData[0].second));
    gatherer.featureData(600, bucketLength, featureData);
    CPPUNIT_ASSERT_EQUAL(std::string("[(0, 2)]"),
                         core::CContainerPrinter::print(featureData[0].second));
    gatherer.featureData(1200, bucketLength, featureData);
    CPPUNIT_ASSERT_EQUAL(std::string("[(0, 1)]"),
                         core::CContainerPrinter::print(featureData[0].second));
...
void CEventRateDataGathererTest::multipleSeriesOutOfOrderFinalResultTests(void)
{
    ...
    core_t::TTime data1[] =
        {
            1, 15, 1200, 190, 400, 550, // bucket 1, 2 & 3
            600, 1250, 1199,            // bucket 2 & 3
            180, 799,                   // bucket 1 & 2
            2480,                       // bucket 5
            2420,  1900,  2490,         // bucket 4 & 5
            10000                       // sentinel
        };
    core_t::TTime data2[] =
        {
...

    {
        ...
        {
            for (/**/;
                 j < 5 && std::min(data1[i1], data2[i2]) >= time + latencyTime;
                 time += bucketLength, ++j)
            {
                LOG_DEBUG("Processing bucket [" << time << ", " << time + bucketLength << ")");
...
void CEventRateDataGathererTest::testResetBucketGivenMultipleSeries(void)
{
    ...
    CDataGatherer gatherer(model_t::E_EventRate, model_t::E_None, params,
                           ...
                           false, key, features,
                           startTime, 0);
    addPerson(gatherer, "p1");
    addPerson(gatherer, "p2");
    addPerson(gatherer, "p3");

    for (std::size_t i = 0; i < boost::size(data); ++i)
    {
        addArrival(gatherer, data[i], "p1");
        addArrival(gatherer, data[i], "p2");
        addArrival(gatherer, data[i], "p3");
    }

    TFeatureSizeFeatureDataPrVecPrVec featureData;

    gatherer.featureData(0, bucketLength, featureData);
    CPPUNIT_ASSERT_EQUAL(std::string("[(0, 2), (1, 2), (2, 2)]"),
                         core::CContainerPrinter::print(featureData[0].second));

    gatherer.featureData(600, bucketLength, featureData);
    CPPUNIT_ASSERT_EQUAL(std::string("[(0, 3), (1, 3), (2, 3)]"),
                         core::CContainerPrinter::print(featureData[0].second));

    gatherer.featureData(1200, bucketLength, featureData);
    CPPUNIT_ASSERT_EQUAL(std::string("[(0, 1), (1, 1), (2, 1)]"),
                         core::CContainerPrinter::print(featureData[0].second));
    ...
    gatherer.featureData(600, bucketLength, featureData);
    CPPUNIT_ASSERT_EQUAL(std::string("[(0, 0), (1, 0), (2, 0)]"),
                         core::CContainerPrinter::print(featureData[0].second));
...
void CEventRateDataGathererTest::testInfluencerBucketStatistics(void)
{
    ...
    testInfluencerPerFeature(model_t::E_IndividualCountByBucketAndPerson,
                             dataVec, influencers, expectedPersonCountsVec, "");
    ...
    testInfluencerPerFeature(model_t::E_IndividualLowCountsByBucketAndPerson,
                             dataVec, influencers, expectedPersonCountsVec, "");

    testInfluencerPerFeature(model_t::E_IndividualArrivalTimesByPerson,
                             dataVec, influencers, expectedArrivalTimeVec, "");
    ...
    testInfluencerPerFeature(model_t::E_IndividualUniqueCountByBucketAndPerson,
                             dataVec, influencers, expectedPersonIndicatorVec, "value");

    testInfluencerPerFeature(model_t::E_IndividualInfoContentByBucketAndPerson,
                             dataVec, influencers, expectedInfoContentVec, "value");
}
...
void CEventRateDataGathererTest::testDistinctStrings(void)
{
    ...
    }
    {
        ...
        influencers.back() = boost::make_shared<std::string>("inf1");
        data.insert("str1", influencers);
        {
            SEventRateFeatureData featureData(0);
            data.populateDistinctCountFeatureData(featureData);
        ...
        data.insert("str2", influencers);
        data.insert("str2", influencers);
        influencers.back() = boost::make_shared<std::string>("inf2");
        data.insert("str1", influencers);
        data.insert("str3", influencers);
        influencers.back() = boost::make_shared<std::string>("inf3");
        data.insert("str3", influencers);
        {
            SEventRateFeatureData featureData(0);
            data.populateDistinctCountFeatureData(featureData);
...

    {
       // Check we can add more than one influencer
        ...
        influencers[0] = boost::make_shared<std::string>("inf1");
        data.insert("str1", influencers);
        data.insert("str2", influencers);
        {
            SEventRateFeatureData featureData(0);
        ...
        influencers[1] = boost::make_shared<std::string>("inf_v2");

        data.insert("str2", influencers);
        influencers[0] = boost::make_shared<std::string>("inf2");
        influencers[1] = boost::make_shared<std::string>("inf_v3");
        data.insert("str3", influencers);
        data.insert("str1", influencers);
        data.insert("str3", influencers);
        {
    ...
    }
    {
        ...
        CPPUNIT_ASSERT(!gatherer.isPopulation());
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(gatherer, "p", "v", 1));
        ...
        addArrival(gatherer, time - (2 * bucketLength), "p", "stringOne", "inf1");
        addArrival(gatherer, time - (2 * bucketLength), "p", "stringTwo", "inf2");
        addArrival(gatherer, time - (1 * bucketLength), "p", "stringThree", "inf3");
        addArrival(gatherer, time - (1 * bucketLength), "p", "stringFour", "inf1");
        addArrival(gatherer, time, "p", "stringFive", "inf2");
        addArrival(gatherer, time, "p", "stringSix", "inf3");
        testPersistence(params, gatherer);
    }
}
...
void CEventRateDataGathererTest::testDiurnalFeatures(void)
{
    ...
    const std::string emptyString("");
    {
        ...
        {
            addArrival(gatherer, time + 0, person);
        ...
        {
            addArrival(gatherer, time + 100, person);
        ...
        {
            addArrival(gatherer, time + 200, person);
        ...
        {
            addArrival(gatherer, time + 300, person);
        ...
        time += bucketLength;
        {
            addArrival(gatherer, time + 400, person);
    ...
    }
    {
        ...
        // Add some data, and check that we get the right numbers out of the featureData
        {
            addArrival(gatherer, time + 0, person, attribute);
        ...
        {
            addArrival(gatherer, time + 100, person, attribute);
        ...
        {
            addArrival(gatherer, time + 200, person, attribute);
        ...
        {
            addArrival(gatherer, time + 300, person, attribute);
        ...
        time += bucketLength;
        {
            addArrival(gatherer, time + 400, person, attribute);
