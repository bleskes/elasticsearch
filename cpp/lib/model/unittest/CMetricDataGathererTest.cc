                 ...
                 const TDoubleVec &values)
{
    ...
    {
        addArrival(gatherer, time + (i * increment), person, values[i]);
    }
}
...
void CMetricDataGathererTest::singleSeriesTests(void)
{
    ...
    TTimeDoublePr bucket2[] =
        {
            TTimeDoublePr(600, 2.0),
            TTimeDoublePr(799, 2.2),
            TTimeDoublePr(1199, 1.8)
        };
    TTimeDoublePr bucket3[] =
        {
            TTimeDoublePr(1200, 2.1),
    ...
    TTimeDoublePr bucket5[] =
        {
            TTimeDoublePr(2420, 3.5),
        ...
        };
    {
        ...
        {
            addArrival(gatherer, bucket1[0].first, "p", bucket1[0].second);
        ...
        {
            addArrival(gatherer, bucket1[i].first, "p", bucket1[i].second);
        }
        {
            TFeatureSizeFeatureDataPrVecPrVec featureData;
        ...
        {
            addArrival(gatherer, bucket2[i].first, "p", bucket2[i].second);
        }
        {
            TFeatureSizeFeatureDataPrVecPrVec featureData;
    ...
    // Test capture of sample measurement count.
    {
        ...
        for (std::size_t i = 0u; i < buckets.size(); ++i)
        {
            ...
            {
                addArrival(gatherer, bucket[j].first, "p", bucket[j].second);
            }
        }
...
void CMetricDataGathererTest::multipleSeriesTests(void)
{
    ...
    CDataGatherer gatherer(model_t::E_Metric, model_t::E_None, params,
                           "", "", "", "", "", TStrVec(), false, KEY,
                           features, startTime, 0);
    CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson("p1", gatherer));
    CPPUNIT_ASSERT_EQUAL(std::size_t(1), addPerson("p2", gatherer));
    ...
    TTimeDoublePr bucket12[] =
        {
            TTimeDoublePr(600, 2.0),
            TTimeDoublePr(799, 2.2),
            TTimeDoublePr(1199, 1.8)
        };
    TTimeDoublePr bucket13[] =
        {
            TTimeDoublePr(1200, 2.1),
            TTimeDoublePr(1250, 2.5)
        };
    ...
    TTimeDoublePr bucket24[] =
        {
            TTimeDoublePr(1900, 3.5),
            TTimeDoublePr(1950, 3.5)
        };
    ...
    for (std::size_t i = 0u; i < 5; ++i)
    {
        ...
        {
            addArrival(gatherer, bucket1[j].first, "p1", bucket1[j].second);
        }
        ...
        {
            addArrival(gatherer, bucket2[j].first, "p2", bucket2[j].second);
    ...
    gatherer.personNonZeroCounts(startTime + 4 * bucketLength, nonZeroCounts);
    CPPUNIT_ASSERT_EQUAL(std::string("[(1, 6)]"),
                         core::CContainerPrinter::print(nonZeroCounts));
...
void CMetricDataGathererTest::testSampleCount(void)
{
    ...
    for (std::size_t i = 0u; i < numberBuckets; ++i)
    {
...

        {
            ...
            addArrival(gatherer, startTime + i * bucketLength + 20, "p1", 1.0);
            addArrival(gatherer, startTime + i * bucketLength + 40, "p1", 1.0);
            addArrival(gatherer, startTime + i * bucketLength + 60, "p1", 1.0);
            addArrival(gatherer, startTime + i * bucketLength + 80, "p1", 1.0);
            addArrival(gatherer, startTime + i * bucketLength + 100, "p1", 1.0);
            addArrival(gatherer, startTime + i * bucketLength + 120, "p1", 1.0);
        }
        {
            TDoubleVec count;
            ...
            {
                addArrival(gatherer, startTime + i * bucketLength + 100 * (j + 1), "p2", 1.0);
            }
        }
    }
    ...
    for (std::size_t i = numberBuckets; i < 100; ++i)
    {
        LOG_DEBUG("Processing bucket " << i);
        gatherer.timeNow(startTime + i * bucketLength);
        addArrival(gatherer, startTime + i * bucketLength + 10, "p1", 1.0);
    }
    LOG_DEBUG("p1 sample count = " << gatherer.effectiveSampleCount(pid1));
...
void CMetricDataGathererTest::testRemovePeople(void)
{
    ...
    CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson("p1", gatherer));
    CPPUNIT_ASSERT_EQUAL(std::size_t(1), addPerson("p2", gatherer));
    CPPUNIT_ASSERT_EQUAL(std::size_t(2), addPerson("p3", gatherer));
    CPPUNIT_ASSERT_EQUAL(std::size_t(3), addPerson("p4", gatherer));
    CPPUNIT_ASSERT_EQUAL(std::size_t(4), addPerson("p5", gatherer));
    CPPUNIT_ASSERT_EQUAL(std::size_t(5), addPerson("p6", gatherer));
    CPPUNIT_ASSERT_EQUAL(std::size_t(6), addPerson("p7", gatherer));
    CPPUNIT_ASSERT_EQUAL(std::size_t(7), addPerson("p8", gatherer));
    ...
    for (std::size_t i = 0u; i < boost::size(values); ++i)
    {
        for (std::size_t j = 0u; j < boost::size(values[i]); ++j)
        {
            if (values[i][j] > 0.0)
            {
                addArrival(gatherer, startTime + times[i][j], gatherer.personName(i), values[i][j]);
            }
        }
    }

    {
        ...
        CDataGatherer expectedGatherer(model_t::E_Metric, model_t::E_None, params,
                                       "", "", "", "", "", TStrVec(), false, KEY,
                                       features, startTime, 0);
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson("p3", expectedGatherer));
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), addPerson("p4", expectedGatherer));
        CPPUNIT_ASSERT_EQUAL(std::size_t(2), addPerson("p5", expectedGatherer));
        CPPUNIT_ASSERT_EQUAL(std::size_t(3), addPerson("p6", expectedGatherer));
        CPPUNIT_ASSERT_EQUAL(std::size_t(4), addPerson("p7", expectedGatherer));
        CPPUNIT_ASSERT_EQUAL(std::size_t(5), addPerson("p8", expectedGatherer));
    ...
    }
    {
        ...
        CDataGatherer expectedGatherer(model_t::E_Metric, model_t::E_None, params,
                                       ...
                                       features, startTime, 0);
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson("p3", expectedGatherer));
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), addPerson("p6", expectedGatherer));
    ...
    expectedRecycled.push_back(addPerson("p1", gatherer));
    expectedRecycled.push_back(addPerson("p7", gatherer));
...
void CMetricDataGathererTest::testSum(void)
{
    ...
    SModelParams params(bucketLength);
    CDataGatherer sum(model_t::E_Metric, model_t::E_None, params,
                      "", "", "", "", "", TStrVec(), false, KEY,
                      sumFeatures, startTime, 0);
    CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson("p1", sum));
    ...
    CDataGatherer nonZeroSum(model_t::E_Metric, model_t::E_None, params,
                             "", "", "", "", "", TStrVec(), false, KEY,
                             nonZeroSumFeatures, startTime, 0);
    CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson("p1", nonZeroSum));
    ...
    for (std::size_t i = 0u; i < boost::size(bucketCounts); ++i)
    {
        ...
        {
            addArrival(sum, bucketStart + static_cast<core_t::TTime>(times[j]), "p1", values[j]);
            addArrival(nonZeroSum, bucketStart + static_cast<core_t::TTime>(times[j]), "p1", values[j]);
...
void CMetricDataGathererTest::testResetBucketGivenSingleSeries(void)
{
    ...
    addArrival(gatherer, 610, "p", 2.0);
    addArrival(gatherer, 620, "p", 3.0);
...
void CMetricDataGathererTest::testResetBucketGivenMultipleSeries(void)
{
    ...
    addPerson("p3", gatherer);

    for (std::size_t i = 0; i < boost::size(data); ++i)
    {
        for (std::size_t pid = 0; pid < gatherer.numberActivePeople(); ++pid)
        {
            addArrival(gatherer, data[i].first, gatherer.personName(pid), data[i].second);
        }
    }
    ...
    for (std::size_t pid = 0; pid < gatherer.numberActivePeople(); ++pid)
    {
        addArrival(gatherer, 610, gatherer.personName(pid), 2.0);
        addArrival(gatherer, 620, gatherer.personName(pid), 3.0);
    }
...
void CMetricDataGathererTest::testInfluenceStatistics(void)
{
    ...
    CDataGatherer gatherer(model_t::E_Metric, model_t::E_None, params,
                           "", "", "", "", "", influencerNames, false, KEY,
                           features, startTime, 2u);

    addPerson("p1", gatherer, influencerNames.size());
    addPerson("p2", gatherer, influencerNames.size());
...
void CMetricDataGathererTest::testMultivariate(void)
{
    ...
    TTimeDoubleDoubleTuple bucket1[] =
        {
            TTimeDoubleDoubleTuple(1,   1.0, 1.0),
    ...
    TTimeDoubleDoubleTuple bucket2[] =
        {
            TTimeDoubleDoubleTuple(600,  2.0, 1.8),
            TTimeDoubleDoubleTuple(799,  2.2, 2.0),
            TTimeDoubleDoubleTuple(1199, 1.8, 1.6)
        };
    TTimeDoubleDoubleTuple bucket3[] =
        {
            TTimeDoubleDoubleTuple(1200, 2.1, 2.0),
    ...
    TTimeDoubleDoubleTuple bucket5[] =
        {
            TTimeDoubleDoubleTuple(2420, 3.5, 3.2),
...

    {
        ...
        {
            addArrival(gatherer, bucket1[0].get<0>(), "p", bucket1[0].get<1>(), bucket1[0].get<2>(), DELIMITER);
            TFeatureSizeFeatureDataPrVecPrVec featureData;
            gatherer.featureData(startTime, bucketLength, featureData);
            LOG_DEBUG("featureData = " << core::CContainerPrinter::print(featureData));
            CPPUNIT_ASSERT_EQUAL(1.0, featureData[0].second[0].second.s_BucketValue->value()[0]);
        ...
        for (size_t i = 1; i < boost::size(bucket1); ++i)
        {
            addArrival(gatherer, bucket1[i].get<0>(), "p", bucket1[i].get<1>(), bucket1[i].get<2>(), DELIMITER);
        }
        {
            TFeatureSizeFeatureDataPrVecPrVec featureData;
            gatherer.sampleNow(startTime);
        ...
        for (size_t i = 0; i < boost::size(bucket2); ++i)
        {
            addArrival(gatherer, bucket2[i].get<0>(), "p", bucket2[i].get<1>(), bucket2[i].get<2>(), DELIMITER);
        }
        {
            TFeatureSizeFeatureDataPrVecPrVec featureData;
            gatherer.sampleNow(startTime + bucketLength);
        ...
        {
            addArrival(gatherer, bucket3[i].get<0>(), "p", bucket3[i].get<1>(), bucket3[i].get<2>(), DELIMITER);
        }
        {
            TFeatureSizeFeatureDataPrVecPrVec featureData;
    ...
    // Test capture of sample measurement count.
    {
        ...
        for (std::size_t i = 0u; i < buckets.size(); ++i)
        {
            ...
            for (std::size_t j = 0u; j < bucket.size(); ++j)
            {
                addArrival(gatherer, bucket[j].get<0>(), "p", bucket[j].get<1>(), bucket[j].get<2>(), DELIMITER);
            }
        }
...
void CMetricDataGathererTest::testStatisticsPersist(void)
{
    ...
    core_t::TTime origTime = stat.sampleTime();
    ...
    core_t::TTime restoredTime;
    {
        ...
        traverser.traverseSubLevel(boost::bind(&CGathererTools::TMeanGatherer::TMetricPartialStatistic::restore,
                                               boost::ref(restored), _1));

        restoredTime = restored.sampleTime();
        {
            core::CRapidXmlStatePersistInserter inserter("root");
...
void CMetricDataGathererTest::testVarp(void)
{
...

    {
        ...
        CDataGatherer gatherer(model_t::E_Metric, model_t::E_None, params,
                               ...
                               features, startTime, 2u);
        CPPUNIT_ASSERT(!gatherer.isPopulation());
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(person, gatherer));
        ...
        {
            addArrivals(gatherer, startTime, 10, person, values);
            gatherer.sampleNow(startTime);
            TFeatureSizeFeatureDataPrVecPrVec featureData;
            gatherer.featureData(startTime, bucketLength, featureData);
            ...
            CPPUNIT_ASSERT_EQUAL(model_t::E_IndividualVarianceByPerson,
                                 fsfd.first);
        ...
        startTime += bucketLength;
        {
            ...
            addArrivals(gatherer, startTime, 100, person, values);
            gatherer.sampleNow(startTime);
            TFeatureSizeFeatureDataPrVecPrVec featureData;
            gatherer.featureData(startTime, bucketLength, featureData);
            LOG_DEBUG("featureData = " << core::CContainerPrinter::print(featureData));
    ...
    LOG_DEBUG("Testing influencers");
    {
        ...
        CDataGatherer gatherer(model_t::E_Metric, model_t::E_None, params,
                               "", "", "", "", "", influencerFieldNames, false, KEY,
                               features, startTime, 2u);
        CPPUNIT_ASSERT(!gatherer.isPopulation());
        CPPUNIT_ASSERT_EQUAL(std::size_t(0), addPerson(person, gatherer, influencerFieldNames.size()));
        ...
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), gatherer.numberFeatures());
        {
            addArrival(gatherer, startTime + 0,   person, 5.0, inf1, inf2);
            addArrival(gatherer, startTime + 100, person, 5.5, inf1, "");
            addArrival(gatherer, startTime + 200, person, 5.9, inf1, "");
            addArrival(gatherer, startTime + 300, person, 5.2, inf1, "");
            addArrival(gatherer, startTime + 350, person, 5.1, inf1, "");
            addArrival(gatherer, startTime + 400, person, 2.2, inf1, inf2);
            addArrival(gatherer, startTime + 500, person, 4.9, inf1, "");
            addArrival(gatherer, startTime + 600, person, 5.1, inf1, "");
            addArrival(gatherer, startTime + 650, person, 1.0, "", "");
            addArrival(gatherer, startTime + 700, person, 5.0, inf1, "");
            addArrival(gatherer, startTime + 800, person, 12.12, inf1, inf2);
            addArrival(gatherer, startTime + 900, person, 5.2, inf1, "");
            addArrival(gatherer, startTime + 950, person, 5.0, inf1, inf3);
