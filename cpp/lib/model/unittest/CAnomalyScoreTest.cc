...
void CAnomalyScoreTest::testNormalizeScoresQuantiles(void)
{
    ...
    model::CModelConfig config = model::CModelConfig::defaultConfig(1800);
    ...
    CPPUNIT_ASSERT(totalError / numberSamples < 0.0032);
}
...
void CAnomalyScoreTest::testNormalizeScoresOrdering(void)
{
    ...
    for (std::size_t i = 200u; i <= n; i += 200)
    {
        ...
        model::CModelConfig config = model::CModelConfig::defaultConfig(300);
        model::CAnomalyScore::CNormalizer normalizer(config);
...
void CAnomalyScoreTest::testJsonConversion(void)
{
    ...
    model::CModelConfig config = model::CModelConfig::defaultConfig();
...
{
    public:
        CParseDouble(CAnomalyScoreTest::TDoubleVec &values) : m_Values(&values)
        {
        }
        ...
        {
            double v;
            CPPUNIT_ASSERT(core::CStringUtils::stringToType(value, v));
            m_Values->push_back(v);
            return true;
        }

    private:
        CAnomalyScoreTest::TDoubleVec *m_Values;
};

}

void CAnomalyScoreTest::importData(const std::string &fileName,
                                   TDoubleVec &scores)
{
    core::CTextFileWatcher file;
    CPPUNIT_ASSERT(file.init(fileName, "\r?\n", core::CTextFileWatcher::E_Start));

    std::string remainder;
    file.readAllLines(CParseDouble(scores), remainder);
