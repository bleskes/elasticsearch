...
{
    // This test is based on Splunk's "flower shop" demo, where there are 11
    // distinct hours in which HTTP status 503 errors correspond to MySQL
    // aborting.  (There are a total of 15 occurrences of this pattern, but only
    // 11 distinct hours in which it happens.)  The first and last times below
