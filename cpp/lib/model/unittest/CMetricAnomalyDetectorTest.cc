...
class CResultWriter : public ml::model::CHierarchicalResultsVisitor
{
    ...
    public:
        CResultWriter(const model::CModelConfig &modelConfig,
    ...
    private:
        const model::CModelConfig &m_ModelConfig;
...
{
    maths::CTimeSeriesTestData::TTimeDoublePrVec timeData;
    CPPUNIT_ASSERT(maths::CTimeSeriesTestData::parse(fileName, timeData));
...
void CMetricAnomalyDetectorTest::testAnomalies(void)
{
    ...
    for (size_t i = 0; i < boost::size(BUCKET_LENGTHS); ++i)
    {
        model::CModelConfig modelConfig =
                model::CModelConfig::defaultConfig(BUCKET_LENGTHS[i]);
        ...
        if (!anomalyFactors.empty())
        {
            ...
            CPPUNIT_ASSERT(signal / noise > 50.0);
        }
    ...
    CPPUNIT_ASSERT(::fabs(lowRateNoise / highRateNoise - 1.0) < 0.5);
}
...
void CMetricAnomalyDetectorTest::testExcludeFrequent(void)
{
...

    {
        ...
        CPPUNIT_ASSERT_DOUBLES_EQUAL(76.0, highAnomalyFactors[1], 0.4);
    }
    {
        model::CModelConfig modelConfig = model::CModelConfig::defaultConfig(BUCKET_LENGTH);
        model::CLimits limits;
        ...
        CPPUNIT_ASSERT_DOUBLES_EQUAL(15.9, highAnomalyFactors[0], 0.2);
    }
}
