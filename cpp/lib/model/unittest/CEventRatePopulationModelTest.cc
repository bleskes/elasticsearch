...
struct SMessage
{
    ...
    bool operator<(const SMessage &other) const
    {
        return maths::COrderings::lexicographical_compare(s_Time, s_Person, s_Attribute,
                                                          other.s_Time, other.s_Person, other.s_Attribute);
    }
...
void CEventRatePopulationModelTest::testFeatures(void)
{
    ...
    typedef TSizeSizeSetMap::const_iterator TSizeSizeSetMapCItr;
    typedef std::pair<std::string, std::string> TStrStrPr;
    typedef std::map<TStrStrPr, uint64_t> TStrStrPrUint64Map;
    typedef TStrStrPrUint64Map::const_iterator TStrStrPrUint64MapCItr;
    ...
    typedef TSizeSizePrUInt64Map::const_iterator TSizeSizePrUInt64MapCItr;
    ...
    // Bucket non-zero count unique person count.
    {
        ...
        TFeaturePriorPtrPrVec priors = factory.defaultPriors(features);
        CPPUNIT_ASSERT_EQUAL(std::size_t(1), priors.size());
        ...
        for (std::size_t i = 0u; i < messages.size(); ++i)
        {
            if (messages[i].s_Time >= startTime + bucketLength)
            {
                ...
                for (TStrStrPrUint64MapCItr countItr = expectedCounts.begin();
                     countItr != expectedCounts.end();
                     ++countItr)
                {
                    std::size_t pid, cid;
                    CPPUNIT_ASSERT(gatherer->personId(countItr->first.first, pid));
                    CPPUNIT_ASSERT(gatherer->attributeId(countItr->first.second, cid));

                    numberAttributes = std::max(numberAttributes, cid + 1);
                    numberPeople = std::max(numberPeople, pid + 1);
                    attributePeople[cid].insert(pid);
                    expectedNonZeroCounts[std::make_pair(pid, cid)] = countItr->second;
                }

                TSizeDoubleVecDoubleVecVecPrMap populationSamples;
                for (TSizeSizePrUInt64MapCItr countItr = expectedNonZeroCounts.begin();
                     countItr != expectedNonZeroCounts.end();
                     ++countItr)
                {
                    std::size_t pid = countItr->first.first;
                    std::size_t cid = countItr->first.second;
                    double count = model_t::offsetCountToZero(
                                         model_t::E_PopulationCountByBucketPersonAndAttribute,
                                         static_cast<double>(countItr->second));

                    TDoubleVec sample(1, count);
                    TDoubleVecVec weight(1, TDoubleVec(1, model->sampleRateWeight(pid, cid)));
                    populationSamples[cid].first.push_back(sample[0]);
                    populationSamples[cid].second.push_back(weight[0]);
                }
                for (TSizeDoubleVecDoubleVecVecPrMapItr itr = populationSamples.begin();
                     itr != populationSamples.end();
                     ++itr)
                {
                    std::size_t cid = itr->first;
                    TDoubleVec &samples = itr->second.first;
                    TDoubleVecVec &weights = itr->second.second;
                    maths::COrderings::simultaneousSort(samples, weights);
                    if (expectedPopulationPriors.count(cid) == 0)
                    {
                        expectedPopulationPriors[cid].reset(priors[0].second->clone());
                    }
                    expectedPopulationPriors[cid]->addSamples(COUNT_WEIGHT, samples, weights);
                }
                ...
                for (std::size_t j = 0u; j < numberAttributes; ++j)
                {
                    expectedPeoplePerAttribute.push_back(TSizeSizePrFeatureDataPr(std::make_pair(size_t(0), j),
                        TFeatureData(j)));
                }
                for (TSizeSizeSetMapCItr attributeItr = attributePeople.begin();
                     attributeItr != attributePeople.end();
                     ++attributeItr)
                {
                    expectedPeoplePerAttribute[attributeItr->first].second = attributeItr->second.size();
                }
                ...
                {
                    const maths::CPrior *populationPrior =
                            model->details()->prior(model_t::E_PopulationCountByBucketPersonAndAttribute, cid);
            ...
            const SMessage &m = messages[i];
            addArrival(m, gatherer);
            ++expectedCounts[TStrStrPr(m.s_Person, m.s_Attribute)];
        }
    }
}
...
void CEventRatePopulationModelTest::testComputeProbability(void)
{
            ...
            for (std::size_t pid = 0u; pid < gatherer->numberActivePeople(); ++pid)
            {
                ...
                for (std::size_t j = 0u; j < annotatedProbability.s_AttributeProbabilities.size(); ++j)
                {
                    attributes.push_back(
                            TDoubleStrPr(annotatedProbability.s_AttributeProbabilities[j].s_Probability,
    ...
    CPPUNIT_ASSERT_EQUAL(boost::size(expectedAnomalies),
                         orderedAnomalies.size());
...
void CEventRatePopulationModelTest::testPeriodicity(void)
{
         ...
         time += bucketLength)
    {
        for (std::size_t i = 0u; i < boost::size(attributes); ++i)
        {
            ...
            for (std::size_t j = 0u; j < rates.size(); ++j)
            {
                ...
                {
                    messages.push_back(SMessage(time + (t * bucketLength) / (rates[j] + 1),
                                                people[j],
                                                attributes[i]));
                }
            }
    ...
    CPPUNIT_ASSERT(totalw > 1.5 * totalwo);
}
...
void CEventRatePopulationModelTest::testSkipSampling()
{
    ...
    CModel::TModelPtr modelNoGapHolder(factory.makeModel(modelNoGapInitData));
    CEventRatePopulationModel *modelNoGap = dynamic_cast<CEventRatePopulationModel*>(modelNoGapHolder.get());

    addArrival(SMessage(100, "p1", "a1"), gathererNoGap);
    addArrival(SMessage(100, "p1", "a2"), gathererNoGap);
    addArrival(SMessage(100, "p2", "a1"), gathererNoGap);
    modelNoGap->sample(100, 200, resourceMonitor);
    addArrival(SMessage(200, "p1", "a1"), gathererNoGap);
    modelNoGap->sample(200, 300, resourceMonitor);
    addArrival(SMessage(300, "p1", "a1"), gathererNoGap);
    ...
    CModel::TModelPtr modelWithGapHolder(factory.makeModel(modelWithGapInitData));
    CEventRatePopulationModel *modelWithGap = dynamic_cast<CEventRatePopulationModel*>(modelWithGapHolder.get());

    addArrival(SMessage(100, "p1", "a1"), gathererWithGap);
    addArrival(SMessage(100, "p1", "a2"), gathererWithGap);
    addArrival(SMessage(100, "p2", "a1"), gathererWithGap);
    modelWithGap->sample(100, 200, resourceMonitor);
    addArrival(SMessage(200, "p1", "a1"), gathererWithGap);
    ...
    addArrival(SMessage(1000, "p1", "a1"), gathererWithGap);
    modelWithGap->sample(1000, 1100, resourceMonitor);
    addArrival(SMessage(1100, "p1", "a1"), gathererWithGap);
    modelWithGap->sample(1100, 1200, resourceMonitor);

    // Check priors are the same
    CPPUNIT_ASSERT_EQUAL(
            modelWithGap->details()->prior(model_t::E_PopulationCountByBucketPersonAndAttribute, 0)->checksum(),
            modelNoGap->details()->prior(model_t::E_PopulationCountByBucketPersonAndAttribute, 0)->checksum());
    CPPUNIT_ASSERT_EQUAL(
            modelWithGap->details()->prior(model_t::E_PopulationCountByBucketPersonAndAttribute, 1)->checksum(),
            modelNoGap->details()->prior(model_t::E_PopulationCountByBucketPersonAndAttribute, 1)->checksum());
...
void CEventRatePopulationModelTest::testInterimCorrections(void)
{
    ...
    while (now < endTime)
    {
        ...
        {
            addArrival(SMessage(now, "p1", "a1"), gatherer);
        }
        for (std::size_t i = 0; i < samples[1]; ++i)
        {
            addArrival(SMessage(now, "p2", "a1"), gatherer);
        }
        for (std::size_t i = 0; i < samples[2]; ++i)
        {
            addArrival(SMessage(now, "p3", "a2"), gatherer);
        }
    ...
    TDouble1Vec p1a1Baseline = model->baselineBucketMean(model_t::E_PopulationCountByBucketPersonAndAttribute,
                                                         0, 0, type, NO_CORRELATES, now);
    TDouble1Vec p2a1Baseline = model->baselineBucketMean(model_t::E_PopulationCountByBucketPersonAndAttribute,
                                                         0, 0, type, NO_CORRELATES, now);
    TDouble1Vec p3a2Baseline = model->baselineBucketMean(model_t::E_PopulationCountByBucketPersonAndAttribute,
                                                         2, 1, type, NO_CORRELATES, now);
