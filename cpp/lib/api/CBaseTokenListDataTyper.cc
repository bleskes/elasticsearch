...
// Initialise statics
const std::string CBaseTokenListDataTyper::RAW_NAME("_raw");
...
CBaseTokenListDataTyper::CBaseTokenListDataTyper(const TTokenListReverseSearchCreatorIntfCPtr &reverseSearchCreator,
    ...
    : CDataTyper(fieldName.empty() ? RAW_NAME : fieldName),
...
{
    // Type number is deque index plus one
    int type(1);
    for (TTokenListTypeDequeCItr iter = m_Types.begin();
         iter != m_Types.end();
         ++iter)
    {
        LOG_DEBUG("Type=" << type <<
                  '-' << iter->numMatches() << ' ' << iter->baseString());
        ++type;
    }
}
                                         ...
                                         size_t rawStringLen)
{
         ...
         ++iter)
    {
        ...
        if (matchesSearch || similarity > m_UpperThreshold)
        {
            ...
            // looking for better matches - use deque index plus one as type
    ...
    {
        // Return the best match - use deque index plus one as type
    ...
    for (TSizeSizePrVecCItr iter = m_WorkTokenIds.begin();
         iter != m_WorkTokenIds.end();
         ++iter)
    {
        // We get away with casting away constness ONLY because the type count
        // is not used in any of the multi-index keys
        const_cast<CTokenInfoItem &>(m_TokenIdLookup[iter->first]).incTypeCount();
    }

    // Type is deque index plus one
    return int(m_Types.size());
}

bool CBaseTokenListDataTyper::createReverseSearch(const std::string &sourceType,
                                                  int type,
                                                  std::string &part1,
                                                  std::string &part2,
                                                  ...
                                                  bool &wasCached)
{
    ...
    // Find the correct type object - type is deque index plus one
    if (type < 1 || static_cast<size_t>(type) > m_Types.size())
    {
        ...
        return m_ReverseSearchCreator->createNullSearch(sourceType,
                                                        part1,
                                                        part2);
    }
    ...
    if (commonUniqueTokenIds.empty())
    {
        ...
        if (m_ReverseSearchCreator->createNoUniqueTokenSearch(sourceType,
                                                              type,
                                                              typeObj.baseString(),
    ...
    size_t availableCost(m_ReverseSearchCreator->availableCost(sourceType));
    ...
    typedef TSizeSizeSizePrMMap::const_iterator TSizeSizeSizePrMMapCItr;
    TSizeSizeSizePrMMap rareIdsWithCost;
    size_t lowestCost(std::numeric_limits<size_t>::max());
    for (TSizeSizePrVecCItr iter = commonUniqueTokenIds.begin();
         iter != commonUniqueTokenIds.end();
         ++iter)
    {
        size_t tokenId(iter->first);
    ...
    typedef TSizeSet::const_iterator TSizeSetCItr;
    TSizeSet costedCommonUniqueTokenIds;
    size_t cheapestCost(std::numeric_limits<size_t>::max());
    TSizeSizeSizePrMMapCItr cheapestIter = rareIdsWithCost.end();
    for (TSizeSizeSizePrMMapCItr iter = rareIdsWithCost.begin();
    ...
    m_ReverseSearchCreator->initStandardSearch(sourceType,
                                               type,
                                               typeObj.baseString(),
                                               typeObj.maxMatchingStringLen(),
                                               part1,
    ...
    for (TSizeSetCItr iter = costedCommonUniqueTokenIds.begin();
         iter != costedCommonUniqueTokenIds.end();
         ++iter)
    {
        m_ReverseSearchCreator->addCommonUniqueToken(m_TokenIdLookup[*iter].str(),
                                                     part1,
                                                     part2);
    }
    ...
    m_ReverseSearchCreator->closeStandardSearch(sourceType,
                                                part1,
                                                part2);
...
{
    for (size_t index = 0; index < m_TokenIdLookup.size(); ++index)
    {
        const CTokenInfoItem &item = m_TokenIdLookup[index];
...
{
    TTokenItr iter = boost::multi_index::get<SToken>(m_TokenIdLookup).find(token);
...
{
    for (CBaseTokenListDataTyper::TSizeSizePrVecCItr iter = translator.s_TokenIds.begin();
