...
bool COutputChainer::settings(const TStrStrUMap &settings)
{
    return m_DataProcessor.handleSettings(settings);
}
...
bool COutputChainer::writeRow(bool isDryRun,
                              ...
                              const TStrStrUMap &overrideDataRowFields)
{
         ...
         ++fieldNameIter, ++preComputedHashIter, ++fieldRefIter)
    {
        ...
        fieldRefIter->get().assign(fieldValueIter->second,
                                   0,
                                   fieldValueIter->second.length());
    }

    if (m_DataProcessor.handleRecord(isDryRun,
                                     m_FieldNames,
                                     m_WorkRecordFields) == false)
    {
        LOG_ERROR("Chained data processor function returned false for record:" << core_t::LINE_ENDING <<
                  CDataProcessor::debugPrintRecord(m_FieldNames,
                                                   m_WorkRecordFields));
        return false;
    }
...
bool COutputChainer::periodicPersistState(core::CDataAdder &persister)
{
