...
// A type of token list data typer that DOESN'T exclude fields from its analysis
                ...
                api::CDataTyper &dataTyper)
{
    ...
    dataTyper.computeType(false, record, raw, origRawLen);
...
*/
    if (argc != 2)
    {
        LOG_FATAL("Usage " << argv[0] << ": filename");
        return EXIT_FAILURE;
    }
    ...
    TTokenListDataTyperKeepsFields dataTyper(NO_REVERSE_SEARCH_CREATOR, 0.7);
