...
CResultNormalizer::CResultNormalizer(const model::CModelConfig &modelConfig,
...
bool CResultNormalizer::handleRecord(bool /* isDryRun */,
                                     ...
                                     const TStrStrUMap &dataRowFields)
{
    ...
    {
        isValidRecord = parseDataFields(fieldNames, dataRowFields, level, partition, partitionValue,
                                        person, function, valueFieldName, probability);
    }
    else
    {
        isValidRecord = parseDataFields(fieldNames, dataRowFields, level, partition, person,
    ...
    if (m_OutputHandler.writeRow(false, dataRowFields, m_OutputFields) == false)
    {
...
bool CResultNormalizer::parseDataFields(const TStrVec &fieldNames, const TStrStrUMap &dataRowFields,
                                        std::string &level, std::string &partition,
                                        std::string &person, std::string &function,
                                        std::string &valueFieldName, double &probability)
{
    return this->parseDataField(fieldNames, dataRowFields, LEVEL, level)
        && this->parseDataField(fieldNames, dataRowFields, PARTITION_FIELD_NAME, partition)
        && this->parseDataField(fieldNames, dataRowFields, PERSON_FIELD_NAME, person)
        && this->parseDataField(fieldNames, dataRowFields, FUNCTION_NAME, function)
        && this->parseDataField(fieldNames, dataRowFields, VALUE_FIELD_NAME, valueFieldName)
        && this->parseDataField(fieldNames, dataRowFields, PROBABILITY_NAME, probability);
}
                                        ...
                                        double &probability)
{
    return this->parseDataField(fieldNames, dataRowFields, LEVEL, level)
        && this->parseDataField(fieldNames, dataRowFields, PARTITION_FIELD_NAME, partition)
        && this->parseDataField(fieldNames, dataRowFields, PARTITION_FIELD_VALUE, partitionValue)
        && this->parseDataField(fieldNames, dataRowFields, PERSON_FIELD_NAME, person)
        && this->parseDataField(fieldNames, dataRowFields, FUNCTION_NAME, function)
        && this->parseDataField(fieldNames, dataRowFields, VALUE_FIELD_NAME, valueFieldName)
        && this->parseDataField(fieldNames, dataRowFields, PROBABILITY_NAME, probability);
