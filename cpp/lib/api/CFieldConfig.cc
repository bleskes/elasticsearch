...
const std::string CFieldConfig::LIST_PREFIX("list.");
...
CFieldConfig::CFieldConfig(bool allowPopulationConfigs)
    : m_AllowPopulationConfigs(allowPopulationConfigs)
{
}
...
CFieldConfig::CFieldConfig(const std::string &fieldName,
    ...
    : m_AllowPopulationConfigs(false),
      m_SummaryCountFieldName(summaryCountFieldName)
{
    ...
    // field name even though this doesn't match the Splunk command line
    // behaviour
...
bool CFieldConfig::initFromFile(const std::string &configFile)
{
         ...
         ++level1Iter)
    {
        ...
        else if (level1Key.length() > LIST_PREFIX.length() &&
                 level1Key.compare(0, LIST_PREFIX.length(), LIST_PREFIX) == 0)
        {
            this->processList(level1Key, value);
        }
        else
        {
...
bool CFieldConfig::writeToFile(const std::string &configFile, bool forceNewKeys) const
{
    LOG_DEBUG("Writing config file " << configFile);

    std::ofstream strm(configFile.c_str());
    if (!strm.is_open())
    {
        LOG_ERROR("Error writing config file " << configFile);
        return false;
    }

    int counter(0);

    // First write detectors
    this->appendConfigSegment(m_FieldOptions, forceNewKeys, counter, strm);

    // Then write influencers
    if (!m_Influencers.empty())
    {
        strm << '\n';
        counter = 0;
        for (TStrVecCItr iter = m_Influencers.begin();
             iter != m_Influencers.end();
             ++iter)
        {
            strm << INFLUENCER_PREFIX << (++counter) << " = " << *iter << '\n';
        }
    }
...
bool CFieldConfig::tokenise(const std::string &clause,
                            TStrVec &copyTokens)
{
    // As per Splunk stats/timechart behaviour, tokenise on spaces or commas.
    // Double quotes are used for quoting, and the escape character is a
    // backslash.
...
void CFieldConfig::retokenise(const TStrVec &tokens,
                              TStrVec &copyTokens)
{
    for (TStrVecCItr iter = tokens.begin(); iter != tokens.end(); ++iter)
    {
        size_t commaPos(iter->find(','));
        if (commaPos == std::string::npos)
        {
            copyTokens.push_back(*iter);
        }
        else
        {
            size_t startPos(0);
            do
            {
                if (commaPos > startPos)
                {
                    ...
                    copyTokens.back().assign(*iter,
                                             startPos,
                                             commaPos - startPos);
                }

                startPos = commaPos + 1;
                commaPos = iter->find(',', startPos);
            }
            while (commaPos != std::string::npos);

            if (startPos < iter->length())
            {
                copyTokens.resize(copyTokens.size() + 1);
                copyTokens.back().assign(*iter,
                                         startPos,
                                         iter->length() - startPos);
            }
        }
    }

    for (TStrVecCItr iter = copyTokens.begin();
         iter != copyTokens.end();
         ++iter)
    {
        LOG_DEBUG(*iter);
                                       ...
                                       std::string &fieldName)
{
    if (thisIndex != copyTokens.size())
    {
        ...
        for (TStrVecCItr iter = clashingNames.begin();
                iter != clashingNames.end(); ++iter)
        {
            if (fieldName == *iter)
            {
...
{
    TDetectionRuleVec &detectionRules = m_DetectorRules[detectorIndex];
    if (rules.empty())
    {
        return true;
    }

    CDetectionRulesJsonParser rulesParser(m_RuleLists);
...
{
    for (TFieldOptionsMIndexCItr iter = m_FieldOptions.begin();
         iter != m_FieldOptions.end();
         ++iter)
    {
        if (!iter->partitionFieldName().empty())
        {
            return true;
        }
    }
                                     ...
                                     TStrVec &copyTokens)
{
    if (this->parseClause(false,
                          ...
                          m_SummaryCountFieldName) == false)
    {
        // parseClause() will have logged the error
        return false;
    }

    if (this->parseRules(configKey, rules) == false)
    {
...
void CFieldConfig::debug(const TFieldOptionsMIndex &fieldsForSourceType,
                         std::ostream &strm) const
{
    for (TFieldOptionsMIndexCItr iter = fieldsForSourceType.begin();
         iter != fieldsForSourceType.end();
         ++iter)
    {
        strm << *iter << '|';
    }
...
bool CFieldConfig::allowPopulationConfigs(void) const
{
    return m_AllowPopulationConfigs;
}

void CFieldConfig::allowPopulationConfigs(bool allowPopulationConfigs)
{
    m_AllowPopulationConfigs = allowPopulationConfigs;
}
...
void CFieldConfig::addInfluencerFieldsFromByOverPartitionFields(const TFieldOptionsMIndex &fieldsForSourceType)
{
    for (TFieldOptionsMIndexCItr iter = fieldsForSourceType.begin();
         iter != fieldsForSourceType.end();
         ++iter)
    {
        this->addInfluencerFieldName(iter->byFieldName(), true);
        this->addInfluencerFieldName(iter->overFieldName(), true);
        this->addInfluencerFieldName(iter->partitionFieldName(), true);
    }
}
...
bool CFieldConfig::processList(const std::string &key,
                               const std::string &value)
{
    size_t sepPos(key.rfind(SUFFIX_SEPARATOR));
    if (sepPos == std::string::npos)
    {
        LOG_ERROR("Unrecognised list key: " + key);
        return false;
    }
    std::string listId = key.substr(sepPos + 1);
    core::CPatternSet &list = m_RuleLists[listId];
    return list.initFromJson(value);
}

std::ostream &CFieldConfig::appendConfigSegment(const TFieldOptionsMIndex &fieldsForSourceType,
                                                bool forceNewKeys,
                                                int &counter,
                                                std::ostream &strm) const
{
         ...
         ++iter)
    {
        if (forceNewKeys)
        {
            iter->appendConfigSegment(++counter,
        ...
        {
            iter->appendConfigSegment(m_SummaryCountFieldName,
                                      m_CategorizationFieldName,
                                      strm);
        }
    }
...
std::ostream &CFieldConfig::CFieldOptions::appendConfigClause(std::ostream &strm) const
{
    strm << this->verboseFunctionName();
    if (!m_FieldName.empty())
    {
        strm << '(' << CTermQuoter::doubleQuote(m_FieldName) << ')';
    }
    bool considerUseNull(false);
    if (!m_ByFieldName.empty())
    {
        strm << ' ' << BY_TOKEN << ' ' << CTermQuoter::doubleQuote(m_ByFieldName);
        considerUseNull = true;
    }
    if (!m_OverFieldName.empty())
    {
        strm << ' ' << OVER_TOKEN << ' ' << CTermQuoter::doubleQuote(m_OverFieldName);
        considerUseNull = true;
    }
    if (!m_PartitionFieldName.empty())
    {
        strm << ' ' << PARTITION_FIELD_OPTION << '=' << CTermQuoter::doubleQuote(m_PartitionFieldName);
    }
...
std::ostream &CFieldConfig::CFieldOptions::appendConfigSegment(const std::string &summaryCountFieldName,
                                                               const std::string &categorizatationFieldName,
                                                               std::ostream &strm) const
{
    return this->appendConfigSegment(m_ConfigKey,
                                     summaryCountFieldName,
                                     categorizatationFieldName,
                                     strm);
}

std::ostream &CFieldConfig::CFieldOptions::appendConfigSegment(int configKeyOverride,
                                                               const std::string &summaryCountFieldName,
                                                               const std::string &categorizatationFieldName,
                                                               std::ostream &strm) const
{
    ...
    {
        strm << DETECTOR_PREFIX << configKeyOverride << DESCRIPTION_SUFFIX << " = " << m_Description << '\n';
    }

    strm << DETECTOR_PREFIX << configKeyOverride << CLAUSE_SUFFIX << " = ";
    ...
    if (!summaryCountFieldName.empty())
    {
        strm << ' ' << SUMMARY_COUNT_FIELD_OPTION << '=' << CTermQuoter::doubleQuote(summaryCountFieldName);
    }
    strm << '\n';
...
{
    options.appendConfigClause(strm);
