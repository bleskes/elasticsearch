...
CHierarchicalResultsWriter::SResults::SResults(bool isAllTimeResult,
    ...
    : s_ResultType(E_PopulationResult),
      s_IsAllTimeResult(isAllTimeResult),
      s_IsOverallResult(isOverallResult),
      s_UseNull(useNull),
      s_IsMetric(metric),
      ...
      s_CorrelatedByFieldValue(correlatedByFieldValue),
      s_OverFieldName(overFieldName),
      s_OverFieldValue(overFieldValue),
      s_MetricValueField(metricValueField),
      s_BucketStartTime(bucketStartTime),
      s_BucketSpan(bucketSpan),
      s_FunctionName(functionName),
      s_FunctionDescription(functionDescription),
      s_FunctionValue(functionValue),
      s_PopulationAverage(populationAverage),
      s_BaselineRate(0.0),
      s_CurrentRate(currentRate),
      s_BaselineMean(0.0),
      s_CurrentMean(0.0),
...
CHierarchicalResultsWriter::SResults::SResults(EResultType resultType,
                                     ...
                                     int identifier,
                                     core_t::TTime bucketSpan)
    : s_ResultType(resultType),
      s_IsAllTimeResult(false),
      s_IsOverallResult(true),
      s_UseNull(useNull),
      s_IsMetric(metric),
      s_PartitionFieldName(partitionFieldName),
      s_PartitionFieldValue(partitionFieldValue),
      ...
      s_ByFieldValue(byFieldValue),
      s_CorrelatedByFieldValue(correlatedByFieldValue),
      // The event rate chart in the population view in the Splunk app expects
      // person field to be "count" for the simple count model
      // TODO - revisit this if we completely redesign the Splunk app output
      s_OverFieldName((byFieldName == COUNT_NAME) ? COUNT_NAME : EMPTY_STRING),
      s_OverFieldValue(EMPTY_STRING),
      s_MetricValueField(metricValueField),
      s_BucketStartTime(bucketStartTime),
      s_BucketSpan(bucketSpan),
      s_FunctionName(functionName),
      s_FunctionDescription(functionDescription),
      s_FunctionValue(0.0),
      s_PopulationAverage(0.0),
      s_BaselineRate(baselineRate),
      s_CurrentRate(currentRate),
      s_BaselineMean(baselineMean),
      s_CurrentMean(currentMean),
      s_RawAnomalyScore(rawAnomalyScore),
      s_NormalizedAnomalyScore(normalizedAnomalyScore),
      s_Probability(probability),
      s_Influences(influences),
                                                       ...
                                                       const TNode &node)
{
    ...
    // the GUI.
    const std::string &functionDescription = node.s_AnnotatedProbability.s_AttributeProbabilities.empty() ?
            EMPTY_STRING : model_t::outputFunctionName(node.s_AnnotatedProbability.s_AttributeProbabilities[0].s_Feature);
                                                       ...
                                                       const TNode &node)
{
    ...
    model_t::EFeature feature = node.s_AnnotatedProbability.s_AttributeProbabilities.empty() ?
