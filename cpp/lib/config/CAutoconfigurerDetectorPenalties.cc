    ...
    CAutoconfigurerDetectorPenalties::penaltyFor(const CDetectorSpecification &spec)
{
    return TPenaltyPtr((  this->fieldRolePenalty(spec)
                        ...
                        * CPolledDataPenalty(m_Params)
                        * CLongTailPenalty(m_Params)
                        * CLowInformationContentPenalty(m_Params)
                        * CNotEnoughDataPenalty(m_Params)
                        * CTooMuchDataPenalty(m_Params)
                        * CLowVariationPenalty(m_Params)
                        * CSparseCountPenalty(m_Params)).clone());
}
    ...
    if (!result)
    {
        ...
        if (spec.byField())
        {
            penalty.addPenalty(constants::BY_INDEX,
                               config_t::isRare(spec.function()) ? penalties.rareByPenalty() :
                                                                   penalties.byPenalty());
        }
        if (spec.overField())
        {
