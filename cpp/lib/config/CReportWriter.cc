...
{
    return  std::string((padTo - value.length()) / 2, ' ')
          + value
...
bool CReportWriter::writeRow(bool /*isDryRun*/,
                                       ...
                                       const CDataSummaryStatistics &summary)
{
    ...
    m_UnclassifiedFields[n][EARLIEST_TIME] = core::CTimeUtils::toLocalString(summary.earliest());
    m_UnclassifiedFields[n][LATEST_TIME]   = core::CTimeUtils::toLocalString(summary.latest());
                                       ...
                                       const CCategoricalDataSummaryStatistics &summary)
{
    ...
    m_CategoricalFields[n][EARLIEST_TIME] = core::CTimeUtils::toLocalString(summary.earliest());
    m_CategoricalFields[n][LATEST_TIME]   = core::CTimeUtils::toLocalString(summary.latest());
...
void CReportWriter::addDetector(const CDetectorSpecification &spec)
{
    ...
    for (std::size_t i = 0u; i < scores.size(); ++i)
    {
        ...
        m_Detectors[n][PARAMETER_SCORES][i][DESCRIPTION_PARAMETER]   = scores[i].s_Descriptions.empty() ?
                                                                       std::string("-") : scores[i].s_Descriptions[0];
...
void CReportWriter::write(void) const
{
    ...
    m_WriteStream << "Found " << (  m_UnclassifiedFields.size()
                                  + m_CategoricalFields.size()
                                  + m_NumericFields.size()) << " fields\n";
