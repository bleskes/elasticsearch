...
{
    uint32_t category = m_Approximating ?
            static_cast<uint32_t>(category_) : CTools::category32(category_);
    std::size_t i = std::lower_bound(m_Calibrators.begin(),
                                     m_Calibrators.end(),
                                     category,
                                     maths::COrderings::SFirstLess()) - m_Calibrators.begin();
...
double CCategoricalDataSummaryStatistics::calibratedCount(std::size_t category) const
{
    if (!m_Approximating)
    {
        return m_ValueCounts.find(category)->second;
    }
