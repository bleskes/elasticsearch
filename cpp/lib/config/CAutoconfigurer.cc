...
class CONFIG_EXPORT CAutoconfigurerImpl : public core::CNonCopyable
{
    ...
    public:
        ...
        bool handleRecord(const TStrVec &fieldNames, const TStrStrUMap &fieldValues);
    ...
    private:
        ...
        bool extractTime(const TStrVec &fieldNames,
                         const TStrStrUMap &fieldValues,
                         core_t::TTime &time) const;

        //! Initialize the field statistics.
        void initializeFieldStatisticsOnce(const TStrVec &fieldNames);
...
bool CAutoconfigurer::handleSettings(const TStrStrUMap &settings)
{
    m_Impl->reportWriter().settings(settings);
...
bool CAutoconfigurer::handleRecord(bool /*isDryRun*/,
                                   const TStrVec &fieldNames,
                                   const TStrStrUMap &fieldValues)
{
    return m_Impl->handleRecord(fieldNames, fieldValues);
}
...
bool CAutoconfigurerImpl::handleRecord(const TStrVec &fieldNames,
                                       const TStrStrUMap &fieldValues)
{
    ++m_NumberRecords;
    ...
    core_t::TTime time = 0;
    if (!this->extractTime(fieldNames, fieldValues, time))
    {
        ++m_NumberRecordsWithNoOrInvalidTime;
        return true;
    }
    this->initializeFieldStatisticsOnce(fieldNames);
...
bool CAutoconfigurerImpl::extractTime(const TStrVec &fieldNames,
                                      const TStrStrUMap &fieldValues,
                                      core_t::TTime &time) const
{
    ...
    if (i == fieldValues.end())
    {
        LOG_ERROR("No time field '" << m_Params.timeFieldName()
                  ...
                  << CAutoconfigurer::debugPrintRecord(fieldNames, fieldValues));
        return false;
    }
...
void CAutoconfigurerImpl::initializeFieldStatisticsOnce(const TStrVec &fieldNames)
{
    if (m_Initialized)
    {
        return;
    ...
    m_FieldStatistics.reserve(fieldNames.size());
