...
std::streamsize CDualThreadStreamBuf::xsputn(const char *s, std::streamsize n)
{
    ...
    if (m_Eof)
    {
        LOG_ERROR("Inconsistency - trying to add data to REST stream buffer after end-of-file");
        return ret;
    }
                                       ...
                                       std::ios_base::openmode which)
{
    ...
    if (off != 0)
    {
        LOG_ERROR("Seeking not supported on REST stream buffer");
        return pos;
    }

    if (way != std::ios_base::cur)
    {
        LOG_ERROR("Seeking from beginning or end not supported on REST stream buffer");
        return pos;
    }
    ...
    {
        LOG_ERROR("Unexpected mode for seek on REST stream buffer: " << which);
    }
