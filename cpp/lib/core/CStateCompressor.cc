                                                    ...
                                                    const std::string &baseId)
{
    ...
    m_FilterSink.index(index, type, baseId);
    return m_OutStream;
}

bool CStateCompressor::streamComplete(CDataAdder::TOStreamP &/*strm*/,
                                      bool /*force*/)
{
...
std::streamsize CStateCompressor::CChunkFilter::write(const char *s, std::streamsize n)
{
    ...
    while (n > 0)
    {
        if (!m_OStream)
        {
            const std::string &currentDocId = m_Adder.makeCurrentDocId(m_BaseId,
                                                                       m_CurrentDocNum);
            LOG_TRACE("Add streamed: " << m_Index << ", " << m_Type << ", " << currentDocId);

            m_OStream = m_Adder.addStreamed(m_Index, m_Type, currentDocId);
            if (!m_OStream)
            {
