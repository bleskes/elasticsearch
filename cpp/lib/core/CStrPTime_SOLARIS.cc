...
// On Solaris, strptime() doesn't support the %z time format,
// which stores an explicit offset from GMT.  To make matters
// worse, on Solaris struct tm doesn't have any members for
// GMT offset that we could set, and strptime() zeroes the
// struct tm it's passed, so we can't even call it multiple
// times to parse different bits of the date.
//
// Arguably even worse still, on Linux strptime() accepts %z,
// but doesn't make any changes to the output based on it!
//
...
//
// Also, strptime() on Solaris doesn't accept %s, so we
// handle that too (which is trivial).
//
                          ...
                          struct tm *tm)
{
    ...
    // Check if we've simply been asked to parse an epoch time
    if (adjFormat == "%s")
    {
        if (*buf == '\0')
        {
        ...
        char *excess(0);
        time_t unixTime(::strtol(buf, &excess, 0));
        if (unixTime <= 0 || unixTime == LONG_MAX)
        {
            return 0;
        }
        else
    ...
    if (excess != 0 && zPos != std::string::npos)
    {
        ...
        // Next we need to find out the offset that Solaris/Linux is currently
        ...
        // On Solaris/Linux, the global timezone variable declared in time.h
