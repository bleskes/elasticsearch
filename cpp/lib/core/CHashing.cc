...
CHashing::CUniversalHash::CUInt32VecHash::a(void) const
{
    return m_A;
}
                                ...
                                uint32_t seed)
{
    ...
    switch (remainder)
    {
    case 3: h ^= remainingData[2] << 16;
    case 2: h ^= remainingData[1] << 8;
    case 1: h ^= remainingData[0];
            h *= m;
    default: break;
    };
                                ...
                                uint64_t seed)
{
    ...
    switch (remainder)
    {
    case 7: h ^= uint64_t(remainingData[6]) << 48;
    case 6: h ^= uint64_t(remainingData[5]) << 40;
    case 5: h ^= uint64_t(remainingData[4]) << 32;
    case 4: h ^= uint64_t(remainingData[3]) << 24;
    case 3: h ^= uint64_t(remainingData[2]) << 16;
    case 2: h ^= uint64_t(remainingData[1]) << 8;
    case 1: h ^= uint64_t(remainingData[0]);
            h *= m;
    default: break;
    };
