...
bool CRuntimeLoadedLibrary::load(const std::string &libraryPath)
{
    ...
    // We specify RTLD_NOW to ensure no nasty surprises with missing symbols
    // later on, and we specify RTLD_LOCAL to avoid hard-to-find errors with
    // dependencies with other dynamically loaded modules
    m_LibraryHandle = ::dlopen(libraryPath.c_str(), RTLD_NOW | RTLD_LOCAL);
    if (m_LibraryHandle == 0)
    {
        LOG_ERROR("Cannot load library " << libraryPath << ": " << ::dlerror());
        return false;
    }
...
intptr_t CRuntimeLoadedLibrary::funcAddr(const std::string &funcName) const
{
    ...
    intptr_t func(reinterpret_cast<intptr_t>(::dlsym(m_LibraryHandle,
                                                     funcName.c_str())));
    if (func == 0)
    {
        LOG_ERROR("Could not find symbol " << funcName <<
                  " in library " << m_LibraryPath << ": " << ::dlerror());
        return 0;
    }
