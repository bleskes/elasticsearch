...
const std::string CDelimiter::DEFAULT_DELIMITER("<END>");
...
bool CDelimiter::parse(const CXmlNode &node)
{
    // Parse a delimiter tag that looks something like this:
    //
    // <delimiter
    //     quote="&quot;"
    //     escape="\"
    //     delimiter_followed_by_regex="\w+\s+\d+,\s+\d+\s+\d+:\d+:\d+\s+\w+"
    //     delimiter_followed_by_time="true"><![CDATA[
    // ]]></delimiter>
    //
    // The attributes are optional

    const std::string &primaryDelimiterStr = node.value();
    if (primaryDelimiterStr.empty())
    {
        LOG_ERROR("Delimiter is empty");
        return false;
    }

    CRegex primaryDelimiter;
    if (primaryDelimiter.init(primaryDelimiterStr) == false)
    {
        LOG_ERROR("Cannot initialise delimiter regex: " << primaryDelimiterStr);
        return false;
    }

    bool delimiterFollowedByRegex(false);
    bool delimiterFollowedByTime(false);
    std::string followingRegexStr;
    CRegex followingRegex;
    if (node.attribute("delimiter_followed_by_regex", followingRegexStr) == true)
    {
        if (followingRegex.init(followingRegexStr) == false)
        {
            LOG_ERROR("Cannot initialise following regex: " << followingRegexStr);
            return false;
        }

        delimiterFollowedByRegex = true;

        std::string delimiterFollowedByTimeStr;
        if (node.attribute("delimiter_followed_by_time", delimiterFollowedByTimeStr) == true)
        {
            // If present, value must be convertible to boolean
            if (CStringUtils::stringToType(delimiterFollowedByTimeStr,
                                           delimiterFollowedByTime) == false)
            {
                LOG_ERROR("Cannot convert '" << delimiterFollowedByTimeStr << "' to boolean");
                return false;
            }
        }
        else
        {
            // If this attribute isn't present, default to true
            delimiterFollowedByTime = true;
        }
    }
    ...
    std::string quoteStr;
    if (node.attribute("quote", quoteStr) == true)
    {
        if (quoteStr.length() != 1)
        {
            LOG_ERROR("Quote character must be a single character, not '" << quoteStr << "'");
            return false;
        }

        std::string escapeStr;
        if (node.attribute("escape", escapeStr) == true)
        {
            if (escapeStr.length() > 1)
            {
                LOG_ERROR("Escape character must be a single character, not '" << escapeStr << "'");
                return false;
            }
        ...
        {
            // If a quote character was specified, but no escape character,
            // default the escape character to a backslash
            escape = '\\';
        }
    ...
    m_Delimiter = primaryDelimiter;
    m_Valid = true;
    m_FollowingRegex = followingRegex;
    m_HaveFollowingRegex = delimiterFollowedByRegex;
    m_WaiveFollowingRegexAfterTime = delimiterFollowedByTime;
    m_Quote = quote;
    m_Escape = escape;
...
CXmlNode CDelimiter::toXmlNode(void) const
{
    return this->toXmlNode("delimiter");
}

CXmlNode CDelimiter::toXmlNode(const std::string &nodeName) const
{
    CXmlNode node(nodeName);

    if (m_Valid)
    {
        node.value(m_Delimiter.str());

        if (m_Quote != '\0')
        {
            node.attribute("quote", m_Quote, true);

            if (m_Escape != '\0')
            {
                node.attribute("escape", m_Escape, true);
            }
            else
            {
                node.attribute("quote", "", true);
            }
        }

        if (m_HaveFollowingRegex)
        {
            node.attribute("delimiter_followed_by_regex",
                           m_FollowingRegex.str(),
                           true);
        }

        if (m_WaiveFollowingRegexAfterTime)
        {
            node.attribute("delimiter_followed_by_time",
                           m_WaiveFollowingRegexAfterTime,
                           true);
        }
    }
                          ...
                          std::string &remainder) const
{
    ...
    for (;;)
    {
        ...
        if (this->isFollowingTextAcceptable(searchPos,
                                            str,
                                            timePassed))
        {
