...
void CStringUtils::trim(const std::string &toTrim,
                        std::string &str)
{
...
size_t CStringUtils::replace(const std::string &from,
                             const std::string &to,
                             std::string &str)
{
    if (from == to)
...
void CStringUtils::escape(char escape,
...
std::string CStringUtils::typeToStringPrecise(double d,
                                              CIEEE754::EPrecision precision)
{
    ...
    case CIEEE754::E_HalfPrecision:
        ret = ::fabs(d) < 1.0 && d != 0.0 ?
              ::sprintf(buf, "%.2e", CIEEE754::round(d, CIEEE754::E_HalfPrecision)) :
              ::sprintf(buf, "%.3g", CIEEE754::round(d, CIEEE754::E_HalfPrecision));
        break;

    case CIEEE754::E_SinglePrecision:
        ret = ::fabs(d) < 1.0 && d != 0.0 ?
              ::sprintf(buf, "%.6e", CIEEE754::round(d, CIEEE754::E_SinglePrecision)) :
              ::sprintf(buf, "%.7g", CIEEE754::round(d, CIEEE754::E_SinglePrecision));
        break;
...
bool CStringUtils::_stringToType(bool silent,
                                 ...
                                 char &c)
{
    if (str.length() != 1)
    {
        if (!silent)
        {
            LOG_ERROR("Unable to convert string '" << str << "' to char: " <<
                      (str.empty() ? "too short" : "too long"));
        }
        return false;
    }
