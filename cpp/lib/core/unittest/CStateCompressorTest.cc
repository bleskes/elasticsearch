...
void CStateCompressorTest::testFile(void)
{
    // Load testfile, compress it then check it
    ...
    }
    {
        ...
        ssd << strm->rdbuf();
        std::string decompressed = ssd.str();
        std::ifstream file("testfiles/rawdata");
        std::ostringstream ssr;
        ssr << file.rdbuf();
        std::string reference = ssr.str();
        CPPUNIT_ASSERT_EQUAL(reference, decompressed);
    }
}
