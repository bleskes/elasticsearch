...
struct SHash
{
    ...
    std::size_t operator()(const SFooWithMemoryUsage &foo) const { return foo.s_Key; }
...
class CBase
{
    public:
        ...
        virtual ~CBase()
        {
            LOG_TRACE(core::CContainerPrinter::print(m_Fixed));
        }
...
class CTrackingAllocator
{
    ...
    public:
        inline CTrackingAllocator(void)
        {
        }

        inline CTrackingAllocator(const CTrackingAllocator &)
        {
        }
        ...
        inline bool operator==(const CTrackingAllocator &) const
        {
            return true;
        }
...
void CMemoryUsageTest::testUsage(void)
{
...

    {
        ...
        LOG_DEBUG("dynamicSize(foosWithMemory) = " << core::CMemory::dynamicSize(foosWithMemory));
...

    {
        ...
        LOG_DEBUG("expected dynamic size = " <<   core::CMemory::dynamicSize(bars2)
    ...
    }
    {
        ...
        LOG_DEBUG("expected dynamic size = " <<   sizeof(SBar)
    ...
    }
    {
        ...
        LOG_DEBUG("expected dynamic size = " <<   variables.capacity() * sizeof(std::size_t)
                                                + sizeof(a)
                                                + core::CMemory::dynamicSize(a)
                                                + sizeof(b)
    ...
    }
    {
        ...
        CPPUNIT_ASSERT(core::CMemory::dynamicSize(*base) < core::CMemory::dynamicSize(*derived));
...
void CMemoryUsageTest::testDebug(void)
{
    ...
    // Check that we can get debug info out of classes with vectors of varying size
    {
        ...
        for (unsigned i = 0; i < 9; ++i)
        {
            ...
            LOG_TRACE("SFooWithMemoryUsage usage: " << sbarVectorDebug.s_State.back().memoryUsage());
        }
    ...
    }
    {
        ...
        std::ostringstream ss;
        memoryUsage.print(ss);
        LOG_TRACE("TBarVecPtr usage: " << core::CMemory::dynamicSize(t) << ", debug: " <<
            memoryUsage.usage());
        LOG_TRACE(ss.str());
    ...
    }
    {
        ...
        t.push_back(TFeatureBarVecPtrPr(E_IndividualHighMeanByPerson,
                                        vec));
        TBarVecPtr vec2(new TBarVec());
        vec2->push_back(SBar(22));
        vec2->push_back(SBar(33));
        t.push_back(TFeatureBarVecPtrPr(E_IndividualCountByBucketAndPerson,
                                        vec));

        t.push_back(TFeatureBarVecPtrPr(E_IndividualHighCountsByBucketAndPerson, TBarVecPtr()));
        core::CMemoryUsage memoryUsage;
        memoryUsage.setName("test", 0);
...
void CMemoryUsageTest::testDynamicSizeAlwaysZero(void)
{
    ...
    // http://www.boost.org/doc/libs/1_62_0/libs/type_traits/doc/html/boost_typetraits/reference/is_pod.html
    // .)
#ifdef BOOST_IS_POD
...
void CMemoryUsageTest::testStringBehaviour(void)
{
    ...
    LOG_INFO("String that was copied to another then assigned an empty string "
             "has data at " << static_cast<const void *>(something2.data()) <<
             " length " << something2.length() << " and capacity " <<
             something2.capacity());
...
#if (defined(Linux) || defined(SOLARIS)) && (!defined(_GLIBCXX_USE_CXX11_ABI) || _GLIBCXX_USE_CXX11_ABI == 0)
...
void CMemoryUsageTest::testSharedPointer(void)
{
    ...
    std::size_t expectedSize = vec1.capacity() * sizeof(TIntVecPtr) +
                               vec2.capacity() * sizeof(TIntVecPtr) +
                               3 * sizeof(TIntVec) +
                               (vec1[0]->capacity() + vec1[1]->capacity() +
                                vec1[3]->capacity()) * sizeof(int);

    LOG_DEBUG("Expected: " << expectedSize << ", actual: " << (core::CMemory::dynamicSize(vec1) +
    ...
    svec1.push_back(TStrPtr(new std::string("Here is some more string data, a little longer than the previous one")));
    ...
    long stringSizeBefore = core::CMemory::dynamicSize(svec1) + core::CMemory::dynamicSize(svec2);

    svec2[0] = svec1[2];
    svec2[1] = svec1[0];
    svec2[2] = svec1[1];

    long stringSizeAfter = core::CMemory::dynamicSize(svec1) + core::CMemory::dynamicSize(svec2);

    CPPUNIT_ASSERT_EQUAL(core::CMemory::dynamicSize(svec1), core::CMemory::dynamicSize(svec2));
...
CppUnit::Test *CMemoryUsageTest::suite(void)
{
    ...
    suiteOfTests->addTest( new CppUnit::TestCaller<CMemoryUsageTest>(
                                   "CMemoryUsageTest::testUsage",
                                   &CMemoryUsageTest::testUsage) );

    suiteOfTests->addTest( new CppUnit::TestCaller<CMemoryUsageTest>(
                                   "CMemoryUsageTest::testDebug",
                                   &CMemoryUsageTest::testDebug) );
    ...
    suiteOfTests->addTest( new CppUnit::TestCaller<CMemoryUsageTest>(
                                   "CMemoryUsageTest::testCompress",
                                   &CMemoryUsageTest::testCompress) );

    suiteOfTests->addTest( new CppUnit::TestCaller<CMemoryUsageTest>(
                                   "CMemoryUsageTest::testStringBehaviour",
                                   &CMemoryUsageTest::testStringBehaviour ) );

    suiteOfTests->addTest( new CppUnit::TestCaller<CMemoryUsageTest>(
                                   "CMemoryUsageTest::testStringMemory",
                                   &CMemoryUsageTest::testStringMemory) );

    suiteOfTests->addTest( new CppUnit::TestCaller<CMemoryUsageTest>(
                                   "CMemoryUsageTest::testStringClear",
                                   &CMemoryUsageTest::testStringClear) );

    suiteOfTests->addTest( new CppUnit::TestCaller<CMemoryUsageTest>(
                                   "CMemoryUsageTest::testSharedPointer",
                                   &CMemoryUsageTest::testSharedPointer) );
