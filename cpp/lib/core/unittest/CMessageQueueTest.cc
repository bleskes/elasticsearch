...
void CMessageQueueTest::testTiming(void)
{
    ...
    // it will be too fast to time on a modern computer).  The choice of 29ms
    // is because the resolution of nanosleep() is quite poor on Solaris, where
    // anything less than 20ms gets rounded to 20ms, but 20ms gets rounded to
    // 30ms!  Mac OS X and Linux are more accurate.  On Windows, the system
    // clock ticks 64 times per second, so all sleeps are multiples of 15.625
    // milliseconds.
