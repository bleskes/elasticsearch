                           ...
                           TStrVec &clauseTokens)
{
    try
    {
        ...
        desc.add_options()
            ...
            ("modeldebugconfig", boost::program_options::value<std::string>(),
                        "Optional model debug config file")
            ...
            ("batchspan", boost::program_options::value<core_t::TTime>(),
                        "Optional interval into which to batch seasonal data (in seconds) - default is 3600")
            ("latency", boost::program_options::value<core_t::TTime>(),
                        "Optional maximum delay for out-of-order records (in seconds) - default is 0")
            ("period", boost::program_options::value<std::size_t>(),
                        "Optional repeat period for seasonal data (in multiples of batchspan) - default is aperiodic")
            ...
            ("timefield", boost::program_options::value<std::string>(),
                        "Optional name of the field containing the timestamp - default is '_time'")
            ...
            ("licenseValidation", boost::program_options::value<std::string>(),
                        "Code confirming you have a valid license")
            ("memoryUsage",
            ...
            ("ignoreDowntime",
                        "Optional flag to ignore downtime up to the first data point")
        ...
        if (vm.count("version") > 0)
        {
            std::cerr << "Model State Version " << model::CAnomalyDetector::STATE_VERSION << std::endl
                      << "Quantile State Version " << model::CAnomalyScore::CURRENT_XML_VERSION << std::endl
        ...
        if (vm.count("modeldebugconfig") > 0)
        {
            modelDebugConfigFile = vm["modeldebugconfig"].as<std::string>();
        }
        if (vm.count("jobid") > 0)
        {
        ...
        if (vm.count("batchspan") > 0)
        {
            batchSpan = vm["batchspan"].as<core_t::TTime>();
        }
        ...
        {
            period = vm["period"].as<std::size_t>();
        }
        ...
        if (vm.count("licenseValidation") > 0)
        {
            license = vm["licenseValidation"].as<std::string>();
        }
