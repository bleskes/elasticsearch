...
struct MODEL_EXPORT SResultSpec
{
    ...
    TStrPtr s_PartitionFieldName;
    //! The value of the partitioning field.
    TStrPtr s_PartitionFieldValue;
    ...
    TStrPtr s_PersonFieldName;
    //! The value of the person field if applicable or an empty string
    //! otherwise
    TStrPtr s_PersonFieldValue;
    //! The name of the field identifying the metric value if this is
    //! metric analysis and an empty string otherwise.
    TStrPtr s_ValueFieldName;
    //! The optional function. Only leaf nodes have populated functions.
    TStrPtr s_FunctionName;
    //! The "by" field name.
    TStrPtr s_ByFieldName;
    //! The function identifier.
    function_t::EFunction s_Function;
};

//! \brief The probability for a detector at a result node.
struct SDetectorProbability
{
    //! Default constructor, to be used to persist restoration only!
    SDetectorProbability(void);
    SDetectorProbability(int detector, int aggregationStyle, double probability);

    //! Persist the probability by passing information to \p inserter.
    void acceptPersistInserter(core::CStatePersistInserter &inserter) const;
    ...
    int s_AggregationStyle;
    //! The probability for this detector at the node.
...
{
    typedef SDetectorProbability TDetectorProbability;
    typedef std::vector<SDetectorProbability> TDetectorProbabilityVec;
    ...
    typedef TNodeCPtrVec::const_iterator TNodeCPtrVecCItr;
    typedef boost::unordered_map<TNodeCPtr, std::size_t> TNodePtrSizeUMap;
    typedef TNodePtrSizeUMap::const_iterator TNodePtrSizeUMapCItr;
    typedef TNodePtrSizeUMap::iterator TNodePtrSizeUMapItr;
    typedef boost::unordered_map<std::size_t, TNodeCPtr> TSizeNodePtrUMap;
    typedef TSizeNodePtrUMap::const_iterator TSizeNodePtrUMapCItr;
    typedef TSizeNodePtrUMap::iterator TSizeNodePtrUMapUMapItr;
    ...
    //! The probabilities for each detector.
    mutable TDetectorProbabilityVec s_DetectorProbabilities;
...
class MODEL_EXPORT CHierarchicalResults
{
    public:
        ...
        typedef hierarchical_results_detail::TStrPtr TStrPtr;
        typedef hierarchical_results_detail::TStrPtrStrPtrPr TStrPtrStrPtrPr;
        typedef hierarchical_results_detail::TStrPtrStrPtrPrDoublePr TStrPtrStrPtrPrDoublePr;
        typedef hierarchical_results_detail::TStrPtrStrPtrPrDoublePrVec TStrPtrStrPtrPrDoublePrVec;
        ...
        typedef TNodeDeque::iterator               TNodeDequeItr;
        typedef TNodeDeque::const_iterator         TNodeDequeCItr;
        typedef TNodeDeque::const_reverse_iterator TNodeDequeCRItr;
        typedef std::map<TStrPtrStrPtrPr, TNode, maths::COrderings::SLexicographicalCompare> TStrPtrStrPtrPrNodeMap;
        typedef TStrPtrStrPtrPrNodeMap::iterator TStrPtrStrPtrPrNodeMapItr;
        typedef TStrPtrStrPtrPrNodeMap::const_iterator TStrPtrStrPtrPrNodeMapCItr;
        typedef TStrPtrStrPtrPrNodeMap::const_reverse_iterator TStrPtrStrPtrPrNodeMapCRItr;
        typedef std::map<TStrPtr, TNode, maths::COrderings::SLess> TStrPtrNodeMap;
        typedef TStrPtrNodeMap::const_iterator TStrPtrNodeMapCItr;
    ...
    public:
        ...
        const TNode *influencer(TStrPtr influencerName,
    ...
    private:
        ...
        TNode &newPivot(TStrPtrStrPtrPr key);

        //! Create or retrieve a pivot root node for the \p key.
        TNode &newPivotRoot(TStrPtr key);
    ...
    private:
        ...
        TStrPtrStrPtrPrNodeMap m_PivotNodes;

        //! Pivot root nodes.
        TStrPtrNodeMap m_PivotRootNodes;
...
class MODEL_EXPORT CHierarchicalResultsVisitor
{
    public:
        ...
        typedef TNode::TDetectorProbability TDetectorProbability;
        typedef TNode::TDetectorProbabilityVec TDetectorProbabilityVec;
    ...
    protected:
        ...
        //! Check if the node is a named person.  The logic used for Splunk
        //! results is a bit different to the logic used during aggregation.
        static bool isPerson(const TNode &node, bool isForSplunkResults);
