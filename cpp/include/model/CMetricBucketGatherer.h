...
class MODEL_EXPORT CMetricBucketGatherer : public CBucketGatherer
{
    ...
    private:
        ...
        //!   -# For Splunk with pre-summarisation, the name of the field
        //!      holding the count followed by the field name(s) of the
        //!      field(s) which hold the statistics themselves, which must
        //!      (for those that are present) be ordered mean, min, max, sum.
