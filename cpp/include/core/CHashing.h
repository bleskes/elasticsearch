...
class CORE_EXPORT CHashing : private CNonInstantiatable
{
    public:
        ...
        static uint32_t murmurHash32(const void *key,
                                     int length,
                                     uint32_t seed);
        ...
        //! and check alignment.
        static uint32_t safeMurmurHash32(const void *key,
                                         int length,
                                         uint32_t seed);
        ...
        static uint64_t murmurHash64(const void *key,
                                     int length,
                                     uint64_t seed);
        ...
        //! the object, which it would if we tried to mix the two approaches
        //! and check alignment.
        static uint64_t safeMurmurHash64(const void *key,
                                         int length,
                                         uint64_t seed);
        ...
        class CORE_EXPORT CMurmurHash2String : public std::unary_function<std::string, std::size_t>
        {
            public:
                ...
                typedef boost::shared_ptr<const std::string> TStrCPtr;
            ...
            public:
                ...
                std::size_t operator()(const TStrCPtr &key) const
                {
                    if (key)
                    {
