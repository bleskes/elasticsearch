...
//! Helper function-object to sort an array of set iterators.
struct SSetItrLess
{
    template<typename T>
...
//! Helper function-object to sort an array of map iterators.
struct SMapItrLess
{
    template<typename T>
    inline bool operator()(const T &lhs,
...
class CORE_EXPORT CPersistUtils
{
    ...
    public:
                                    ...
                                    const char delimiter = DELIMITER)
        {
            ...
            CItr begin = collection.begin();
            CItr end = collection.end();
        ...
        //! Restore a vector from a string created by persist.
        //!
        ...
        //! Restore a boost::array from a string created by persist.
        //!
        //! \param[in] state The string description of the
        //! collection.
        //! \param[in] stringFunc The function used to persist
                               ...
                               ITR inserter)
        {
            ...
            while (lastDelimPos != std::string::npos)
            {
                ...
                if (delimPos == std::string::npos)
                {
                    token.assign(state,
                                 lastDelimPos + 1,
                                 state.length() - lastDelimPos);
                }
                else
                {
                    token.assign(state,
                                 lastDelimPos + 1,
                                 delimPos - lastDelimPos - 1);
                }
    ...
    public:
                             ...
                             CStatePersistInserter &inserter)
        {
            ...
            else
            {
                ...
                std::sort(iterators.begin(), iterators.end(), SSetItrLess());
                             ...
                             CStatePersistInserter &inserter)
        {
            ...
            std::sort(iterators.begin(), iterators.end(), SMapItrLess());
            doInsert(tag, iterators, inserter, boost::false_type(), boost::true_type());
        }
            ...
            bool operator()(T &container, CStateRestoreTraverser &traverser)
            {
                ...
                do
                {
                    if (traverser.name() == SIZE_TAG)
                    {
                        ...
                        {
                            LOG_ERROR("Failed to determine size: " << traverser.value());
                            return false;
                        }
