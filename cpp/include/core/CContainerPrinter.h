...
class CLeafPrinter<false_>
{
    public:
        ...
        {
            typedef typename boost::is_arithmetic<T>::type IsArithmetic;

            // Use CStringUtils if possible: it's much faster but
            // only supports fundamental types.
            return print_(value, IsArithmetic());
        }

    private:
        //! Fast CStringUtil implementation.
        template<typename T>
        inline static std::string print_(const T &value, true_/*is arithmetic*/)
        {
...
class CORE_EXPORT CContainerPrinter : private CNonInstantiatable
{
    ...
    public:
        ...
        //! Print a boost::icl::interval_set for debug.
        template<typename T>
        static std::string print(const boost::icl::interval_set<T> &intervals)
        {
            std::ostringstream result;
            result << intervals;
            return result.str();
