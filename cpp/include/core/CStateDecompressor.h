...
class CORE_EXPORT CStateDecompressor : public CDataSearcher
{
    public:
        ...
        static const std::string END_OF_STREAM;
        ...
        class CDechunkFilter
        {
            ...
            private:
                ...
                //! (CDataSearcher::PERSISTENCE_ITERATION_ATTRIBUTE) then continue
                //! reading until the compressed array is found.
                //! If successful the reader member is pointing at the compressed data
            ...
            private:
                ...
                boost::shared_ptr<rapidjson::GenericReadStream> m_InputStreamWrapper;

                //! JSON reader for the downstream stream
                boost::shared_ptr<rapidjson::PullReader> m_Reader;

                //! The current token - this is retained between consecutive calls to Read
                rapidjson::Token m_Token;
