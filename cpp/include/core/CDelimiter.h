...
class CORE_EXPORT CDelimiter
{
    ...
    public:
        ...
        //! Initialise by parsing an XML node
        bool parse(const CXmlNode &node);

        //! Convert to an XML node for output using default name of "delimiter"
        CXmlNode toXmlNode(void) const;

        //! Convert to an XML node for output, specifying the node name
        CXmlNode toXmlNode(const std::string &nodeName) const;
