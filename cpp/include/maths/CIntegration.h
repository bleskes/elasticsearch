...
class MATHS_EXPORT CIntegration
{
    public:
        ...
        typedef boost::function2<bool, double, double &> TLogFunc;
    ...
    public:
        ...
        static bool productGaussLegendre(const F &f, const G &g,
                                         double a, double b,
                                         U &productIntegral,
                                         U &fIntegral,
                                         V &gIntegral)
        ...
        //! As above but fixed to order 3 and using boost::function instead of
        //! templates for the function to work around a Solaris Studio 12.4
        //! compiler bug that manifested itself when building the file
        //! ProbabilityAggregators.cc.  (Making this a non-template function
        //! restricts the amount of inlining the compiler can do which in turn
        //! stops it getting itself into a mess with incorrect optimisations.)
        static bool logGaussLegendreOrder3(const TLogFunc &function,
                                           double a,
                                           double b,
                                           double &result);
        ...
        class CSparseGaussLegendreQuadrature : private core::CNonCopyable
        {
            ...
            public:
                static const CSparseGaussLegendreQuadrature<O, D> &instance(void)
                {
                    if (!ms_Instance)
                    {
                        ...
                        if (!ms_Instance)
                        {
                            ms_Instance.reset(new CSparseGaussLegendreQuadrature());
                        }
                    }
                    return *ms_Instance;
                }
            ...
            private:
                ...
                CSparseGaussLegendreQuadrature(void)
                {
                    ...
                    typedef std::map<TVector, double> TVectorDoubleMap;
                    typedef typename TVectorDoubleMap::const_iterator TVectorDoubleMapCItr;
                    ...
                    for (TVectorDoubleMapCItr itr = ordered.begin();
                         itr != ordered.end();
                         ++itr)
                    {
                        m_Weights.push_back(itr->second);
                        m_Points.push_back(itr->first);
                    }
                }

                static TPtr ms_Instance;
        ...
        static bool sparseGaussLegendre(const F &function,
                                        ...
                                        T &result)
        {
            typedef CSparseGaussLegendreQuadrature<ORDER, DIMENSION> TSparseQuadrature;
            typedef typename TSparseQuadrature::TVector TVector;
            typedef typename TSparseQuadrature::TVectorVec TVectorVec;
...
typename CIntegration::CSparseGaussLegendreQuadrature<O, D>::TPtr CIntegration::CSparseGaussLegendreQuadrature<O, D>::ms_Instance;
