...
class CRandomProjectionClusterer
{
    ...
    public:
        virtual ~CRandomProjectionClusterer(void) {}
    ...
    protected:
        ...
        typedef boost::array<TVector, N> TVectorArray;
        typedef std::vector<TVectorArray> TVectorArrayVec;
    ...
    protected:
        ...
        {
            typedef boost::array<TDoubleVec, N> TDoubleVecArray;
...
{
    public:
        typedef typename CRandomProjectionClusterer<N>::TDoubleVec TDoubleVec;
        typedef typename CRandomProjectionClusterer<N>::TSizeVec TSizeVec;
        typedef typename CRandomProjectionClusterer<N>::TVector TVector;
        typedef typename CRandomProjectionClusterer<N>::TVectorVec TVectorVec;
        typedef std::vector<TDoubleVec> TDoubleVecVec;
        typedef std::vector<TSizeVec> TSizeVecVec;
        typedef boost::unordered_set<std::size_t> TSizeUSet;
        typedef TSizeUSet::const_iterator TSizeUSetCItr;
        typedef CVectorNx1<double, N> TVectorNx1;
        typedef typename SEigenVectorNx1<N>::Type TEigenVectorNx1;
        typedef std::vector<TVectorNx1> TVectorNx1Vec;
        typedef std::vector<TVectorNx1Vec> TVectorNx1VecVec;
        typedef CSymmetricMatrixNxN<double, N> TSymmetricMatrixNxN;
        typedef Eigen::JacobiSVD<typename SEigenMatrixNxN<N>::Type> TSvdNxN;
        typedef std::vector<TSvdNxN> TSvdNxNVec;
        typedef std::vector<TSvdNxNVec> TSvdNxNVecVec;
        ...
        typedef std::vector<TMeanAccumulatorVec> TMeanAccumulatorVecVec;
    ...
    public:
        ...
        virtual ~CRandomProjectionClustererBatch(void) {}
    ...
    protected:
        ...
        void clusterProjections(CLUSTERER clusterer,
                                ...
                                TSizeUSet &I) const
        {
            typedef boost::reference_wrapper<const TVectorNx1> TVectorNx1CRef;
            typedef boost::unordered_map<TVectorNx1CRef,
                                         std::size_t,
                                         typename TVectorNx1::CHash,
                                         typename TVectorNx1::CEqual> TVectorNx1CRefSizeUMap;
            typedef typename CLUSTERER::TClusterVec TClusterVec;
            typedef CBasicStatistics::SSampleCovariances<double, N> TSampleCovariancesNxN;
            ...
            for (std::size_t i = 0u; i < b; ++i)
            {
                ...
                for (std::size_t j = 0u; j < clusters.size(); ++j)
                {
                    ...
                    TSvdNxN Cij(CBasicStatistics::covariances(covariances).toEigen(),
                    ...
                    for (std::size_t k = 0u; k < nij; ++k)
                    {
                        ...
                        if (I.count(index) == 0)
                        {
                            TEigenVectorNx1 x = (points[k] - mij).toEigen();
        ...
        {
            typedef boost::unordered_map<TVector, std::size_t, typename TVector::CHash> TVectorSizeUMap;
            ...
            S.reserve(I.size());
            TVector concat(b * N);
            for (TSizeUSetCItr i = I.begin(); i != I.end(); ++i)
            {
            ...
            for (std::size_t i = 0u; i < b; ++i)
            {
                ...
                for (std::size_t c = 0u; c < nci; ++c)
                {
                    ...
                    for (std::size_t j = 0u; j < h; ++j)
                    {
                        ...
                        {
                            TEigenVectorNx1 x = (X[H[j][k]] - Mi[c]).toEigen();
                                   ...
                                   TSizeVecVec &result) const
        {
            ...
            typedef CNaturalBreaksClassifier::TDoubleTuple TDoubleTuple;
            typedef CNaturalBreaksClassifier::TDoubleTupleVec TDoubleTupleVec;
...
{
    public:
        typedef CXMeans<CVectorNx1<double, N>, COST> TClusterer;
        typedef typename TClusterer::TClusterVec TClusterVec;
        typedef CVectorNx1<double, N> TVectorNx1;
        typedef std::vector<TVectorNx1> TVectorNx1Vec;
...
{
    public:
        typedef CKMeansFast<CVectorNx1<double, N> > TClusterer;
        typedef typename TClusterer::TClusterVec TClusterVec;
        typedef CVectorNx1<double, N> TVectorNx1;
        typedef std::vector<TVectorNx1> TVectorNx1Vec;
...
//! \brief Implements random projection clustering for a stream
//! of data points.
template<std::size_t N>
class CRandomProjectionClustererOnline : public CRandomProjectionClusterer<N>
{
    public:
        virtual ~CRandomProjectionClustererOnline(void)
        {
        }
};
