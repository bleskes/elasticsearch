...
class MATHS_EXPORT CSplineTypes
{
    public:
        ...
        //! -# Mean value cubic spline. Uses a constraint on the
        //!    mean value in the interval in place of a continuity
        //!    constraint on the curvature at the knot points.
        enum EType
        {
            E_Linear,
            E_Cubic,
...
class CSpline : public CSplineTypes
{
    ...
    public:
        ...
        void clear(void)
        {
            ...
            this->knots().swap(noKnots);
            TNonConstValues noValues;
            this->values().swap(noValues);
            TNonConstCurvatures noCurvatures;
            this->curvatures().swap(noCurvatures);
        }
                          ...
                          TDoubleVec *d = 0) const
        {
            ...
            case E_CubicMean:
                ...
                for (std::size_t i = 1u; i < this->knots().size(); ++i)
                {
                    if (a) a->push_back(0.0);
                    if (b) b->push_back(0.0);
                    if (c) c->push_back(0.0);
                    if (d) d->push_back(0.0);
                }
                break;
            }
        }
                         ...
                         EBoundaryCondition boundary)
        {
            ...
            oldKnots.swap(this->knots());
            oldValues.swap(this->values());
            this->knots().assign(knots.begin(), knots.end());
            ...
            for (std::size_t i = 1u; i <= n; ++i)
            {
                ...
                if (i - i_ > 1)
                {
                    ...
                    this->values()[i_] = CBasicStatistics::mean(value);
                }
                if (++last != i_)
                {
                    this->knots()[last] = this->knots()[i_];
                    this->values()[last] = this->values()[i_];
                }
            }
            this->knots().erase(this->knots().begin() + last + 1, this->knots().end());
            this->values().erase(this->values().begin() + last + 1, this->values().end());
            n = this->knots().size();
            ...
            if (this->knots().size() < 2)
            {
                ...
                this->knots().swap(oldKnots);
                this->values().swap(oldValues);
                return false;
            }
            ...
            case E_Cubic:
            {
                ...
                case E_Periodic:
                    ...
                    c.push_back(h - 1.0);
                    this->curvatures().push_back(6.0 * (  (this->values()[1] - this->values()[0]) / h
                ...
                for (std::size_t i = 1u; i + 1 < n; ++i)
                {
                    ...
                    b.push_back(2.0 * (h + h_));
                    c.push_back(h);
                    this->curvatures().push_back(6.0 * (  (this->values()[i+1] - this->values()[i]) / h
                ...
                case E_Natural:
                    ...
                    b.push_back(1.0);
                    this->curvatures().push_back(0.0);
                    if (!spline_detail::solveTridiagonal(a, b, c, this->curvatures()))
                    {
                ...
                case E_Periodic:
                    {
                        ...
                        v[1] = 1.0;
                        v[n-2] = h_;
                        this->curvatures().push_back(6.0 * (  (this->values()[1] - this->values()[n-1]) / h
                                                            - (this->values()[n-1] - this->values()[n-2]) / h_));
                        if (!spline_detail::solvePeturbedTridiagonal(a, b, c, u, v, this->curvatures()))
                        {
                            LOG_ERROR("Failed to calculate curvatures");
                            return false;
                        }
        ...
        void describe(const std::string &indent,
        ...
        //! Get the values at the knot points of the spline.
        inline TNonConstValues &values(void)
        {
            return boost::unwrap_ref(m_Values);
        }
        ...
        //! Get the curvatures at the knot points of the spline.
        inline TCurvatures &curvatures(void)
        {
            return boost::unwrap_ref(m_Curvatures);
        }
