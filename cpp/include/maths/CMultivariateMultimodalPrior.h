...
MATHS_EXPORT
void print(const TModeVec &modes,
...
class CMultivariateMultimodalPrior : public CMultivariatePrior
{
    ...
    public:
        ...
        virtual void decayRate(double value)
        {
            ...
            for (std::size_t i = 0u; i < m_Modes.size(); ++i)
            {
                m_Modes[i].s_Prior->decayRate(this->decayRate());
            }
                                  ...
                                  const TDouble10Vec4Vec1Vec &weights)
        {
            ...
            for (std::size_t i = 0u; i < m_Modes.size(); ++i)
            {
                m_Modes[i].s_Prior->adjustOffset(weightStyles, samples, weights);
            }
        }
                                ...
                                const TDouble10Vec4Vec1Vec &weights)
        {
            ...
            TDouble10Vec4Vec1Vec weights_(1);
            TSizeDoublePr2Vec clusters;

            std::size_t seasonalVarianceScale = static_cast<std::size_t>(
                                                      std::find(weightStyles.begin(),
                                                                weightStyles.end(),
                                                                maths_t::E_SampleSeasonalVarianceScaleWeight)
                                                    - weightStyles.begin());
            std::size_t count = static_cast<std::size_t>(  std::find(weightStyles.begin(),
                                                                     weightStyles.end(),
                                                                     maths_t::E_SampleCountWeight)
                                                         - weightStyles.begin());
            if (count == weightStyles.size())
            {
            ...
            try
            {
                ...
                for (std::size_t i = 0u; i < samples.size(); ++i)
                {
                    ...
                    double m = this->smallest(maths_t::count(N, weightStyles_, weights[i]));
                    double n = this->smallest(maths_t::countForUpdate(N, weightStyles_, weights[i]));
                    if (hasSeasonalScale)
                    {
                    ...
                    clusters.clear();
                    m_Clusterer->add(x, clusters, m);

                    sample[0] = x.template toVector<TDouble10Vec>();
                    weights_[0] = weights[i];
                    if (seasonalVarianceScale < weights_[0].size())
                    {
                        weights_[0][seasonalVarianceScale].assign(N, 1.0);
                    }
                    if (count == weights_[0].size())
                    {
                        weights_[0].push_back(TDouble10Vec(N, 1.0));
                    }
                    ...
                    {
                        TModeVecItr k = std::find_if(m_Modes.begin(),
                                                     m_Modes.end(),
                        ...
                        m = clusters[j].second;
                        if (!CMathsFuncs::isFinite(m))
                        {
                            LOG_ERROR("Bad cluster weight " << m);
                            continue;
                        }
                        weights_[0][count].assign(N, m);
                        k->s_Prior->addSamples(weightStyles, sample, weights_);
                    }
                }
            }
                                                       ...
                                                       const TSizeDoublePr10Vec &condition) const
        {
            ...
            {
                weights[i] = ::exp(weights[i] - maxWeight[0]);
                Z += weights[i];
            }
            for (std::size_t i = 0u; i < weights.size(); ++i)
            {
            ...
            return std::make_pair(TUnivariatePriorPtr(new CMultimodalPrior(this->dataType(), modes)),
                                            ...
                                            const TSizeDoublePr10Vec &condition) const
        {
            ...
            for (std::size_t i = 0u; i < m_Modes.size(); ++i)
            {
                TPriorPtrDoublePr prior(m_Modes[i].s_Prior->bivariate(marginalize, condition));
                if (prior.first == 0)
                {
            ...
            return std::make_pair(TPriorPtr(new CMultivariateMultimodalPrior<2>(this->dataType(), modes)),
                                  Z > 0.0 ? maxWeight[0] + ::log(Z) : 0.0);
        }
        ...
        virtual TDouble10VecDouble10VecPr marginalLikelihoodSupport(void) const
        {
            if (m_Modes.size() == 0)
            {
                return std::make_pair(TPoint::smallest().template toVector<TDouble10Vec>(),
            ...
            for (std::size_t i = 0u; i < m_Modes.size(); ++i)
            {
                TDouble10VecDouble10VecPr s = m_Modes[i].s_Prior->marginalLikelihoodSupport();
                lower = min(lower, TPoint(s.first));
                upper = max(upper, TPoint(s.second));
            }

            return std::make_pair(lower.template toVector<TDouble10Vec>(),
                                                    ...
                                                    const TDouble10Vec4Vec &weight) const
        {
            ...
            for (std::size_t i = 0u; i < m_Modes.size(); ++i)
            {
                double w = m_Modes[i].weight();
                const TPriorPtr &prior = m_Modes[i].s_Prior;
                ...
                if (prior->jointLogMarginalLikelihood(TWeights::COUNT_VARIANCE,
                                                      mode,
                                                      weight_,
                                                      likelihood) & maths_t::E_FpAllErrors)
                {
                    continue;
                }
        ...
        virtual void print(const std::string &separator,
    ...
    protected:
        ...
        typedef TModeVec::iterator TModeVecItr;
            ...
            public:
                                ...
                                std::size_t rightSplitIndex) const
                {
                    ...
                    modes.push_back(TMode(leftSplitIndex, m_Prior->m_SeedPrior));
                    {
                        ...
                        for (std::size_t i = 0u; i < samples.size(); ++i)
                        {
                            samples_.push_back(samples[i].template toVector<TDouble10Vec>());
                        }
            ...
            public:
                                ...
                                std::size_t targetIndex) const
                {
                    ...
                    detail::modeMergeCallback(N, m_Prior->m_Modes, m_Prior->m_SeedPrior,
                                              MODE_MERGE_NUMBER_SAMPLES,
                                              leftMergeIndex, rightMergeIndex, targetIndex);
                }
