...
#ifndef INCLUDED_prelert_maths_CMultimodalPriorUtils_h
#define INCLUDED_prelert_maths_CMultimodalPriorUtils_h
...
class MATHS_EXPORT CMultimodalPriorUtils : private core::CNonInstantiatable
{
    public:
                                                                    ...
                                                                    const TDouble4Vec &weights)
        {
            ...
            double x0 = marginalLikelihoodMean(modes);
            ...
            try
            {
                ...
                if (   percentage > 0.0
                    && (   (f20 < 0  && !CSolvers::rightBracket(a, b, fa, fb, f2, maxIterations))
                                       ...
                                       double &result)
        {
            ...
            if (status & maths_t::E_FpFailed)
            {
                LOG_ERROR("Failed to compute likelihood (" << debugWeights(modes) << ")");
            ...
            case maths_t::E_TwoSided:
                {
                    ...
                    double m = marginalLikelihoodMean(modes);

                    double a = boost::numeric::bounds<double>::highest();
                    double b = boost::numeric::bounds<double>::lowest();
                    for (std::size_t i = 0u; i < modes.size(); ++i)
                    {
                        double mode = modes[i].s_Prior->marginalLikelihoodMode();
                        a = std::min(a, mode);
                        b = std::max(b, mode);
                    }
                    ...
                    TDouble4Vec weightUnitSeasonalScale;
                    TDouble1Vec wt(1);
                    ...
                    {
                        double x = samples[i];
                        double seasonalScale = ::sqrt(svi < weights[i].size() ? weights[i][svi] : 1.0);

                        double fx;
                        sample[0] = x;
                        weight[0] = weights[i];
                        weightUnitSeasonalScale = weights[i];
                        if (svi < weightUnitSeasonalScale.size())
                        {
                            weightUnitSeasonalScale[svi] = 1.0;
                        }

                        maths_t::EFloatingPointErrorStatus status =
                                jointLogMarginalLikelihood(modes, weightStyles, sample, weight, fx);
                        if (status & maths_t::E_FpFailed)
                        {
                        ...
                        CPriorUtils::CLogMarginalLikelihood<PRIOR> logLikelihood(prior, weightStyles, weight);

                        CTools::CMixtureProbabilityOfLessLikelySample calculator(modes.size(), x, fx,
                                                                                 m + seasonalScale * (a - m),
                                                                                 m + seasonalScale * (b - m));
                        for (std::size_t j = 0u; j < modes.size(); ++j)
                        {
                            double w = modes[j].weight() / Z;
                            double centre = modes[j].s_Prior->marginalLikelihoodMode(weightStyles, weightUnitSeasonalScale);
                            double spread = ::sqrt(modes[j].s_Prior->marginalLikelihoodVariance(weightStyles, weightUnitSeasonalScale));
                            calculator.addMode(w, centre, spread, m, seasonalScale);
                            double mode = m + seasonalScale * (centre - m);
                            if (x < mode)
                            {
...
#endif // INCLUDED_prelert_maths_CMultimodalPriorUtils_h
