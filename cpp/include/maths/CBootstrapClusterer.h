...
class CBootstrapClusterer
{
        ...
        struct SCutState
        {
            ...
            void mergeAfter(std::size_t n)
            {
                if (s_ToVisit.size() > n)
                {
                    ...
                    std::inplace_merge(s_ToVisit.begin(), s_ToVisit.begin() + n, s_ToVisit.end());
                }
            }
    ...
    protected:
                                    ...
                                    if (edges.insert(std::make_pair(u, v)).second)
                                    {
                                        boost::put(boost::edge_weight,
                                                   ...
                                                   std::min(  m_ChainingFactor
                                                            * (overlaps[l] - m_OverlapThreshold * sum), 1.0));
                                    }
                                }
                            }
                        }
                           ...
                           TPointVecVec &result) const
        {
            ...
            for (std::size_t i = 0u; i < points.size(); ++i)
            {
                ...
                for (std::size_t j = 0u; j < n; ++j)
                {
                    ...
                    if (COrderings::lexicographical_compare(c, n_, cmax, nmax,
            ...
            result.erase(std::remove_if(result.begin(), result.end(), SIsEmpty()), result.end());
        }
        ...
        std::size_t thickets(std::size_t n, const TGraph &graph, TSizeVec &components) const
        {
            ...
            for (std::size_t i = 0u; i < n; ++i)
            {
                ...
                if (this->separate(component, parities))
                {
                    ...
                    LOG_TRACE("parities = " << core::CContainerPrinter::print(parities.begin(),
                                                                              parities.begin() + Vi));
        ...
        bool separate(const TGraph &graph, TBoolVec &result) const
        {
            ...
            {
                std::size_t C = std::max( i * (D - std::min(D, i - 1)),
                                         (i * (V - i)) - std::min( i * (V - i),
                                                                  (V * (V - 1)) / 2 - E));
            ...
            for (std::size_t i = 0u; i < seeds.size(); ++i)
            {
                ...
                double cost;
                if (this->cutSearch(cut.back().first, cut.back().second, graph, threshold, cost, result))
                {
                    return true;
                }
                ...
                if (n > 0)
                {
                    ...
                    newCut.clear();
                    std::set_intersection(cut.begin(), cut.begin() + n,
                                          cut.begin() + n, cut.end(),
                                          std::back_inserter(newCut));
                    cut.swap(newCut);
                }
            }
    ...
    private:
        ...
        void copy(const TGraph &graph,
                  const TSizeVec &mapping,
                  const TSizeVec &inverse,
                  TGraph &result) const
        {
        ...
        bool findNext(const TBoolVec &parities, const TGraph &graph, SCutState &state) const
        {
            ...
            for (std::size_t i = 0u; i < state.s_ToVisit.size(); ++i)
            {
                ...
                bool connected = (this->positiveSubgraphConnected(graph, parities, components) == 1);
        ...
        void visit(std::size_t next,
                   const TGraph &graph,
                   TBoolVec &parities,
                   SCutState &state) const
        {
