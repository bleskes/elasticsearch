...
class MATHS_EXPORT CSampling : private core::CNonInstantiatable
{
    public:
        ...
        {
            public:
                typedef boost::random::mt11213b::result_type result_type;
            ...
            private:
                typedef boost::optional<result_type> TOptionalResultType;
    ...
    public:
        ...
        static void gammaSampleQuantiles(double shape,
                                         double rate,
                                         std::size_t n,
                                         TDoubleVec &result);
        ...
        static void multinomialSampleStable(TDoubleVec probabilities,
                                            std::size_t n,
                                            TSizeVec &sample);
...
static void multivariateNormalSampleDelegate(const CVectorNx1<double, N> &mean,                \
                                             const CSymmetricMatrixNxN<double, N> &covariance, \
                                             std::size_t n,                                    \
                                             std::vector<CVectorNx1<double, N> > &samples);    \
static void multivariateNormalSampleDelegate(CPRNG::CXorOShiro128Plus &rng,                    \
                                             const CVectorNx1<double, N> &mean,                \
                                             const CSymmetricMatrixNxN<double, N> &covariance, \
                                             std::size_t n,                                    \
                                             std::vector<CVectorNx1<double, N> > &samples);    \
static void multivariateNormalSampleDelegate(CPRNG::CXorShift1024Mult &rng,                    \
                                             const CVectorNx1<double, N> &mean,                \
                                             const CSymmetricMatrixNxN<double, N> &covariance, \
                                             std::size_t n,                                    \
                                             std::vector<CVectorNx1<double, N> > &samples)
...
#undef MULTIVARIATE_NORMAL_SAMPLE_DELEGATE
    ...
    public:
        //! Overload of multivariate normal sample for CVectorNx1 and
        //! CSymmetricMatrixNxN.
        template<typename T, std::size_t N>
        static void multivariateNormalSample(const CVectorNx1<T, N> &mean,
                                             const CSymmetricMatrixNxN<T, N> &covariance,
                                             std::size_t n,
                                             std::vector<CVectorNx1<T, N> > &samples)
        {
            multivariateNormalSampleDelegate(mean, covariance, n, samples);
        }

        //! Overload of multivariate normal sample for CVectorNx1 and
        //! CSymmetricMatrixNxN using \p rng
        template<typename T, std::size_t N>
        static void multivariateNormalSample(CPRNG::CXorOShiro128Plus &rng,
                                             ...
                                             std::vector<CVectorNx1<T, N> > &samples)
        {
            multivariateNormalSampleDelegate(rng, mean, covariance, n, samples);
        }
        ...
        static void normalSample(double mean,
                                 double variance,
                                 std::size_t n,
                                 TDoubleVec &result);
        ...
        //! Get \p n uniform integer samples in the range [\p a, \p b).
        static void uniformSample(std::size_t a,
                                  std::size_t b,
                                  std::size_t n,
                                  TSizeVec &result);
        ...
        //! Get a uniform real sample in the range [\p a, \p b).
        static double uniformSample(double a, double b);

        //! Get a uniform real sample in the range [\p a, \p b) using \p rng.
        static double uniformSample(CPRNG::CXorOShiro128Plus &rng,
                                    double a,
                                    double b);

        //! Get a uniform real sample in the range [\p a, \p b) using \p rng.
        static double uniformSample(CPRNG::CXorShift1024Mult &rng,
                                    double a,
                                    double b);

        //! Get \p n uniform real samples in the range [\p a, \p b).
        static void uniformSample(double a,
                                  double b,
                                  std::size_t n,
                                  TDoubleVec &result);
        ...
        static void chiSquaredSample(CPRNG::CXorOShiro128Plus &rng,
                                     double f,
                                     std::size_t n,
                                     TDoubleVec &result);
        ...
        //! Get \p n samples of a \f$\chi^2\f$ random variable with \p f
        //! degrees of freedom using \p rng.
        static void chiSquaredSample(CPRNG::CXorShift1024Mult &rng,
                                     double f,
                                     std::size_t n,
                                     TDoubleVec &result);
