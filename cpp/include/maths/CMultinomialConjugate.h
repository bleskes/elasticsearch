...
//!
//! The Splunk K-V store won't accept strings longer than ~ 512K, in some rare cases
//! where there are lots of category values the length of the persisted categories
//! string can exceed this. No more that MAX_CATEGORIES_PERSIST_SIZE categories
//! should be written in a single string, the values may have to be split across
//! multiple strings.
...
class MATHS_EXPORT CMultinomialConjugate : public CPrior
{
    ...
    public:
        //! To avoid generating very large strings persisted state
        //! strings if there are more then this number of categories
        //! split them across multiple arrays.
        static const std::size_t MAX_CATEGORIES_PERSIST_SIZE;
    ...
    public:
        ...
        //! Max number a categories that can be persisted in a single
        //! string. See MAX_CATEGORIES_PERSIST_SIZE
        std::size_t maxCategoriesPersistSize() const;

        void maxCategoriesPersistSize(std::size_t);
    ...
    private:
        ...
        //! Helper function for persisting large state vectors
        void splitAndPersistedDoubleVec(const std::string &tag,
    ...
    private:
        ...
        //! Max number a categories that can be persisted in a single
        //! string. Defaults to MAX_CATEGORIES_PERSIST_SIZE
        std::size_t m_MaxCategoriesPersistSize;
};
