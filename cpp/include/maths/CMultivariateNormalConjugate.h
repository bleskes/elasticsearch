...
class CMultivariateNormalConjugate : public CMultivariatePrior
{
    ...
    public:
                                                       ...
                                                       const TSizeDoublePr10Vec &condition) const
        {
            ...
            if (this->isNonInformative())
            {
                return TUnivariatePriorPtrDoublePr(TUnivariatePriorPtr(
                               CNormalMeanPrecConjugate::nonInformativePrior(dataType, decayRate).clone()), 0.0);
            }
            ...
            {
                return TUnivariatePriorPtrDoublePr(TUnivariatePriorPtr(
                               new CNormalMeanPrecConjugate(dataType, m1, p, s, c11 * v / 2.0, decayRate)), 0.0);
            }

            TSize10Vec condition_;
            condition_.reserve(condition.size() + 1);
            Eigen::VectorXd xc(condition.size());
            this->unpack(condition, condition_, xc);

            try
            {
                std::size_t n = condition_.size();
                Eigen::VectorXd m2  = projectedVector(condition_, m);
                condition_.push_back(i1[0]);
                Eigen::MatrixXd cp  = projectedMatrix(condition_, c);
                Eigen::VectorXd c12 = cp.topRightCorner(1, n);
                Eigen::JacobiSVD<Eigen::MatrixXd> c22(cp.topLeftCorner(n, n),
                ...
                Eigen::VectorXd c22SolvexcMinusm2 = c22.solve(xc - m2);
                                            ...
                                            const TSizeDoublePr10Vec &condition) const
        {
            ...
            if (this->isNonInformative())
            {
                return TPriorPtrDoublePr(boost::shared_ptr<CMultivariatePrior>(
                               CMultivariateNormalConjugate<2>::nonInformativePrior(dataType, decayRate).clone()), 0.0);
            }
            ...
            {
                return TPriorPtrDoublePr(boost::shared_ptr<CMultivariatePrior>(
                               new CMultivariateNormalConjugate<2>(dataType, m1, p, f, c11, decayRate)), 0.0);
            }
            ...
            try
            {
                ...
                TPoint2 mean(m1.toEigenDynamic() + c12.transpose() * c22SolvexcMinusm2);
                TMatrix2 covariance(c11.toEigenDynamic() - c12.transpose() * c22.solve(c12));
                double weight;
                ...
                return TPriorPtrDoublePr(boost::shared_ptr<CMultivariatePrior>(
                               new CMultivariateNormalConjugate<2>(dataType, mean, p, f, covariance, decayRate)), weight);
            }
            catch (const std::exception &e)
            {
                LOG_ERROR("Failed to get univariate prior: " << e.what());
                                              ...
                                              TDouble10Vec1Vec &samples) const
        {
            ...
            {
                samples.push_back(samples_[i].template toVector<TDouble10Vec>());
            }
        }
        ...
        void randomSamplePrecisionMatrixPrior(std::size_t n, TMatrixVec &result)
        {
            ...
            Eigen::JacobiSVD<TEigenMatrix> precision(m_WishartScaleMatrix.toEigen(),
            ...
            TEigenVector diag = precision.singularValues();
            ...
            TEigenMatrix L = TEigenMatrix::Zero(N, N);
            ...
            for (std::size_t s = 0u; s < n; ++s)
            {
                ...
                result.push_back(TMatrix(L * A * A.transpose() * L.transpose()));
            }
        }
        ...
        TMatrix precision(void) const
        {
            ...
            return TMatrix(result.toEigen().inverse());
        }
