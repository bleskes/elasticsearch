...
class CChecksumImpl<BasicChecksum>
{
    public:
        ...
        //! Checksum of sample central moment accumulator.
        template<typename T, unsigned int N>
        static uint64_t dispatch(uint64_t seed,
                                 const typename CBasicStatistics::SSampleCentralMoments<T, N> &target)
        {
            return core::CHashing::hashCombine(seed, CBasicStatistics::checksum(target));
        }

        //! Checksum of sample central moment accumulator.
        template<typename T, std::size_t N>
        static uint64_t dispatch(uint64_t seed,
                                 const typename CBasicStatistics::SSampleCovariances<T, N> &target)
        {
            return core::CHashing::hashCombine(seed, CBasicStatistics::checksum(target));
        }
...
class CChecksumImpl<ContainerChecksum>
{
    public:
        ...
        //! Handle CBasicStatistics::COrderStatisticsStack which has const_iterator.
        template<typename T, std::size_t N, typename LESS>
        static uint64_t dispatch(uint64_t seed, const CBasicStatistics::COrderStatisticsStack<T, N, LESS> &target)
        {
            return target.checksum(seed);
        }
        ...
        static uint64_t dispatch(uint64_t seed, const CBasicStatistics::COrderStatisticsHeap<T, LESS> &target)
        {
            return target.checksum(seed);
        }
};
