...
#undef TUPLE_FIRST_LESS
    ...
    private:
        ...
        {
            public:
                typedef std::vector<KEY> TKeyVec;

            public:
                CIndexLess(const TKeyVec &keys, const COMP &comp = COMP()) :
                    m_Keys(&keys),
                    m_Comp(comp)
                {}
    ...
    public:
        ...
        // at most N steps. Each time swap is made one more set of item are
        // in their correct place, and we update the ordering accordingly.
...
#define SIMULTANEOUS_SORT_IMPL if (boost::algorithm::is_sorted(keys.begin(), keys.end(), comp)) \
                               ...
                               std::stable_sort(ordering.begin(), ordering.end(), CIndexLess<KEY, COMP>(keys, comp)); \
...
#define CUSTOM_SWAP_VALUES swap(values[j_], values[j]);
        ...
        static bool simultaneousSort(std::vector<KEY> &keys,
        ...
        //! Simultaneously sort \p keys and \p values using the \p comp
        //! order of \p keys.
        template<typename KEY, typename VAL, typename COMP, std::size_t M, std::size_t N>
        static bool simultaneousSort(core::CSmallVector<KEY, M> &keys,
                                     core::CSmallVector<VAL, N> &values,
                                     const COMP &comp)
        {
            if (keys.size() != values.size())
            {
...
#undef CUSTOM_SWAP_VALUES
        //! Overload for default operator< comparison of \p KEY.
        template<typename KEY, typename VAL>
        static bool simultaneousSort(std::vector<KEY> &keys,
                                     std::vector<VAL> &values)
        {
            return simultaneousSort(keys, values, std::less<KEY>());
        }
        //! Overload for default operator< comparison of \p KEY.
        template<typename KEY, typename VAL, std::size_t M, std::size_t N>
        static bool simultaneousSort(core::CSmallVector<KEY, M> &keys,
                                     core::CSmallVector<VAL, N> &values)
        {
            return simultaneousSort(keys, values, std::less<KEY>());
...
#define CUSTOM_SWAP_VALUES swap(values1[j_], values1[j]); \
        ...
        template<typename KEY,
                 ...
                 typename COMP>
        static bool simultaneousSort(std::vector<KEY> &keys,
                                     std::vector<VAL1> &values1,
                                     std::vector<VAL2> &values2,
                                     const COMP &comp)
        {
        ...
        template<typename KEY,
                 ...
                 std::size_t P>
        static bool simultaneousSort(core::CSmallVector<KEY, M> &keys,
                                     core::CSmallVector<VAL1, N> &values1,
                                     core::CSmallVector<VAL2, P> &values2,
                                     const COMP &comp)
        {
            if (   keys.size() != values1.size()
                || values1.size() != values2.size())
...
#undef CUSTOM_SWAP_VALUES
        //! Overload for default operator< comparison of \p KEY.
        template<typename KEY, typename VAL1, typename VAL2>
        static bool simultaneousSort(std::vector<KEY> &keys,
                                     std::vector<VAL1> &values1,
                                     std::vector<VAL2> &values2)
        {
            return simultaneousSort(keys, values1, values2, std::less<KEY>());
        }
        //! Overload for default operator< comparison of \p KEY.
        template<typename KEY,
                 typename VAL1,
...
#undef CUSTOM_SWAP_VALUES
        //! Overload for default operator< comparison of \p KEY.
        template<typename KEY, typename VAL1, typename VAL2, typename VAL3>
        static bool simultaneousSort(std::vector<KEY> &keys,
                                     std::vector<VAL1> &values1,
                                     std::vector<VAL2> &values2,
                                     std::vector<VAL3> &values3)
        {
            return simultaneousSort(keys, values1, values2, values3, std::less<KEY>());
        }
