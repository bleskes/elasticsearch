 ...
 * strictly prohibited.
 */
#ifndef INCLUDED_prelert_api_CCsvInputParser_h
#define INCLUDED_prelert_api_CCsvInputParser_h
...
class API_EXPORT CCsvInputParser : public CInputParser
{
    public:
        ...
        //! Character to ignore at the end of fields/lines
        static const char STRIP_BEFORE_END;

    public:
        //! Construct with a string to be parsed
        //! If XML is expected in the input checkForXmlMessage should
        //! be set true this functionality if specific to Splunk.
        ...
        //! instructed to do a dry run, each record is fed through the
        //! processing twice, first in dry run mode, and then for real.
        virtual bool readStream(bool doDryRun,
    ...
    protected:
        ...
        //! Attempt to parse the next token from the working record into the
        //! working field.
        bool parseNextCsvToken(const char *end, const char *&current);
    ...
    protected:
        ...
        //! Did the separator character appear after the last CSV field we
        //! parsed?
        ...
        //! Similar to the working buffer, the working field is a raw character
        //! array rather than a string because it is built up one character at a
        //! time, and when you append a character to a string the following
        //! character has to be set to the zero terminator.  The array of
        //! characters is NOT zero terminated and hence avoids this overhead.
        //! This is something to be aware of when accessing it, but improves
        //! performance of the parsing by about 20%.  (This is the sort of
        //! optimisation to be used ONLY after careful profiling in the rare
        //! cases where the reduction in code clarity yields a large performance
        //! benefit.)  The character array is always big enough to hold the
        //! entire current row string such that the code that pulls out
        //! individual fields doesn't need to check the capacity - even if the
        //! current row has just one field, the working field array will be big
        //! enough to hold it.
...
#endif // INCLUDED_prelert_api_CCsvInputParser_h
