...
{
    public:
        //! Name of the field that contains the complete event text
        static const std::string RAW_NAME;

    public:
        //! Shared pointer to reverse search creator that we're will function
        //! after being shallow copied
        typedef boost::shared_ptr<const CTokenListReverseSearchCreatorIntf> TTokenListReverseSearchCreatorIntfCPtr;
    ...
    public:
        ...
        virtual bool createReverseSearch(const std::string &sourceType,
                                         int type,
                                         std::string &part1,
                                         std::string &part2,
    ...
    protected:
        ...
        //! second -> type deque index
        typedef std::list<TSizeSizePr>              TSizeSizePrList;
        typedef TSizeSizePrList::iterator           TSizeSizePrListItr;
        typedef TSizeSizePrList::const_iterator     TSizeSizePrListCItr;
    ...
    private:
        ...
        //! Used to hold the distinct types we compute - deque not vector as
        //! reallocations are expensive
        typedef std::deque<CTokenListType>          TTokenListTypeDeque;
        typedef TTokenListTypeDeque::iterator       TTokenListTypeDequeItr;
        typedef TTokenListTypeDeque::const_iterator TTokenListTypeDequeCItr;

        //! The types
        TTokenListTypeDeque                          m_Types;

        //! List of match count/index into type deque in descending order of
        //! match count
        ...
        typedef boost::multi_index::index_iterator<TTokenMIndex,
                                                   SToken>::type TTokenItr;
