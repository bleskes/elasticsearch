...
//! 2) Data rows
//!
//! The Splunk raw input CSV format differs from the Excel style in that
...
//! Splunk uses Python's CSV functionality to create and parse CSV data for
//! custom search commands by default Python uses the Excel style CSV format.
//!
//! Annoyingly however, when Splunk inputs raw CSV data for indexing, it
//! expects quotes to be escaped with backslashes.  Therefore, for
//! situations where the output of this class will be sent into Splunk
//! as raw input, there is the option to override the escaping character.
//! When this is done the assumption is that the both the quote character
//! and the escape character need to be escaped using the escape character.
//!
