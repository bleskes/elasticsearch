...
//! arbitrary format
//!
class API_EXPORT CAnomalyDetector : public CDataProcessor
{
    public:
        ...
        static const std::string SOURCETYPE_FIELD_NAME;
    ...
    public:
        ...
        typedef TKeyAnomalyDetectorPtrUMap::iterator        TKeyAnomalyDetectorPtrUMapItr;
        ...
        typedef model::CAnomalyDetector::TBoxPlotDataVec TBoxPlotDataVec;
        typedef TBoxPlotDataVec::const_iterator TBoxPlotDataVecCItr;
        typedef model::CBucketQueue<TBoxPlotDataVec> TBoxPlotDataVecQueue;
        typedef boost::function0<bool> TFirstProcessorPeriodicPersistFunc;
        ...
        struct SBackgroundPersistArgs
        {
            ...
            TBoxPlotDataVecQueue s_ModelDebugDataQueue;
            core_t::TTime s_Time;
    ...
    public:
        CAnomalyDetector(const std::string &jobId,
                         ...
                         CBackgroundPersister *periodicPersister = 0,
                         core_t::TTime periodicPersistInterval = -1,
        ...
        //! Set the periodic persist function of the first processor in the chain.
        virtual void firstProcessorPeriodicPersistFunc(TFirstProcessorPeriodicPersistFunc func);
    ...
    private:
        ...
        //! Check whether a background periodic persistence is appropriate at
        //! this time, and if so trigger it.
        bool checkBackgroundPersist(void);
        ...
        //! Generate debug data for the models of the specified detector in the
        //! specified time range.
        void generateModelDebugData(core_t::TTime startTime,
                                  core_t::TTime endTime,
                                  const model::CAnomalyDetector &detector);

        //! Write the pre-generated debug data to the output stream of the user's
        //! choosing: either file or streamed to the API
        void writeOutDebugData(core_t::TTime resultsTime);

        //! Write the pre-generated debug data to the output stream of the user's
        //! choosing: either file or streamed to the API
        void writeOutDebugData(core_t::TTime, CBoxPlotDataJsonWriter &writer);
    ...
    protected:
        ...
        const TAnomalyDetectorPtr &detectorForKey(core_t::TTime time,
    ...
    private:
        ...
        model::CModelConfig        &m_ModelConfig;

        //! Keep count of how many records we've handled
        uint64_t                   m_NumRecordsHandled;

        //! Map from source type to detector keys.
        TStrKeyVecUMap             m_DetectorKeys;
        ...
        //! The overall first time seen in the current search (irrespective of
        //! partition field)
        core_t::TTime              m_CurrentSearchOverallFirstTime;
        ...
        //! License restriction on whether partitions are allowed
        bool                       m_ArePartitionsAllowed;

        //! Should skip sampling
        bool                       m_SkipSampling;

        //! Pointer to periodic persister that works in the background.  May be
        //! NULL if periodic persistence is not required.
        CBackgroundPersister       *m_PeriodicPersister;

        //! At what frequency should we do periodic persistence?
        ...
        //! persistence of model state?  0 => we've never done one
        ...
        //! Also store the model debug data for the buckets for each
        //! result time - these will be output when the corresponding
        //! result is output
        TBoxPlotDataVecQueue       m_ModelDebugDataQueue;

        //! The periodic persist function of the first processor in the chain.
        TFirstProcessorPeriodicPersistFunc m_FirstProcessortPeriodicPersistFunc;

    friend class ::CBackgroundPersisterTest;
    friend class ::CAnomalyDetectorTest;
};
