...
//! detector.1.sourcetype = farequote
//!
...
class API_EXPORT CFieldConfig
{
    public:
        ...
        //! Prefix for list setting
        static const std::string LIST_PREFIX;
        ...
        //! Splunk command line setting used to specify an influencer field
    ...
    public:
        ...
        class API_EXPORT CFieldOptions
        {
            public:
                ...
                //! detector-specific.
                std::ostream &appendConfigClause(std::ostream &strm) const;

                //! Write the config file entries for one detector.
                std::ostream &appendConfigSegment(const std::string &summaryCountFieldName,
                                                  const std::string &categorizatationFieldName,
                                                  std::ostream &strm) const;

                //! As above but replace the current config key with a specific
                //! config key passed as an argument.
                std::ostream &appendConfigSegment(int configKeyOverride,
    ...
    public:
        ...
        //! Index of field names to field options for a single sourcetype.
    ...
    public:
        ...
        CFieldConfig(bool allowPopulationConfigs = true);
        ...
        bool initFromCmdLine(const std::string &configFile,
                             const TStrVec &tokens);

        //! Initialise from a config file.
        bool initFromFile(const std::string &configFile);

        //! Write the current field config to a file.
        bool writeToFile(const std::string &configFile, bool forceNewKeys = false) const;
        ...
        //! Access the superset of all field names that are enabled for any
        //! sourcetype or by default, either as analysis fields or as "by"
        //! fields.
        ...
        //! Accessor
        bool allowPopulationConfigs(void) const;

        //! Set flag for allowing population configs - call this BEFORE
        //! initialising from a file or command line to change the default set
        //! by the constructor
        void allowPopulationConfigs(bool allowPopulationConfigs);
    ...
    private:
        ...
        //! Split a config clause on whitespace and commas.  This uses
        //! Splunk rules for quoting and escaping.
        bool tokenise(const std::string &clause,
                      TStrVec &copyTokens);
        ...
        std::string findParameter(const std::string &parameter,
                                  TStrVec &copyTokens);
        ...
        //! Add influencer fields for all the by/over/partition fields of
        //! configurations for the given sourcetype
        void addInfluencerFieldsFromByOverPartitionFields(const TFieldOptionsMIndex &fieldsForSourceType);

        //! Store one influencer field name
        void addInfluencerFieldName(const std::string &influencer,
                                    bool quiet = false);
        ...
        //! Process and store a rule list
        bool processList(const std::string &key,
                         const std::string &value);

        //! Write the config file entries
        std::ostream &appendConfigSegment(const TFieldOptionsMIndex &fieldsForSourceType,
                                          bool forceNewKeys,
                                          int &counter,
                                          std::ostream &strm) const;

        //! Debug dump of fields for sourcetype
        void debug(const TFieldOptionsMIndex &fieldsForSourceType,
                   std::ostream &strm) const;

    private:
        //! Are population configs allowed?  (This is a legacy of compare mode,
        //! which didn't support population modelling.  Now this should always
        //! be true.)
        bool                              m_AllowPopulationConfigs;

        //! The fields options.
        TFieldOptionsMIndex               m_FieldOptions;

        //! The superset of all field names that are enabled for any sourcetype
        //! or by default, either as analysis fields or as "by" fields.
        ...
        //! The lists per id used by categorical rule conditions.
        TStrPatternSetUMap                m_RuleLists;
