...
class API_EXPORT CLengthEncodedInputParser : public CInputParser
{
    public:
        ...
        //! never called for this input parser.  The supplied reader function is
        //! called once per record.  If the supplied reader function returns
        //! false, reading will stop.  This method keeps reading until it
        //! reaches the end of the stream or an error occurs.  If it
        //! successfully reaches the end of the stream it returns true,
        //! otherwise it returns false.  If instructed to do a dry run, each
        //! record is fed through the processing twice, first in dry run mode,
        //! and then for real.
