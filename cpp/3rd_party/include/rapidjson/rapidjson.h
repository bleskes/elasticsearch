...
// On Solaris direct inclusion of inttypes.h causes a raft of macro
// redefinition warning messages, so include sys/types.h instead.
#include <sys/types.h>
#else
#include <inttypes.h>
#endif
...
private:
	static const size_t kAlignMask = sizeof(void *) - 1; //!< Used to align memory allocations on word boundaries
public:
	...
	*/
		...
		chunkHead_(0), chunk_capacity_(chunkSize), userBuffer_(0), baseAllocator_(baseAllocator), ownBaseAllocator_(0)
	{
		...
		uintptr_t alignedBuffer = (reinterpret_cast<uintptr_t>(buffer) + static_cast<uintptr_t>(kAlignMask))
				& ~static_cast<uintptr_t>(kAlignMask);
		size_t adj = alignedBuffer - reinterpret_cast<uintptr_t>(buffer);
		RAPIDJSON_ASSERT(size > sizeof(ChunkHeader) + adj);
		userBuffer_ = reinterpret_cast<ChunkHeader*>(alignedBuffer);
		chunkHead_ = userBuffer_;
		chunkHead_->capacity = size - sizeof(ChunkHeader) - adj;
		chunkHead_->size = 0;
		chunkHead_->next = 0;
		if (!baseAllocator_)
			ownBaseAllocator_ = baseAllocator_ = new BaseAllocator();
	...
	*/
	...
	void* Malloc(size_t size) {
		if (size > 0)
		{
			// Round the requested size up to the next highest multiple of the
			// size of a pointer to avoid alignment problems
			size = (size + kAlignMask) & ~kAlignMask;
			if (chunkHead_->size + size > chunkHead_->capacity)
				AddChunk(chunk_capacity_ > size ? chunk_capacity_ : size);
		}
		char *buffer = reinterpret_cast<char *>(chunkHead_ + 1) + chunkHead_->size;
	...
	void* Realloc(void* originalPtr, size_t originalSize, size_t newSize) {
		if (originalPtr == 0 || originalSize == 0)
		...
		if (originalPtr == reinterpret_cast<char *>(chunkHead_ + 1) + chunkHead_->size - originalSize) {
			size_t increment = (newSize - originalSize + kAlignMask) & ~kAlignMask;
...
private:
	...
	*/
	void AddChunk(size_t capacity) {
		ChunkHeader* chunk = static_cast<ChunkHeader*>(baseAllocator_->Malloc(sizeof(ChunkHeader) + capacity));
	...
	*/
	...
	ChunkHeader* userBuffer_;			//!< User supplied buffer.
