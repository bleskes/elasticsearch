	...
	GenericValue& AddMember(GenericValue& name, GenericValue& value, Allocator& allocator) {
		...
		if (o.size >= o.capacity) {
			if (o.capacity == 0) {
				...
				o.members = static_cast<Member*>(allocator.Malloc(o.capacity * sizeof(Member)));
			}
			else {
				SizeType oldCapacity = o.capacity;
				o.capacity *= 2;
				o.members = static_cast<Member*>(allocator.Realloc(o.members, oldCapacity * sizeof(Member), o.capacity * sizeof(Member)));
			}
		}
...
private:
	...
	void SetStringRaw(const Ch* s, SizeType length, Allocator& allocator) {
		...
		data_.s.str = static_cast<char *>(allocator.Malloc(length + 1));
		data_.s.length = length;
		memcpy(const_cast<Ch*>(data_.s.str), s, length);
	...
	*/
	...
	GenericDocument& ParseStream(Stream& stream) {
		...
		if (reader.template Parse<parseFlags>(stream, *this)) {
			...
			// This reference is a workaround to get the line after it to
			// compile with both clang and Visual C++
			ValueType &base = *this;
			base.RawAssign(*stack_.template Pop<ValueType>(1));
			parseError_ = 0;
			errorOffset_ = 0;
		}
		else {
...
private:
	...
	void EndObject(SizeType memberCount) {
		...
		stack_.template Top<ValueType>()->SetObjectRaw(members, static_cast<SizeType>(memberCount), GetAllocator());
	}
