...
public final class BytesRefValComparator extends NestedWrappableComparator<BytesRef> {
    ...
    private static final class MultiValuedBytesWrapper extends FilteredByteValues {
        ...
        private final BytesRef multiValueScratch = new BytesRef();
        ...
        protected void swapByteRefs(BytesRef a, BytesRef b) {
            byte[] tempBytes = a.bytes;
            int tempLength = a.length;
            int tempOffset = a.offset;
            a.bytes = b.bytes;
            a.offset = b.offset;
            a.length = b.length;
            b.bytes = tempBytes;
        ...
        public BytesRef getValueScratch(int docId, BytesRef relevantVal) {
            ...
            iter.next(relevantVal);

            while (iter.hasNext()) {
                iter.next(multiValueScratch);
                int cmp = multiValueScratch.compareTo(relevantVal);
                if (sortMode == SortMode.MAX) {
                    if (cmp > 0) {
                        // swap the bytes from relevant with scratch, this allows us not to always use bytes generated by the underlying iter
                        // and thus not copy them to our own buffer
                        swapByteRefs(relevantVal, multiValueScratch);
                    }
                } else {
                    if (cmp < 0) {
                        swapByteRefs(relevantVal, multiValueScratch);
