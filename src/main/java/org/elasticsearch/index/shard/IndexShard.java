...
import org.elasticsearch.index.sequence.ReplicaSequenceNumberService;
...
public class IndexShard extends AbstractIndexShardComponent implements IndexShardComponent {
    ...
    private final ReplicaSequenceNumberService replicaSequenceNumberService;
    ...
    public IndexShard(ShardId shardId, @IndexSettings Settings indexSettings, IndexSettingsService indexSettingsService, IndicesLifecycle indicesLifecycle, Store store, MergeSchedulerProvider mergeScheduler, Translog translog,
                      ...
                      @Nullable IndicesWarmer warmer, SnapshotDeletionPolicy deletionPolicy, AnalysisService analysisService, SimilarityService similarityService, MergePolicyProvider mergePolicyProvider, PrimarySequenceNumberService primarySequenceNumberService, ReplicaSequenceNumberService replicaSequenceNumberService, EngineFactory factory) {
        super(shardId, indexSettings);
        this.primarySequenceNumberService = primarySequenceNumberService;
        this.replicaSequenceNumberService = replicaSequenceNumberService;
        ...
        this.config = new EngineConfig(shardId,
                ...
                threadPool, indexingService, indexSettingsService, this.primarySequenceNumberService, warmer, store, deletionPolicy, translog, mergePolicyProvider, mergeScheduler,
    ...
    public Engine.Create prepareCreate(SourceToParse source, long version, VersionType versionType) throws ElasticsearchException {
        ...
        return new Engine.Create(docMapper.v1(), docMapper.v1().uidMapper().term(doc.uid().stringValue()), doc, SequenceNo.UNKNOWN, version, versionType, startTime);
    }

    public ParsedDocument create(Engine.Create create) throws ElasticsearchException {
        // we only create on primary (replicas use index)
        writeAllowed(true);
        create = indexingService.preCreate(create);
        try {
            if (logger.isTraceEnabled()) {
                logger.trace("create [{}][{}]{}", create.type(), create.id(), create.docs());
            }
            engineSafe().performOperation(create, true);
            create.endTime(System.nanoTime());
        } catch (Throwable ex) {
            indexingService.postCreate(create, ex);
            throw ex;
        } finally {
            replicaSequenceNumberService.observe(create.sequenceNo());
        }
    ...
    public Engine.Index prepareIndex(SourceToParse source, long version, VersionType versionType) throws ElasticsearchException {
        return prepareIndex(source, version, versionType, SequenceNo.UNKNOWN);
    }

    public Engine.Index prepareIndex(SourceToParse source, long version, VersionType versionType, SequenceNo sequenceNo) throws ElasticsearchException {
        long startTime = System.nanoTime();
        Tuple<DocumentMapper, Boolean> docMapper = mapperService.documentMapperWithAutoCreate(source.type());
        ParsedDocument doc = docMapper.v1().parse(source).setMappingsModified(docMapper);
        return new Engine.Index(docMapper.v1(), docMapper.v1().uidMapper().term(doc.uid().stringValue()), doc, sequenceNo, version, versionType, startTime);
    }

    public ParsedDocument index(Engine.Index index, boolean asPrimary) throws ElasticsearchException {
        writeAllowed(asPrimary);
        index = indexingService.preIndex(index);
        try {
            if (logger.isTraceEnabled()) {
                logger.trace("index [{}][{}]{}", index.type(), index.id(), index.docs());
            }
            engineSafe().performOperation(index, asPrimary);
            index.endTime(System.nanoTime());
        } catch (Throwable ex) {
            indexingService.postIndex(index, ex);
            throw ex;
        } finally {
            replicaSequenceNumberService.observe(index.sequenceNo());
        }
        indexingService.postIndex(index);
        return index.parsedDoc();
    }

    public Engine.Delete prepareDelete(String type, String id, long version, VersionType versionType) throws ElasticsearchException {
        return prepareDelete(type, id, version, versionType, SequenceNo.UNKNOWN);
    }

    public Engine.Delete prepareDelete(String type, String id, long version, VersionType versionType, SequenceNo sequenceNo) throws ElasticsearchException {
        long startTime = System.nanoTime();
        DocumentMapper docMapper = mapperService.documentMapperWithAutoCreate(type).v1();
        return new Engine.Delete(type, id, docMapper.uidMapper().term(type, id), sequenceNo, version, versionType, startTime, false);
    }

    public void delete(Engine.Delete delete, boolean asPrimary) throws ElasticsearchException {
        writeAllowed(asPrimary);
        ...
        try {
            ...
            engineSafe().performOperation(delete, asPrimary);
    ...
    public Engine.DeleteByQuery prepareDeleteByQuery(BytesReference source, @Nullable String[] filteringAliases, String... types) throws ElasticsearchException {
        ...
        return new Engine.DeleteByQuery(query, source, filteringAliases, aliasFilter, parentFilter, startTime, types);
    }

    public void deleteByQuery(Engine.DeleteByQuery deleteByQuery, boolean asPrimary) throws ElasticsearchException {
        writeAllowed(asPrimary);
        if (logger.isTraceEnabled()) {
    ...
    /**
     * Performs a single recovery operation, and returns the operation if it resulting in an {@link Engine.Index} operation (or null if not)
     * that can then be used for mapping updates (for example) if needed.
     */
    public Engine.Index performRecoveryOperation(Translog.Operation operation) throws ElasticsearchException {
        if (state != IndexShardState.RECOVERING) {
            throw new IndexShardNotRecoveringException(shardId, state);
        }
        Engine.IndexingOperation indexingOperation = null;
        Engine.Index retValue = null;
        try {
            switch (operation.opType()) {
                ...
                case SAVE:
                    ...
                    Engine.Index engineIndex = prepareIndex(source(index.source()).type(index.type()).id(index.id())
                            ...
                            index.version(), index.versionType(), index.sequenceNo());
                    indexingOperation = retValue = engineIndex;
                    engineSafe().performOperation(engineIndex, false);
                    break;
                case DELETE:
                    Translog.Delete delete = (Translog.Delete) operation;
                    Uid uid = Uid.createUid(delete.uid().text());
                    Engine.Delete engineDelete = prepareDelete(uid.type(), uid.id(), delete.version(), delete.versionType(), delete.sequenceNo());
                    indexingOperation = engineDelete;
                    engineSafe().performOperation(engineDelete, false);
                    break;
                case DELETE_BY_QUERY:
                    Translog.DeleteByQuery deleteByQuery = (Translog.DeleteByQuery) operation;
                    engineSafe().delete(prepareDeleteByQuery(deleteByQuery.source(), deleteByQuery.filteringAliases(), deleteByQuery.types()));
                    break;
                default:
            ...
            if (indexingOperation != null) {
                replicaSequenceNumberService.observe(indexingOperation.sequenceNo());
            }
        }
        return retValue;
    }
    ...
    private void writeAllowed(boolean asPrimary) throws IllegalIndexShardStateException {
        IndexShardState state = this.state; // one time volatile read
        ...
        if (asPrimary) {
            ...
            if (state != IndexShardState.STARTED && state != IndexShardState.RELOCATED) {
                throw new IllegalIndexShardStateException(shardId, state, "operation only allowed when started/recovering [acting as primary]");
            }
        } else {
            ...
            if (state != IndexShardState.STARTED && state != IndexShardState.RELOCATED && state != IndexShardState.RECOVERING && state != IndexShardState.POST_RECOVERY) {
                throw new IllegalIndexShardStateException(shardId, state, "operation only allowed when started/recovering [acting as replica]");
            }
        }
    }
