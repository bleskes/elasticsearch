...
public class TransportIndexAction extends TransportShardReplicationOperationAction<IndexRequest, IndexRequest, IndexResponse> {
    ...
    protected Tuple<IndexResponse, IndexRequest> shardOperationOnPrimary(ClusterState clusterState, PrimaryOperationRequest shardRequest) {
        ...
        request.version(index.version());
        request.versionType(index.versionType());
        request.sequenceNo(index.sequenceNo());

        assert request.versionType().validateVersionForWrites(request.version());

        return new Tuple<>(new IndexResponse(shardRequest.shardId.getIndex(), request.type(), request.id(), index.version(), index.sequenceNo(), index.created()), request);
    }
    ...
    protected ShardReplicationOperationReplicaResponse shardOperationOnReplica(ReplicaOperationRequest shardRequest) {
        ...
        SourceToParse sourceToParse = SourceToParse.source(SourceToParse.Origin.REPLICA, request.source()).type(request.type()).id(request.id())
                .routing(request.routing()).parent(request.parent()).timestamp(request.timestamp()).ttl(request.ttl());
        Engine.Index index = indexShard.prepareIndex(sourceToParse, request.version(), request.versionType(), request.sequenceNo());
