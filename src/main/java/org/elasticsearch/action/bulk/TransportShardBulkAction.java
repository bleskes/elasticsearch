...
public class TransportShardBulkAction extends TransportShardReplicationOperationAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {
    ...
    protected Tuple<BulkShardResponse, BulkShardRequest> shardOperationOnPrimary(ClusterState clusterState, PrimaryOperationRequest shardRequest) {
        ...
        for (int requestIndex = 0; requestIndex < request.items().length; requestIndex++) {
            ...
            if (item.request() instanceof IndexRequest) {
                ...
                try {
                    try {
                        WriteResult result = shardIndexOperationOnPrimary(request, indexRequest, clusterState, indexShard, true);
                        // add the response
    ...
    private WriteResult shardIndexOperationOnPrimary(BulkShardRequest request, IndexRequest indexRequest, ClusterState clusterState,
        ...
        try {
            ...
            indexRequest.versionType(op.versionType());
            indexRequest.version(op.version());
            indexRequest.sequenceNo(op.sequenceNo());
        } catch (Throwable t) {
        ...
        IndexResponse indexResponse = new IndexResponse(request.index(), indexRequest.type(), indexRequest.id(), op.version(), op.sequenceNo(), op.created());
    ...
    private WriteResult shardDeleteOperation(BulkShardRequest request, DeleteRequest deleteRequest, IndexShard indexShard) {
        ...
        deleteRequest.versionType(delete.versionType());
        deleteRequest.version(delete.version());
        deleteRequest.sequenceNo(delete.sequenceNo());

        assert deleteRequest.versionType().validateVersionForWrites(deleteRequest.version());

        DeleteResponse deleteResponse = new DeleteResponse(request.index(), deleteRequest.type(), deleteRequest.id(), delete.version(), delete.sequenceNo(), delete.found());
    ...
    private UpdateResult shardUpdateOperation(ClusterState clusterState, BulkShardRequest bulkShardRequest, UpdateRequest updateRequest, IndexShard indexShard) {
        ...
        switch (translate.operation()) {
            ...
            case INDEX:
                ...
                try {
                    WriteResult result = shardIndexOperationOnPrimary(bulkShardRequest, indexRequest, clusterState, indexShard, false);
            ...
            if (item.request() instanceof IndexRequest) {
                ...
                try {
                    SourceToParse sourceToParse = SourceToParse.source(SourceToParse.Origin.REPLICA, indexRequest.source()).type(indexRequest.type()).id(indexRequest.id())
                            .routing(indexRequest.routing()).parent(indexRequest.parent()).timestamp(indexRequest.timestamp()).ttl(indexRequest.ttl());
                    Engine.Index index = indexShard.prepareIndex(sourceToParse, indexRequest.version(), indexRequest.versionType(), indexRequest.sequenceNo());
                    indexShard.index(index, false);
                } catch (Throwable e) {
                ...
                DeleteRequest deleteRequest = (DeleteRequest) item.request();
                try {
                    Engine.Delete delete = indexShard.prepareDelete(deleteRequest.type(), deleteRequest.id(), deleteRequest.version(), deleteRequest.versionType(), deleteRequest.sequenceNo());
