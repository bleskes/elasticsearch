...
public class MembershipAction extends AbstractComponent {
    ...
    public static final String DISCOVERY_JOIN_ACTION_NAME = "internal:discovery/raft/join";
    public static final String DISCOVERY_JOIN_VALIDATE_ACTION_NAME = "internal:discovery/raft/join/validate";
    public static final String DISCOVERY_LEAVE_ACTION_NAME = "internal:discovery/raft/leave";
    ...
    public MembershipAction(Settings settings, TransportService transportService, MembershipListener listener, RaftDiscovery raftDiscovery) {
        super(settings);
     ...
     */
    public void sendValidateJoinRequestBlocking(long term, DiscoveryNode node, TimeValue timeout) throws ElasticsearchException {
        transportService.submitRequest(node, DISCOVERY_JOIN_VALIDATE_ACTION_NAME, new ValidateJoinRequest(term), EmptyTransportResponseHandler.INSTANCE_SAME)
                .txGet(timeout.millis(), TimeUnit.MILLISECONDS);
    }
    ...
    private class ValidateJoinRequestRequestHandler extends BaseTransportRequestHandler<ValidateJoinRequest> {
        ...
        public void messageReceived(ValidateJoinRequest request, TransportChannel channel) throws Exception {
            raftDiscovery.handleValidateJoin(request.term);
            channel.sendResponse(TransportResponse.Empty.INSTANCE);
        }
