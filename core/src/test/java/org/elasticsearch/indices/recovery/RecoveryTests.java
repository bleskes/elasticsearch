...
public class RecoveryTests extends ESIndexLevelReplicationTestCase {
    ...
    public void testDifferentTranslogUUIDDisablesOPsRecovery() throws Exception {
        try (ReplicationGroup shards = createGroup(1)) {
            shards.startAll();
            ...
            try (IndexWriter writer = new IndexWriter(replica.store().directory(), iwc)) {
                userData.put(Translog.TRANSLOG_UUID_KEY, "_not_the_chosen_one_");
            ...
            // translog uuid was restored
            final String translogUUID = shards.getPrimary().getTranslog().getTranslogUUID();
            assertThat(newReplica.getTranslog().getTranslogUUID(), equalTo(translogUUID));
            assertThat(newReplica.commitStats().getUserData().get(Translog.TRANSLOG_UUID_KEY), equalTo(translogUUID));
