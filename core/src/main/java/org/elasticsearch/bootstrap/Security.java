...
final class Security {
    ...
    static void addBindPermissions(Permissions policy, Settings settings) throws IOException {
        String httpRange = NettyHttpServerTransport.portFromSettings(settings);
        if (httpRange != null) {
            policy.add(new SocketPermission("localhost:" + httpRange, "listen,resolve"));
        }

        for (String port : NettyTransport.extractPortsFromSettings(settings)) {
            policy.add(new SocketPermission("localhost:" + port, "listen,resolve"));
        }
    }
