 ...
 */
public class ImmutableTranslogReader extends TranslogReader implements Closeable {
     ...
     */
    public ImmutableTranslogReader(Checkpoint checkpoint, ChannelReference channelReference, long firstOperationOffset) {
        super(checkpoint.generation, channelReference, firstOperationOffset);
            ...
            if (b1 == LUCENE_CODEC_HEADER_BYTE) {
                ...
                switch (version) {
                    ...
                    case TranslogWriter.VERSION_CHECKPOINTS:
                        ...
                        return new ImmutableTranslogReader(checkpoint, channelReference, ref.length + CodecUtil.headerLength(TranslogWriter.TRANSLOG_CODEC) + RamUsageEstimator.NUM_BYTES_INT);
                    default:
    ...
    public long getLocalCheckpoint() {
        return checkpoint.localCheckpoint;
    }
    ...
    protected void readBytes(ByteBuffer buffer, long position) throws IOException {
        if (position >= checkpoint.offset) {
            throw new EOFException("read requested past EOF. pos [" + position + "] end: [" + checkpoint.offset + "]");
        }
    ...
    protected void ensureOpen() {
        if (isClosed()) {
            throw new AlreadyClosedException("translog immutable reader [" + getGeneration() + "] is already closed");
        }
    }
