...
public class ProcessCtrlTests extends ESTestCase {
    ...
    // assertTrue(command.contains(ProcessCtrl.SUMMARY_COUNT_FIELD_ARG +
    // "summaryField"));
    // assertTrue(command.contains(ProcessCtrl.RESULT_FINALIZATION_WINDOW_ARG +
    // "2"));
    ...
    // int expectedMaxQuantileInterval = 21600 +
    // ProcessCtrl.calculateStaggeringInterval(job.getId());
    // assertTrue(command.contains(ProcessCtrl.MAX_QUANTILE_INTERVAL_ARG +
    // expectedMaxQuantileInterval));
    // assertTrue(String.join(", ", command),
    // command.contains(ProcessCtrl.PERSIST_URL_BASE_ARG +
    // "http://localhost:" + ProcessCtrl.ES_HTTP_PORT +
    // "/prelertresults-unit-test-job"));
    ...
    //
    // List<String> command = ProcessCtrl.buildNormaliserCommand(env, jobId,
    // 300, true);
