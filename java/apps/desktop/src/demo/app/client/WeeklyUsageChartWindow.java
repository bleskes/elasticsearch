 ...
 */
public class WeeklyUsageChartWindow extends Window
{
	...
	public WeeklyUsageChartWindow(DesktopApp desktop)
	{
		m_Desktop = desktop;
	...
	{
		m_Desktop.getUserUsageQueryServiceInstance().getTotalWeeklyUsageData("total", 
	        ...
	        public void onSuccess(List<UsageRecord> records)
	        {
	        	...
	        	// Set the x axis range for this week.
	        	UsageRecord lastRecord = records.get(records.size() - 1);
	        	Date lastTime = lastRecord.getTime();
	        	GWT.log("Last time is " + lastTime, null);
	        	Date endDate = new Date(lastTime.getYear(), lastTime.getMonth(), lastTime.getDate());
	        	endDate = new Date(endDate.getTime() + (24*60*60*1000)); // Adjust to midnight on day of last record.
	        	Date startDate = new Date(endDate.getTime() - (7*24*60*60*1000));
	        	
	        	GWT.log("startDate: " + startDate + ", endDate: " + endDate, null);
	        	
	        	m_LineChart.setXAxisRange(startDate, endDate, 8, 2);
