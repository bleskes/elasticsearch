...
public class EvidenceHistoryWindow extends ViewWindow
{
	 ...
	 */
	public EvidenceHistoryWindow(Desktop desktop, TimeFrame timeFrame, 
	...
	{	
		...
		EvidenceView evidenceView = m_HistoryView.getEvidenceView(m_TimeFrame);	
		if (evidenceView != null)
		{
			m_Loader.setSortField(evidenceView.getTimeColumnName());
	...
	public void setTimeFrame(TimeFrame timeFrame)
    {
		...
		{			
			...
			ColumnModel newColumnModel = this.buildColumnModel(m_TimeFrame);
	 ...
	 * @return the history view displayed in the Window.
	 */
    public View getView()
    {
	...
	public void runListViewTool(ListViewTool tool)
	{
		...
		{
			m_Desktop.openEvidenceViewForType(getDataType(), selectedRecord.getId());
		}
	}
	...
	/**
	 * Runs a View Tool against the selected row of evidence to launch a new Usage View.
	 * No action is taken if no row is currently selected.
	...
	public void runUsageViewTool(UsageViewTool tool)
	{
		...
		{
        	Date date = ClientUtil.parseTimeField(selectedEvRecord, m_TimeFrame);
			String source = null;
			if (tool.getSourceArg() != null)
			{
				source = selectedEvRecord.get(tool.getSourceArg());
			}
			String attributeValue = null;
			if (tool.getAttributeValueArg() != null)
			{
				attributeValue = selectedEvRecord.get(tool.getAttributeValueArg());
			}
	...
	protected ColumnModel buildColumnModel(TimeFrame timeFrame)
	{
		...
		for (String columnName : allColumns)
		{
		    ...
		    if (columnName.equals("probable_cause") )
			{
		    	...
		    	columnConf.setRenderer(ProbableCauseCellRenderer.getInstance());
			}
