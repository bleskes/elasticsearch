 ...
 * user to select a date, and a TimeField combo box component below, which
 * allows the user to pick a time from 15 minute intervals.
 * <p>
 ...
 * ADempiereGWT/client/src/com/extjs/gxt/ui/client/widget/DateTimePicker.java
 * 
 * <dt><b>Events:</b></dt>
 * 
 * <dd><b>Select</b> : DatePickerEvent(datePicker, date)<br>
 * <div>Fires when the OK button has been pressed.</div>
 * <ul>
 ...
 * </dd> 
 * 
 * <dd><b>Close</b> : DatePickerEvent(datePicker, date)<br>
 * <div>Fires when the Cancel button has been pressed.</div>
 * <ul>
 * <li>datePicker : this</li>
 * <li>date : the selected date</li>
 * </ul>
...
public class DateTimePicker extends BoxComponent
{
	...
	private HorizontalPanel m_TimePanel;
	...
	/**
	 * Returns the date time picker messages.
	 * 
	...
	/**
	 * Sets the date time picker messages.
	 * 
	...
	{
		m_Value = new DateWrapper(date);// .clearTime();
	...
	/**
	 * Sets the value in the time field of the date time picker.
	 * @param value	the Time value to set in the time field.
	 */
	...
	/**
	 * Returns the current selected time value in the time picker.
	...
	protected void doAttachChildren()
	{
		...
		ComponentHelper.doAttach(m_TimePanel);
	...
	protected void doDetachChildren()
	{
		...
		ComponentHelper.doDetach(m_TimePanel);
	...
	protected void onRender(Element target, int index)
	{
		...
		m_TimePanel = new HorizontalPanel();
		m_TimePanel.setHorizontalAlign(HorizontalAlignment.CENTER);
		m_TimePanel.setStyleName("time-picker-time-panel");

		if (GXT.isIE)
		{
			m_TimePanel.setWidth(175);
		}
		...
		spacerLeft.setWidth(35);

		m_TimePanel.add(spacerLeft, new TableData(HorizontalAlignment.CENTER,
		        VerticalAlignment.MIDDLE));
		m_TimePanel.add(m_HoursSpinner, new TableData(HorizontalAlignment.CENTER,
		        VerticalAlignment.MIDDLE));
		m_TimePanel.add(new LabelField(":"), new TableData(
		        HorizontalAlignment.CENTER, VerticalAlignment.MIDDLE));
		m_TimePanel.add(m_MinsSpinner, new TableData(HorizontalAlignment.CENTER,
		        VerticalAlignment.MIDDLE));
		...
		getElement().appendChild(m_TimePanel.getElement());
	...
	{
		String ok = messages.getOkText() != null ? 
				messages.getOkText() : myMessages.datePicker_okText();
		String cancel = messages.getCancelText() != null ? 
				messages.getCancelText() : myMessages.datePicker_cancelText();
		...
		for (int i = 0; i < 6; i++)
		{
			...
			if (i == 0)
			{
				buf.append("<td class=x-date-mp-ybtn align=center><a class=x-date-mp-prev href=#></a></td>");
				buf.append("<td class='x-date-mp-ybtn' align=center><a class='x-date-mp-next'></a></td></tr>");
			}
			else
			{
				buf.append("<td class='x-date-mp-year'><a href='#'></a></td>");
				buf.append("<td class='x-date-mp-year'><a href='#'></a></td></tr>");
			}
		}
	...
	/**
	 * Defines the header component in the Date Time picker, which displays 
	 * the month field and controls for changing the month value.
	 */
