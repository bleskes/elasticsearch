...
/**
 * A specialized toolbar that is bound to a {@link DatePagingLoader} and provides
 * automatic paging controls to navigate across a range of dates.
 * 
 * <dl>
...
{
	protected DatePagingLoader<DatePagingLoadConfig, DatePagingLoadResult> m_Loader;
	...
	protected Label m_DisplayText;
	protected LabelField m_PageText;
	protected PagingToolBarMessages m_Msgs;
	protected boolean m_ShowToolTips = true;

	private boolean m_ReuseConfig = true;
	private LoadEvent<DatePagingLoadConfig, DatePagingLoadResult> m_RenderEvent;
	...
	protected void onLoad(
	        LoadEvent<DatePagingLoadConfig, DatePagingLoadResult> event)
	{
		if (!rendered)
		{
			m_RenderEvent = event;
		...
		DatePagingLoadResult result = event.data;
		...
		m_DisplayText.setText(getDisplayText());
	}
	...
	protected void onRender(Element target, int index)
	{
		...
		//m_RefreshTool = new SplitButton("&#160;");	// To fix bug with Firefox / Ext GWT 1.2 - some text is needed.
		...
		m_PageText = new LabelField();

		m_DisplayText = new Label();
		...
		m_ToolBar.add(new AdapterToolItem(timeFrameGroup));
		...
		m_ToolBar.add(new SeparatorToolItem());
		m_ToolBar.add(new AdapterToolItem(m_RefreshTool));
		...
		m_ToolBar.add(new AdapterToolItem(m_DisplayText));
	 ...
	 * @return the end time of the current time frame.
	 */
