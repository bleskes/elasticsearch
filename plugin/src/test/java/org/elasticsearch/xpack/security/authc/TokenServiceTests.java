...
public class TokenServiceTests extends ESTestCase {
    ...
    public void testTokenExpiry() throws Exception {
        ClockMock xpackMockClock = ClockMock.frozen();
        ...
        try (ThreadContext.StoredContext ignore = requestContext.newStoredContext(true)) {
            ...
            xpackMockClock.fastForwardSeconds(fastForwardAmount);
            PlainActionFuture<UserToken> future = new PlainActionFuture<>();
            tokenService.getAndValidateToken(requestContext, future);
            assertEquals(authentication, future.get().getAuthentication());
        }
        ...
        try (ThreadContext.StoredContext ignore = requestContext.newStoredContext(true)) {
            // move to expiry
            xpackMockClock.fastForwardSeconds(Math.toIntExact(defaultExpiration.getSeconds()) - fastForwardAmount);
            PlainActionFuture<UserToken> future = new PlainActionFuture<>();
            tokenService.getAndValidateToken(requestContext, future);
            assertEquals(authentication, future.get().getAuthentication());
        }
        ...
        try (ThreadContext.StoredContext ignore = requestContext.newStoredContext(true)) {
            ...
            xpackMockClock.fastForwardSeconds(1);
            PlainActionFuture<UserToken> future = new PlainActionFuture<>();
            tokenService.getAndValidateToken(requestContext, future);
            ElasticsearchSecurityException e = expectThrows(ElasticsearchSecurityException.class, future::actionGet);
            final String headerValue = e.getHeader("WWW-Authenticate").get(0);
