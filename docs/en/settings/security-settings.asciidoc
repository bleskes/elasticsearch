...
You can set the following document and field level security settings in
`elasticsearch.yml`. For more information, see
{xpack-ref}/field-and-document-access-control.html[Setting Up Document and Field Level Security].
...
The valid settings vary depending on the realm type. For more information, see
{xpack-ref}/setting-up-authentication.html[Setting Up Authentication].
...
user credentials. See the
{xpack-ref}/controlling-user-cache.html#controlling-user-cache[Cache hash algorithms]
...
values see
{xpack-ref}/ldap-realm.html#ldap-load-balancing[LDAP load balancing and failover types].
...
if `user_search.base_dn` is specified. For more information on the different
modes, see {xpack-ref}/ldap-realm.html[LDAP realms].
...
`files.role_mapping`::
The {xpack-ref}/security-files.html[location] for the
{xpack-ref}/mapping-roles.html#mapping-roles[YAML role mapping configuration file].
...
in-memory cached user credentials (see
{xpack-ref}/controlling-user-cache.html#controlling-user-cache[Cache hash algorithms]
table for all possible values). Defaults to `ssha256`.
...
The behavior to use when there are multiple LDAP URLs defined. For supported
values see
{xpack-ref}/active-directory-realm.html#ad-load-balancing[load balancing and failover types].
Defaults to  `failover`.
...
the in-memory cached user credentials (see
{xpack-ref}/controlling-user-cache.html#controlling-user-cache[Cache hash algorithms]
table for all possible values). Defaults to `ssha256`.
...
==== IP Filtering Settings
You can configure the following settings for
{xpack-ref}/ip-filtering.html[IP filtering].
